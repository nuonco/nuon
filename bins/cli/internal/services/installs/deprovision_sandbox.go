...
func (s *Service) DeprovisionSandbox(ctx context.Context, installID string, role string, asJSON bool) error {
	installID, err := lookup.InstallID(ctx, s.api, installID)
	if err != nil {
		return ui.PrintError(err)
	}

	resp, err := s.api.DeprovisionInstallSandbox(ctx, installID, role)
	if err != nil {
