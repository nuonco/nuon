...
Example — register:
  ...
  # Cursor: save the JSON below to ~/.cursor/mcp.json, then agent mcp enable nuon

Or by config file — .mcp.json for Claude Code, ~/.cursor/mcp.json or
.cursor/mcp.json for Cursor, the same block under the literal
"amp.mcpServers" key in ~/.config/amp/settings.json for Amp:
