...
func (c *cli) agentsContextMarkdown() string {
	...
	b.WriteString("```json\n{\"mcpServers\": {\"nuon\": {\"command\": \"nuon\", \"args\": [\"agents\", \"mcp\", \"--allow-writes\"]}}}\n```\n\n")
	b.WriteString(fmt.Sprintf("The upstream URL above (`%s`) comes from `api_url` in the CLI config. Override it with `--url`, and the name in the client's MCP list with `--name`, by putting those flags on the registered command:\n\n", mcpURL))
	b.WriteString("```bash\nnuon agents mcp --allow-writes --url https://mcp.example.com/mcp --name nuon-example\n")
	b.WriteString("claude mcp add --transport stdio nuon -- nuon agents mcp --allow-writes --url https://mcp.example.com/mcp --name nuon-example\n")
	b.WriteString("amp mcp add nuon -- nuon agents mcp --allow-writes --url https://mcp.example.com/mcp --name nuon-example\n```\n\n")
	b.WriteString("A non-default `-C` config goes on the same command (`nuon -C /path/to/config agents mcp --allow-writes`). Cursor: put those args in mcp.json.\n\n")
	...
	b.WriteString("Tool JSON timestamps are UTC (Zulu) RFC3339 and always end in `Z`, for example `2026-09-04T04:23:00Z`. The `Z` means UTC, not the user's local clock.\n\n")
	b.WriteString("- If you name a calendar day, clock time, or age, convert the UTC instant to this machine's local timezone first. Example: `2026-09-04T04:23:00Z` is still the evening of September 3 in US Pacific.\n")
	b.WriteString("- Never say \"today\" or \"yesterday\" from the UTC date digits. The UTC calendar day can be a day ahead of local time.\n\n")
	...
	b.WriteString("| Apps | `list_apps`, `get_app`, `list_app_branches`, `get_app_branch`, `list_app_branch_runs`, `get_app_branch_run`, `list_app_branch_preview_sources` | `preview_app_branch` |\n")
	...
	b.WriteString("| Runbooks | `list_runbooks`, `get_runbook` | `run_runbook` |\n\n")
	...
	b.WriteString("- What changed in the latest run of the default branch for app <name>?\n")
	b.WriteString("- What changed in the preview run for PR <number> on app <name>?\n")
	b.WriteString("- Preview PR <number> for app <name> against install <name>, then follow that exact run (plan-only unless I ask to apply).\n")
	b.WriteString("- Describe install <name>, including what last updated it, its components, current health, and README.\n")
	b.WriteString("- Run runbook <name> on install <name>; ask me before triggering it (write).\n")
