...
func (c *cli) secretsCmd() *cobra.Command {
	...
	listCmd := &cobra.Command{
		...
		Run: c.wrapCmd(SecretsListEvent, func(cmd *cobra.Command, _ []string) error {
	...
	deleteCmd := &cobra.Command{
		...
		Run: c.wrapCmd(SecredsDeleteEvent, func(cmd *cobra.Command, _ []string) error {
			svc := secrets.New(c.apiClient, c.cfg)
	...
	createCmd := &cobra.Command{
		...
		PersistentPreRunE: c.persistentPreRunE,
		Run: c.wrapCmd(SecretsCreateEvent, func(cmd *cobra.Command, _ []string) error {
			svc := secrets.New(c.apiClient, c.cfg)
