...
// mcpClientJSON renders the client config block. Every client reads
// "mcpServers" except Amp, which uses the literal "amp.mcpServers" key.
...
func agentsSetupGuide(st *agentsStatus) string {
	...
	p(
		...
		`  OPERATION:") and needs a token with create permission. Leave it off for a`,
		"  read-only proxy.",
		"",
		"  Claude Code",
		"",
		...
		`    Amp uses the literal "amp.mcpServers" key, in`,
		"    ~/.config/amp/settings.json (you) or .amp/settings.json (a workspace):",
		"",
		mcpClientJSON("amp.mcpServers", "      "),
		"",
		"  Any other stdio MCP client",
		"",
		"    Have it run nuon agents mcp --allow-writes, or paste the mcpServers",
		"    block above into whichever config file it reads.",
		"",
		"3. Check it works",
		"",
		"    nuon agents context",
		...
		"  deployment where the two hostnames differ. --name renames the server in",
		"  your client's list. Both go on the command the client runs:",
		"",
	...
	p(
		...
		"",
		"Calling MCP over HTTP, without the proxy",
		"",
		"  Point the client at the MCP URL above and send both headers yourself:",
		"",
		"    Authorization: Bearer <api_token>",
		"      api_token in ~/.nuon, written by nuon auth login",
		"    X-Nuon-Org-ID: <org_id>",
		"      org_id in ~/.nuon, written by nuon orgs select",
		"",
		"  X-Nuon-Org-ID is required on multi-org accounts unless the agent calls",
		"  the select_org tool first. The stdio proxy sends both for you.",
		"",
...
MCP server with Claude Code, Cursor, or Amp, overriding the derived MCP URL,
and calling MCP over HTTP without the proxy.
