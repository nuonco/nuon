...
// build is happening _inside_ the vm. This will push it to our local registry running on port 5000 (ultimately, on the
// host)
//
// When we copy the image from the local registry to ECR, we can always access it as localhost:5000/runner.
//
// For running builds inside kaniko, we can simply use localhost:5000 for everything.

// Return a tag that can be used to build+push from the run-local environment
func GetLocalTag(version string) string {
	return fmt.Sprintf("host.containers.internal:5000/runner:%s", version)
}

// Return a tag that can be used inside kaniko
func GetKanikoTag(version string) string {
	return fmt.Sprintf("localhost:5000/runner:%s", version)
}
