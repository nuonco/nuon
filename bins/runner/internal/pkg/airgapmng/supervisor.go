...
func Run(ctx context.Context, options Options) error {
	...
	for _, key := range []string{"RUNNER_ID", "AWS_REGION", "AIRGAP_BUNDLE_URI"} {
		if env[key] == "" {
	...
	workdir := env["AIRGAP_WORKDIR"]
	if workdir == "" {
		workdir = "/tmp/airgap"
	}
	data := serviceData{EnvFile: options.EnvFile, ImageFile: options.ImageFile, RunnerID: env["RUNNER_ID"], AWSRegion: env["AWS_REGION"], BundleURI: env["AIRGAP_BUNDLE_URI"], StateURI: env["AIRGAP_STATE_URI"], StackOutputsURI: env["AIRGAP_STACK_OUTPUTS_URI"], InstallInputsURI: env["AIRGAP_INSTALL_INPUTS_URI"], Workdir: workdir, DeploymentID: env["AIRGAP_DEPLOYMENT_ID"]}
	if err := writeService(data); err != nil {
		return err
	}
	if registry := env["AIRGAP_ECR_REGISTRY"]; registry != "" {
...
func writeService(data serviceData) error {
	...
	tmpl, err := template.New("service").Parse(monitor.RunnerServiceAirgapAWS)
	if err != nil {
