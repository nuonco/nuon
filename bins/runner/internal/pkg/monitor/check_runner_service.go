...
func (h *Monitor) ensureRunnerTokenFile(ctx context.Context) error {
	...
	h.l.Debug(fmt.Sprintf("ensuring runner token file exists: %s", token.Filename))
	_, err := os.Stat(token.Filename)
	if err != nil {
...
func (h *Monitor) ensureRunnerTokenValid(ctx context.Context) error {
	...
	h.l.Warn("runner token is invalid - fetching new token")
	...
	result, err := fetchtoken.FetchAndStoreToken(ctx, unauthClient)
	if err != nil {
