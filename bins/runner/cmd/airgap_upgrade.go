...
func waitForInstallControls(ctx context.Context, store statestore.Store, deps *airgapDay2Deps, client *airgap.Client, logger *zap.Logger) {
...
func waitForCandidateApproval(ctx context.Context, syncer *airgapS3Sync, digest string, client *airgap.Client, logger *zap.Logger) {
	...
	for {
		...
		case <-ticker.C:
			...
			approvalKey := day2.CandidateApprovalKey(digest)
			if phase == "sandbox" {
				approvalKey = day2.CandidateSandboxApprovalKey(digest)
			}
			_, found, err := syncer.readControlObject(ctx, approvalKey)
			if err != nil || !found {
				continue
			}
