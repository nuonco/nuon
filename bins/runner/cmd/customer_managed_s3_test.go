...
func TestCollectUploadFiles(t *testing.T) {
	...
	require.NoError(t, os.WriteFile(filepath.Join(dir, "job-logs", "otel.jsonl"), []byte("{\"resourceLogs\":[]}\n"), 0o600))
	...
	require.ElementsMatch(t, []string{
		...
		"install/state/job-logs/otel.jsonl",
	}, keys)
}
...
func TestCollectUploadFilesSkipsReservedObjects(t *testing.T) {
	...
	require.NoError(t, os.MkdirAll(filepath.Join(dir, "operation"), 0o700))
	...
	require.NoError(t, os.WriteFile(filepath.Join(dir, "operation", "candidate.json"), []byte("{}"), 0o600))
	require.NoError(t, os.WriteFile(filepath.Join(dir, "operation", "staged-candidate.json"), []byte("{}"), 0o600))
	require.NoError(t, os.WriteFile(filepath.Join(dir, "operation", "stack-candidate.json"), []byte("{}"), 0o600))
...
func TestLegacyRunnerStatePath(t *testing.T) {
	for _, name := range []string{
		...
		"operations/catalog.json",
		"operations/bundle.json",
		"operations/bundles/sha256-one.json",
		"day2/catalog.json",
		"day2/bundle.json",
		"day2/bundles/sha256-legacy.json",
	} {
	...
	for _, name := range []string{
		...
		"operations/candidate.json",
		"operations/staged-candidate.json",
		"operations/stack-candidate.json",
		"operations/candidates/sha256-one/approval.json",
...
func TestUploadDirSyncsUnknownRunnerArtifact(t *testing.T) {
	...
	syncer := &customerManagedS3Sync{client: client, bucket: "state", prefix: "deployment"}
