...
func TestRunCustomerManagedHeartbeatOverwritesUntilCanceled(t *testing.T) {
	...
	go func() {
		defer close(done)
		runCustomerManagedHeartbeat(ctx, customermanaged.RunnerHeartbeat{
	...
	var first, second customermanaged.RunnerHeartbeat
...
func TestRunCustomerManagedHeartbeatRetriesWriteErrors(t *testing.T) {
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	done := make(chan struct{})
	var mu sync.Mutex
	...
	go func() {
		defer close(done)
		runCustomerManagedHeartbeat(ctx, customermanaged.RunnerHeartbeat{}, 5*time.Millisecond, func(context.Context, []byte) error {
			mu.Lock()
			defer mu.Unlock()
			attempts++
			if attempts == 1 {
