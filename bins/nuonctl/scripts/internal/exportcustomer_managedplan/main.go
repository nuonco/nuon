...
const source = "nuonctl export-customer-managed-plan"
...
func run(ctx context.Context, args []string) error {
	flags := flag.NewFlagSet("export-customer-managed-plan", flag.ContinueOnError)
	installID := flags.String("install-id", "", "install ID to export")
	out := flags.String("out", "./customer-managed-envelope.json", "output path")
...
WHERE id = $1 AND deleted_at = 0`, appConfigID).Scan(&raw)
	...
	if len(probe.Sandbox) == 0 || string(probe.Sandbox) == "null" {
		return nil, fmt.Errorf("app config %s has no sandbox config; customer-managed replay of sandbox jobs requires one", appConfigID)
	}
	return raw, nil
}
