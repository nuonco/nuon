...
var _ = Describe("CreateNamespace", func() {
	var (
		namespace string
		a         *server.Activities
		req       server.CreateNamespaceRequest
		e         *testsuite.TestActivityEnvironment
	)

	BeforeEach(func() {
		namespace = uuid.New().String()
		a = server.NewActivities()
		a.Kubeconfig = cfg
	...
	Context("When creating a namespace", func() {
		It("Should create namespace", func() {
			resp, err := e.ExecuteActivity(a.CreateNamespace, req)
			Expect(err).NotTo(HaveOccurred())
...
var _ = Describe("InstallWaypointServer", func() {
	var (
		namespace string = "default"
		a         *server.Activities
		req       server.InstallWaypointServerRequest
		e         *testsuite.TestActivityEnvironment
	)
	...
	Context("When installing waypoint", func() {
		It("Should install waypoint", func() {
			resp, err := e.ExecuteActivity(a.InstallWaypointServer, req)
			Expect(err).NotTo(HaveOccurred())
			// TODO(jdt): this assertion is pretty weak
			Expect(resp).NotTo(BeNil())
		})

		It("Should not error if waypoint is already installed", func() {
			resp, err := e.ExecuteActivity(a.InstallWaypointServer, req)
			Expect(err).NotTo(HaveOccurred())
			Expect(resp).NotTo(BeNil())
		...
		It("Should fail with invalid version", func() {
			...
			_, err := e.ExecuteActivity(a.InstallWaypointServer, req)
