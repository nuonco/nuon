// Code generated by go-swagger; DO NOT EDIT.

package models

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"context"
	"encoding/json"

	"github.com/go-openapi/errors"
	"github.com/go-openapi/strfmt"
	"github.com/go-openapi/validate"
)

// AppRoleType app role type
//
// swagger:model app.RoleType
type AppRoleType string

func NewAppRoleType(value AppRoleType) *AppRoleType {
	return &value
}

// Pointer returns a pointer to a freshly-allocated AppRoleType.
func (m AppRoleType) Pointer() *AppRoleType {
	return &m
}

const (

	// AppRoleTypeOrgAdmin captures enum value "org_admin"
	AppRoleTypeOrgAdmin AppRoleType = "org_admin"

	// AppRoleTypeOrgSupport captures enum value "org_support"
	AppRoleTypeOrgSupport AppRoleType = "org_support"

	// AppRoleTypeOrgReadOnly captures enum value "org_read_only"
	AppRoleTypeOrgReadOnly AppRoleType = "org_read_only"

	// AppRoleTypeOrgBuilder captures enum value "org_builder"
	AppRoleTypeOrgBuilder AppRoleType = "org_builder"

	// AppRoleTypeInstaller captures enum value "installer"
	AppRoleTypeInstaller AppRoleType = "installer"

	// AppRoleTypeRunner captures enum value "runner"
	AppRoleTypeRunner AppRoleType = "runner"

	// AppRoleTypeHostedDashInstaller captures enum value "hosted-installer"
	AppRoleTypeHostedDashInstaller AppRoleType = "hosted-installer"

	// AppRoleTypeCustom captures enum value "custom"
	AppRoleTypeCustom AppRoleType = "custom"
)

// for schema
var appRoleTypeEnum []any

func init() {
	var res []AppRoleType
	if err := json.Unmarshal([]byte(`["org_admin","org_support","org_read_only","org_builder","installer","runner","hosted-installer","custom"]`), &res); err != nil {
		panic(err)
	}
	for _, v := range res {
		appRoleTypeEnum = append(appRoleTypeEnum, v)
	}
}

func (m AppRoleType) validateAppRoleTypeEnum(path, location string, value AppRoleType) error {
	if err := validate.EnumCase(path, location, value, appRoleTypeEnum, true); err != nil {
		return err
	}
	return nil
}

// Validate validates this app role type
func (m AppRoleType) Validate(formats strfmt.Registry) error {
	var res []error

	// value enum
	if err := m.validateAppRoleTypeEnum("", "body", m); err != nil {
		return err
	}

	if len(res) > 0 {
		return errors.CompositeValidationError(res...)
	}
	return nil
}

// ContextValidate validates this app role type based on context it is used
func (m AppRoleType) ContextValidate(ctx context.Context, formats strfmt.Registry) error {
	return nil
}
