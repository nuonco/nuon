...
// WithRequestTimeout overrides the default per-request timeout enforced by
// the underlying http.Client. A caller-supplied context deadline still wins
// if shorter. Pass 0 to disable the SDK-side bound and rely solely on caller
// contexts — strongly discouraged for long-lived loops.
