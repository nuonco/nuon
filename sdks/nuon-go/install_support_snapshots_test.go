...
func TestInstallSupportSnapshotRequests(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case r.Method == http.MethodPost:
			...
			fmt.Fprint(w, `{"id":"iss-1","install_id":"vinst-1"}`)
		case r.URL.Path == "/v1/installs/vinst-1/support-snapshots/iss-1":
			fmt.Fprint(w, `{"id":"iss-1","install_id":"vinst-1"}`)
		default:
			fmt.Fprint(w, `[{"id":"iss-1","install_id":"vinst-1"}]`)
		}
	...
	require.NoError(t, err)
	require.Equal(t, "iss-1", created.ID)

	listed, err := client.ListInstallSupportSnapshots(context.Background(), installID)
	require.NoError(t, err)
	require.Len(t, listed, 1)

	got, err := client.GetInstallSupportSnapshot(context.Background(), installID, "iss-1")
	require.NoError(t, err)
	require.Equal(t, "iss-1", got.ID)
}
