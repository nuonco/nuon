...
These subcommands take the app **ID** (`app...`), not the app name. Get it from `nuon apps list`. Running
`nuon apps select` with no flags and picking from the list stores the ID, which lets you omit `--app-id` from `trigger`
and `runs`; `nuon apps select --app <name>` stores the name and leaves those commands failing.
