...
| `nuon agents mcp` | Read-only stdio MCP proxy. Its URL is derived from the configured API hostname (`api.<hostname>` becomes `mcp.<hostname>/mcp`). Add `--allow-writes` to expose mutating tools. `--url` / `--name` override the upstream server and the name in the client's MCP list. |
