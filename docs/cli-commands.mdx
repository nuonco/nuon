...
| `nuon apps validate` | Validate the config in the current directory. Requires login, and the app must already exist with a name matching the directory — run `nuon apps create` first. |
...
| Command | What it does |
|---------|--------------|
| `nuon sync --branch <name>` | Sync the current directory, then trigger a run on that [app branch](/guides/app-branches). Add `--preview` for a plan-only run, or use `--app-branch` to pick the branch interactively. `nuon apps sync` takes the same three flags. |
...
Operate Nuon from an LLM client. See [Agents](/guides/agents/overview).

| Command | What it does |
|---------|--------------|
| `nuon agents context` | Print auth, selected org/app/install, and the MCP HTTP URL. |
| `nuon agents mcp` | Stdio MCP proxy (`--allow-writes` in examples so mutating tools are listed). Its URL is derived from the configured API hostname (`api.<hostname>` becomes `mcp.<hostname>/mcp`). `--url` / `--name` override the upstream server and the name in the client's MCP list. Register with `claude mcp add --transport stdio nuon -- nuon agents mcp --allow-writes`, `amp mcp add nuon -- nuon agents mcp --allow-writes`, or Cursor mcp.json (see [Agents](/guides/agents/overview)). |
