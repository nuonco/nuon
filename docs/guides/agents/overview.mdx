...
`nuon agents mcp setup` writes that stdio config for Claude Code, Cursor, or Amp. The proxy connects to `https://mcp.nuon.co/mcp` and registers as `nuon`.
...
The proxy derives its upstream URL from `api_url` in your CLI config, which resolves to `https://mcp.nuon.co/mcp`. To point somewhere else, pass `--url`; to change the name in the client's MCP list, pass `--name`.
