...
A failure and its recovery, as they arrive in Slack. The failing resource and the reason travel with
the alert, so the first thing you read is already the diagnosis — and because the component's crossing
is what moved the install, both are reported in one message rather than two.
...
<Frame caption="90-day health for an install, with per-component uptime.">
  <img src="/images/guides/component-health/90-day-health.png" alt="A 90-day health card showing 97.57% uptime, a row of daily status bars, and a list of components each with a Healthy badge and its own uptime percentage." />
</Frame>
