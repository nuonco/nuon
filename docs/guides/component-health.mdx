...
<Steps>
  <Step title="Deploy, or refresh cluster access">
    Health reads your customers' clusters using the same access your deploys already use. There's
    nothing to install and no agent runs in your customers' clusters. An install that hasn't
    deployed recently needs one deploy — or click **Refresh cluster access** on the install's health
    card to pick it up immediately.
  </Step>
