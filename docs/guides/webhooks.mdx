...
`unknown` — the runner is offline or its observations are stale — never notifies; runner inactivity has its own
event. A worsening inside the bad band (`degraded` to `unhealthy`) does not re-notify. Like `drift_detected`,
the flag is gated **independently of `outcome`**.
