...
| `stack_runs`         | `bool`    | `false` | Deliver SDK provisioner stack-run lifecycle events (`started` / `succeeded` / `failed`). Independent of `outcome` and `ops`. Only meaningful for `installs`. |
...
#### Install stack runs (SDK provisioner)

To receive notifications when the AWS-native SDK provisioner begins, succeeds, or fails a stack
run on an install, set `stack_runs: true` on `installs`. Each transition is its own single-shot
event delivered as a `com.nuon.install_stack_run.lifecycle.v1` CloudEvent. The `kind` field on the
event identifies whether the run was a `provision`, `reprovision`, or `deprovision`. Gated
**independently of `outcome`** so a `stack_runs`-only subscription can mute install workflow
lifecycle events while still receiving SDK provisioner notifications.
