...
<Note>
In order for components to work with both `aws-ecs` and `aws-ecs-byovpc` sandboxes, subnets must be tagged consistently.

We recommend each VPC used with `byo-vpc` adds a tag of `visibility:private` to all private subnets, and
`visibility:public` to all public subnets to match the tagging in `aws-ecs`.
</Note>
