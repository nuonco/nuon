...
icon: "code-branch"
keywords: ["configure app branches", "branch.toml", "install_groups", "deployment groups", "label installs", "nuon installs labels set", "nuon apps branches trigger", "nuon sync --branch", "PR preview setup", "re-apply config version", "rollback install config", "app-config-updates", "staged rollout setup"]
---
...
<Tip>
  ...
  the rest of your config, and apply them with `nuon installs sync -d installs/`. To start from an install that
  already exists, export it: `nuon installs generate-config -i <install-id> > installs/<name>.toml`.
</Tip>
