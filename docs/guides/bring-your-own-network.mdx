...
- [App inputs](/guides/app-inputs) to enable the customer to provide information about the network during the installation process.
- A [Sandbox](/guides/sandboxes) that accepts the inputs as [Terraform variables](https://developer.hashicorp.com/terraform/language/values/variables), and uses them to create [Terraform data sources](https://developer.hashicorp.com/terraform/language/data-sources).
...
resource "nuon_app_sandbox" "main" {
  ...
  # Delete these to use tagged subnets instead.

  var {
    name = "private_subnet_ids"
    value = "{{.nuon.install.inputs.private_subnet_ids}}"
  }

  var {
    name = "public_subnet_ids"
    value = "{{.nuon.install.inputs.public_subnet_ids}}"
  }
}
```
