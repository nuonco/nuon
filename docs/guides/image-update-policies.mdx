...
A [container action](/guides/container-actions) that points its `image` at `img_whoami`'s `image.ref` output picks the new build up the same way, without waiting for a deploy: its next run syncs the newer build into the install registry before the action's steps start.
