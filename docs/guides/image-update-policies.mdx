...
In your helm component, reference the image outputs by digest. Using the digest-pinned `ref` output means each install is locked to the exact bytes that were resolved at build time:
...
image_repository  = "{{ .nuon.components.img_whoami.outputs.image.ref }}"
