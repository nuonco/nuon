...
Label values can use [Nuon templating](/guides/using-variables) to render from install state:
...
If your Nuon BYOC deployment has a telemetry relay configured, set `[telemetry]` in the install config to enable
or disable forwarding of application logs, metrics, and traces. See
[Manage Install Telemetry](/guides/manage-install-telemetry) for the configuration and org-default behavior.
