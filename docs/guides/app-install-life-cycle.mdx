...
defined life cycle that starts with
...
controls, and monitoring and remediating the installs.
...
<Note>
We have several example app configurations in [this public GitHub repository](https://github.com/nuonco/example-app-configs) to see how apps are packaged.
</Note>
...
module source or a Helm chart, you must manually re-build the component in the Dashboard UI.

If you include Helm or Terraform source code inside of your app repository and make a change to that source, you must first push your branch to your remote repo and then manually re-build the component in the Dashboard UI.

<Note>
A common overlook is to have an incorrect branch name in the component `.toml` file if the src is included in the app repo.
</Note>

Again, if anything referenced in the `.toml` files change, you must run `nuon apps sync` with the Nuon CLI.
