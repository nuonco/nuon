...
icon: "aws"
boost: 10
keywords: ["cloudformation", "install-stacks", "install stack", "stack", "aws", "create-stack", "template", "quick create", "quick-create link", "one-click install", "AWS console", "aws cli"]
---
...
<Note>CloudFormation is AWS-only. For what customers do on Azure and Google Cloud, see [What your customer does on each cloud](/concepts/stacks#what-your-customer-does-on-each-cloud).</Note>
...
<Steps>
  ...
  <Step title="Open the quick-create link">
    ...
    If the install pins a region, the link opens in that region. Otherwise it opens in whichever region the console
    currently has selected, so switch to the intended region before continuing.
  </Step>
