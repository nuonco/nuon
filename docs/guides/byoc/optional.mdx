...
credentials once. Then enable forwarding for individual customer installs and send application logs, metrics, and
traces to their private OTLP endpoints. The relay attaches verified install identity before forwarding the data.

This is separate from Runner audit-log export and does not use the `telemetry-export-config` secret.
See [Collect Install Telemetry](/guides/byoc/telemetry) for setup, per-install enablement, and verification.
