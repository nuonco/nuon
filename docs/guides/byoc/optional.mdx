...
Update the secret to export Runner audit logs to your own OTLP-compatible backend.
...
With Nuon's assistance, deploy a telemetry relay in your BYOC control plane and configure your observability backend
credentials once. Customer installs can then send application logs, metrics, and traces through the relay to your
backend. This is separate from Runner audit-log export and does not use the `telemetry-export-config` secret.

Follow [Set Up Install Telemetry](/guides/byoc/telemetry) to get started. Once it is set up, use
[Manage Install Telemetry](/guides/manage-install-telemetry) to choose your org default and enable or disable installs.
