...
| slack     | The Slack integration listener (OAuth callback, slash commands, events, interactions). Only provisioned if the Slack integration is enabled. | true |
...
Create a Slack app so Nuon can post deploy, action, workflow, and approval events into Slack channels, and accept approval responses from Slack. Skip this section if you don't plan to use the Slack integration — the feature is gated by the `slack` org feature flag and can be enabled later.

1. Go to [Slack API: Your Apps](https://api.slack.com/apps) and click **Create New App → From scratch**.

2. Name the app (e.g., "Nuon BYOC") and select the workspace you want to develop it in. Apps can be distributed to other workspaces later.

3. Under **OAuth & Permissions**, add the following Bot Token scopes:

| Scope             | Why                                              |
| ----------------- | ------------------------------------------------ |
| `commands`        | Register the `/nuon` slash command.              |
| `chat:write`      | Post messages into subscribed channels.          |
| `chat:write.public` | Post into channels the bot isn't a member of.  |
| `channels:read`   | List public channels in the subscribe dialog.    |
| `groups:read`     | List private channels the bot is invited to.     |
| `users:read`      | Resolve approver identities for audit trail.     |
| `users:read.email` | Match Slack users to Nuon accounts by email.    |
...
| ---------------- | -------------------------------------------------- |
| Command          | `/nuon`                                            |
| Request URL      | `https://slack.<your-root-domain>/slack/commands/nuon` |
| Short Description | `Subscribe this channel to Nuon events`           |
| Usage Hint       | `subscribe \| unsubscribe \| status \| help`       |

6. Under **Interactivity & Shortcuts**, enable interactivity and set:

   ```
   Request URL: https://slack.<your-root-domain>/slack/interactions
   ```

7. Under **Event Subscriptions**, enable events and set:

   ```
   Request URL: https://slack.<your-root-domain>/slack/events
   ```

   Subscribe to bot events: `app_uninstalled`, `tokens_revoked`.

8. Under **Basic Information**, note your **Client ID**, **Client Secret**, and **Signing Secret** — you'll share these with Nuon as inputs and secrets.
