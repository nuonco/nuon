...
If you do not want your users to have to set up DNS when installing your app, you can optionally configure your apps to use DNS delegation. A unique subdomain will be created for each install under a shared subdomain. In Nuon Cloud, for example, we use `nuon.run`. Installs created in Cloud will be given a subdomain based on their install ID, such as `inl160z2xmng8w1jnq0xxhelln.nuon.run`.
