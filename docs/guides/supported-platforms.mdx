...
Each Nuon app allows you to deploy to a single platform, by combining a [Sandbox](/guides/sandboxes) with your application and
...
You can create apps that provision a completely new network (VPC) and k8s cluster, or use an existing customer managed network or cluster (using [app-inputs](./app-inputs)).
...
Read more about this in [sandboxes](./sandboxes#aws-eks).
</Note>

## AWS ECS

Nuon supports running your app via [AWS ECS](https://aws.amazon.com/ecs/).
You can create apps that provision a completely new network (VPC) and ECS cluster, or use an existing customer managed network or cluster (using [app-inputs](./app-inputs)).
...
Read more about this in [sandboxes](./sandboxes#aws-ecs).
</Note>
...
If you would like to support third party cloud providers that Nuon does not natively support, you can use [app inputs](./app-inputs) and [terraform components](./terraform-components.mdx).
