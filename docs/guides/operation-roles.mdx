...
|----------|--------|-------|
| 1 (highest) | Runtime override (`--role-name` flag / dashboard selection) | Any operation |
...
[[roles.policies]]
name     = "{{.nuon.install.id}}-deploy-policy"
contents = """
{
  "Version": "2012-10-17",
  "Statement": [
...
<Note>
  Defining a custom role grants **AWS** permissions only. If the role operates inside an EKS cluster (Helm
  or Kubernetes manifest deploys), it also needs an EKS access entry mapping its ARN to Kubernetes RBAC —
  see [Operation Roles on Kubernetes Clusters](/concepts/operation-roles#operation-roles-on-kubernetes-clusters).
</Note>
...
Pass `--role-name <role-name>` when triggering an action, or a component deploy or teardown:

```sh
# Override role for a manual action run
nuon actions create-run -w <action-workflow-id> -i <install-id> --role-name my-custom-role
```
