...
Operation-role permissions, break-glass roles, and custom roles are read entirely from the control plane.
They will be automatically pulled in by the next `terraform plan`.
Your customer can enable or disable new roles before applying, but this is optional.
