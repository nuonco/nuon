...
Reprovisioning re-runs provisioning for an install against its current app config. Used when customer stack facing app config changes needs to be
...
<Steps>
  ...
  <Step title="Approve gated steps">
    ```sh
    nuon installs workflows steps approve -i $INSTALL_ID --workflow-id $WORKFLOW_ID --step-id $STEP_ID
    ```
  </Step>
</Steps>
...
deployed are untouched by the stack run.
