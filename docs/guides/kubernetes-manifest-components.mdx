...
data:
  ...
  image_repository: "{{.nuon.components.image.outputs.image.repository}}"
  image_tag: "{{.nuon.components.image.outputs.image.tag}}"
...
spec:
  ...
  template:
    ...
    spec:
      containers:
      - name: app
        image: "{{.nuon.components.app_image.outputs.image.repository}}:{{.nuon.components.app_image.outputs.image.tag}}"
        env:
        - name: DATABASE_URL
...
### Template variables in Kustomize trees
...
<Note>
Kustomize-backed manifests are templated on the runner, not in the planner. This keeps the Temporal workflow payload small even for large Kustomize trees. Inline manifests, by contrast, are still rendered in the planner.
</Note>
