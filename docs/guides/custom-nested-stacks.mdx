...
description: 'Extend the BYOC install stack with custom CloudFormation nested templates to provision AWS or Kubernetes resources before the sandbox runs.'
icon: "layer-group"
...
<Note>
For `aws-cloudformation` stacks, both `vpc_nested_template_url` and `runner_nested_template_url` are
required. The CLI rejects a `stack.toml` that omits either field with an error such as
`vpc_nested_template_url is required when type is aws-cloudformation`. Define them at the top of
`stack.toml` before any `[[custom_nested_stacks]]` blocks.
</Note>
