...
install stack. The built-in collection supports `bucket`, `cloudsql`, `kms`, `service_account`, and `dns`.
...
the install ID and stack `name`. The `cloudsql` module requires `db_password`; it does not include the password in its
outputs.
...
curated module defines its own accepted parameters and returns a single `outputs` object. The built-in modules cover
storage, databases, encryption keys, service accounts, and DNS zones.
...
- A GCP `cloudsql` module without `parameters.db_password`
