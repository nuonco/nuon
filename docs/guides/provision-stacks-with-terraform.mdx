...
icon: "terminal"
boost: 10
keywords: ["terraform", "install-stacks", "install stack", "stack", "terraform apply", "inputs.auto.tfvars", "secrets.auto.tfvars", "aws", "gcp", "google cloud", "backend", "remote state", "Infrastructure Manager"]
---
...
This flow is available on AWS and Google Cloud. On Google Cloud it is the default option, since no console link is generated there. Your customer can run the apply from their workstation or from any Terraform runner, such as Spacelift or Google Cloud's Infrastructure Manager.
