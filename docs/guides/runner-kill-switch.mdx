...
A core promise of BYOC is that the customer always has ultimate control over what runs in their cloud. Because the Nuon [Runner](/concepts/runners) is an EC2 instance inside an Auto Scaling Group (ASG) in the **customer's** AWS account, the customer can shut it down at any time — and Nuon cannot override that decision.

This is especially important for customers in regulated industries (healthcare, defense, financial services) who need assurance that a vendor cannot access their infrastructure without explicit permission.

## How It Works

The Runner's ASG is created by the customer's [Stack](/concepts/stacks) during install provisioning. The customer has full ownership of this ASG in their AWS account. By changing the ASG capacity settings, the customer controls whether the Runner is running.
...
To resume normal operation, edit the ASG and set the capacity back to **1/1/1** (Desired: 1, Min: 1, Max: 1). The Runner will start automatically and reconnect to the Nuon control plane.
...
For maximum control, customers can layer additional AWS-native restrictions:

- **IAM Deny Policies**: Attach a deny policy to the Runner's IAM role to block specific actions even if the Runner is running.
- **Service Control Policies (SCPs)**: Apply organization-wide policies that prevent the Runner from performing certain operations across all accounts in an AWS Organization.

These can be combined with the ASG kill switch for defense-in-depth.
