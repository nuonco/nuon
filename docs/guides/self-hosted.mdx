...
icon: 'server'
keywords: ['self-hosted Nuon', 'customer-managed Nuon', 'fully self-hosted', 'no remote management', 'compliance Nuon', 'sovereign Nuon']
---
...
This is the same software as [Nuon BYOC](/guides/byoc), but without the remote operator. There is no Nuon Runner in your account polling Nuon Cloud for jobs, no managed upgrades, and no remote troubleshooting. In exchange, the installation has no mechanism for Nuon to access — useful for compliance, sovereignty, or disconnected-environment requirements where any remote-managed control plane (even a single-tenant one) is ruled out.
