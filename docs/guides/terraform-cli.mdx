...
Create a Nuon API token for your current org

```bash
nuon orgs api-tokens create --name terraform-cli --output json | jq -r .api_token
```

Replace the three token `undefined` blocks in `nuon_backend.tf` with the token value. The token is only shown once, so copy it before continuing.
