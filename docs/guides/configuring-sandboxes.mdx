...
- `nuon_dns.public_domain.name` - The public root domain
- `nuon_dns.internal_domain.name` - The internal root domain
- `namespaces` - Namespaces created in the cluster

Each `nuon_dns` domain is an object rather than a string: `name` holds the hostname, alongside `zone_id` and `nameservers`. Reference `.name` when you want the domain itself.
