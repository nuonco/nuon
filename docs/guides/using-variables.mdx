...
icon: "sliders"
keywords: ["inputs", "variables", "variables guide", "Go template syntax", "Nuon templating", "reference outputs", "nuon variables", "template expressions", "share values between components"]
---
...
Templating also works in [install labels](/guides/install-configs#dynamic-labels) and app-level [default labels](/guides/managing-apps#default-labels).
...
- `{{ .nuon.components.<component_name_slug>.outputs.image.repository }}`
  - Repository containing the image synced into the install, without a tag or digest
- `{{ .nuon.components.<component_name_slug>.outputs.image.tag }}`
  - Resolved image tag
- `{{ .nuon.components.<component_name_slug>.outputs.image.ref }}`
  - Digest-pinned image reference
- `{{ .nuon.components.<component_name_slug>.outputs.image.display_tag }}`
  - Human-readable resolved tag for display purposes
