...
description: "Learn how to grant permissions to create an install, by sandbox."
icon: code
---

In order to deploy your application into customer cloud accounts, or test accounts - cross account access must be
granted to Nuon for the initial install.

Permissions granting Nuon access to a customer account are _only_ required when first setting up the install. Once the
install is initially provisioned, the [runner](../configure/runners.mdx) is responsible for all additional management.

## AWS Install Access

For applications that use an AWS sandbox, a [cross account IAM
  role](https://docs.aws.amazon.com/IAM/latest/UserGuide/tutorial_cross-account-with-roles.html) delegating access to
  Nuon must be created.
...
You can use `terraform` to create and manage IAM roles for creating / destroying Nuon installs.
...
locals {
  ...
  ecs_artifact_base_url = "https://raw.githubusercontent.com/nuonco/sandboxes/${local.branch}/${local.sandbox}/artifacts"
}
...
<Note>
[mailto:team@nuon.co](Please get in touch) if you would like to learn more about running Nuon in your own account.
</Note>
