---
title: 'Health Checks'
description: "Health checks tell you whether every component on every install is running right now."
icon: "heart-pulse"
keywords: ["health checks", "health check", "healthcheck", "healthchecks", "health-check", "health checking", "component health", "component health check", "kubernetes health check", "helm health check", "terraform health check", "install health", "liveness", "readiness", "probe", "probes", "http probe", "tcp probe", "exec probe", "custom health check", "required checks", "verified deploys", "deploy gate", "block_deploy", "stabilization window", "uptime", "uptime monitoring", "canary", "bake period", "degraded", "unhealthy", "monitoring", "alerting", "resource explorer"]
---
...
<Steps>
  <Step title="Deploy, or refresh cluster access">
    ...
    your customers' clusters. Health reads your customers' clusters using the same access your deploys
    already use. An install that hasn't deployed recently needs one deploy — or click **Refresh cluster
...
custom check](#choosing-a-check) to give a Terraform component a real verdict.
...
flowchart LR
    ...
    custom["Custom checks<br/>your systems"] --> v
...
| **Custom check** | Only *your* systems know — CI, a business metric, a nightly audit | Your systems, on your clock | Yes, with `required_checks` |
...
<Warning>
**Probes run from the runner, not from inside your cluster.** A probe target has to be resolvable
...
connect, and the probe records that failure as a real `unhealthy` observation. That is why the
Kubernetes example above probes the install's public hostname instead of the in-cluster service.

If a component has no target the runner can reach, give it no probe at all — set
...
<Frame caption="A component degrading and recovering, delivered to Slack.">
  <img src="/images/guides/health-checks/slack-alert.png" alt="Two Slack messages from Nuon Cloud. The first is titled Component degraded, install degraded, ctl_api, and shows the component, its previous healthy verdict, the failing Deployment and a readiness probe connection-refused error. The second is titled Component recovered, install recovered, and shows all 52 resources healthy." />
</Frame>
...
<Frame caption="90-day health for an install, with per-component uptime.">
  <img src="/images/guides/health-checks/90-day-health.png" alt="A 90-day health card showing 97.57% uptime, a row of daily status bars, and a list of components each with a Healthy badge and its own uptime percentage. One component, nginx_gateway, reads 97.57% while the rest read 100%." />
</Frame>
...
<AccordionGroup>
  <Accordion title="Everything shows unknown or no resources at all">
    Nuon can't reach the cluster. This is almost always an install that hasn't deployed recently.
