...
<Warning>
**The Helm release is named after `chart_name`, not after the component.** Two components that share a `chart_name` and a `namespace` therefore resolve to the same Helm release, and whichever deploys last overwrites the other's resources. Give each component its own `chart_name` (or its own `namespace`) when you deploy the same chart more than once. A `chart_name` must still be a valid DNS label of 5-62 characters, so short names like `api` are rejected at sync time.
</Warning>
...
Examples for accessing common template variables:
...
To customize a Helm chart's values for a single install without changing your app config, use
