...
reference](https://registry.terraform.io/providers/nuonco/nuon/latest/docs/resources/helm_chart_component) for
full configuration
options.
...
The `aws-eks` managed sandbox ships with standard Helm components + DNS zones to solve for common use cases, such
...
The sandboxes are [open source](https://github.com/nuonco/sandboxes) and can be [customized](/concepts/sandboxes#create-a-custom-sandbox), if these components do not work for your application.
