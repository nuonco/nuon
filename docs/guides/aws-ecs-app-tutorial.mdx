...
Here's an overview of the application you will deploy to your AWS account during this tutorial.

<Diagram title="ECS Service Architecture Diagram" src="/images/guides/aws-ecs-app-tutorial.jpg" />

## Setup

### Create an Account

Install the [Nuon CLI](/cli), and sign up for an account.
...
Since this guide uses the Nuon Terraform provider, [install Terraform](https://developer.hashicorp.com/terraform/install).

### Set Up an AWS Account

[Set up an AWS account](https://docs.aws.amazon.com/SetUp/latest/UserGuide/setup-overview.html). This is the account you will create an install in.
...
Once the config is synced, you should be able to select it using the CLI.
...
This app consists of two components: one to build the Docker image, and another to provision the ECS service. The source code for both is available in [our guides repo](https://github.com/nuonco/guides/tree/main/create-an-aws-ecs-byoc-app) if you would like more details.
...
Initial buids for each components will also have been created. Verify with the CLI that they were successful.
...
When an install is provisioned, it will check to see if there are any builds available. If there are, it will automatically create deploys to update itself.

## Create an Install

Creating an install requires two steps, granting access to the AWS acount via an IAM role, and then provisioning the sandbox to that account.
...
You will know the install is provisioned when it's status is "active".

### Monitor Deploys

When the install is ready, it will check to see if there any builds available for the app components. If there are, it will create deploys to update itself. You can monitor these with the CLI as well.
...
Then create a release to update the install. You do not need a new build, since you did not change the componet source code, just the config.
...
- Dig into our [App Configuration](/configure/apps) guide to learn how to configure more complex apps.
