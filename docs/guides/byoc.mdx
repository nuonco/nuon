...
BYOC Nuon deployments typically require two domains. The first is the `root_domain` under which all of the services
(e.g. APIs & Frontend) are served. The second, `nuon_dns_root_domain`, is optional but it is used to automate the
provisioning of Route53 zones for installs during creation.

|                 | Description                                                                          |
| --------------- | ------------------------------------------------------------------------------------ |
| Root Domain     | The root domain from which the nuon services are served.                             |
| Nuon DNS Domain | The domain used to provision domains for installs managed by this BYOC Nuon Install. |

We recommend hosting BYOC Nuon under a custom domain of your choice, for example:

- `byoc.organization.com`

Nuon DNS should be hosted under a separate domain or a dedicated subdomain, such as:

- `installs.my-domain.com`
- `hosted.my-domain.io`

<Warning>
  We strongly encourage you to choose `root_domain` and `nuon_dns_root_domain` values that do not overlap.
</Warning>

#### AWS Route53 Setup Instructions

If you wanted to host Nuon BYOC at `nuon.my-domain.com`, complete the following steps.
...
<Warning>
  The Nuon Dashboard uses cookies for authentication, and they will be shared on all subdomains of the provided root
  domain. We strongly recommend creating a Nuon-specific subdomain to avoid leaking auth cookies.
</Warning>
...
<Warning>
  Nuon BYOC previously required Auth0 for authentication, but this dependency has been removed. This documentation is
  retained for anyone still using Auth0, but new Nuon BYOC installs should use the new IdP integration documented above.
</Warning>

To use Auth0 for authentication, you will need to configure an API, applications, and a custom action in your Auth0
tenant.

Nuon provides a Terraform module to automate Auth0 configuration. We recommend this over manual configuration. Apply the
following Terraform to use it.
...
<Note>The Identifier must match your API URL exactly. It cannot be changed after creation.</Note>
