...
| `type` | string | Yes | `deploy`, `component_tear_down`, `action`, `sandbox_reprovision`, or `sandbox_deprovision`. |
| `component_name` | string | deploy / tear-down steps | Component to deploy or tear down. Required when `type = "deploy"` or `"component_tear_down"`. |
| `deploy_dependents` | bool | No | When true, also deploys the component's transitive dependents (downstream subgraph) in dependency order. Only applies to `deploy` steps. |
