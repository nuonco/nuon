...
* **`component_deploy`** — Deploy a component. Set `deploy_dependents` to also roll out the component's downstream dependents, in dependency order, or `plan_only` to check for drift without applying the plan. (`deploy` is accepted as a legacy alias.)
...
* **`sandbox_reprovision`** — Reprovision the install's sandbox infrastructure. Set `skip_component_deploys` to reprovision the infrastructure only, without redeploying components on top, or `plan_only` to check for drift without applying or redeploying components.
...
| `plan_only` | bool | No | When true, generates the plan without applying it. Applies to `component_deploy` and `sandbox_reprovision`; plan-only sandbox steps also skip component redeployment. |
