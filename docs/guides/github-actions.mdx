...
<Note>
  `org_admin` is the simplest role to start with, but it grants far more than a CI job needs. To scope the trust
  policy to just this app, create a [custom role](/guides/custom-roles) and pass its role id to `--role` instead.
</Note>
...
As with OIDC, `--role` also accepts a [custom role](/guides/custom-roles) id if you want the token confined to one
app rather than the whole org.
