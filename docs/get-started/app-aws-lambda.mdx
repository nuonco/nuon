...
description: "Package and deploy a Lambda app with DynamoDB and API Gateway into your own AWS account."
---

**What you'll build:**

- An [App](/concepts/apps)
- Several [Components](/concepts/components): a Lambda function, Docker image, DynamoDB table, certificate, and API Gateway
- An [Install](/guides/app-install-life-cycle) running on the [AWS minimal sandbox](/concepts/sandboxes#nuon-managed-sandboxes)

**What you'll need:**

- A Nuon org and the Nuon CLI authenticated ([Quickstart](/get-started/quickstart))
- An AWS account ([set one up here](https://docs.aws.amazon.com/SetUp/latest/UserGuide/setup-overview.html)) — this is where the app will run

<Note>
	The full app config is in [`example-app-configs`](https://github.com/nuonco/example-app-configs). You can clone it instead of typing each TOML by hand. This page explains what each config does, so you can author your own apps next.
</Note>
...
**Next:** [How it Works — Overview →](/concepts/overview)
