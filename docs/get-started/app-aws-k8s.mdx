...
<Note>
	This [App Config](/concepts/apps) can be found in a [GitHub
	repository](https://github.com/nuonco/example-app-configs).
</Note>

For a more complex app config example, read the blog post on
[Deploying Coder with Nuon](https://nuon.co/blog/installing-coder-with-nuon/) or
review the app config in the
...
<Note>
	...
	`eks-simple` App Config versus creating each config file manually. This guide
	is meant to explain the concepts behind each config file, so you can create
	your own App Configs in the future.
</Note>
...
Look at the `eks-simple` App Config in the
[`example-app-configs` repository](https://github.com/nuonco/example-app-configs)
