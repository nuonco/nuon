...
<Note>
This [App Config](/concepts/apps) can be found in a [GitHub repository](https://github.com/nuonco/example-app-configs).
</Note>

For a more complex app config example, read the blog post on [Deploying Coder with Nuon](https://nuon.co/blog/installing-coder-with-nuon/) or review the app config in the [example-app-configs repository](https://github.com/nuonco/example-app-configs).
...
- An [Install](/concepts/installs), using our [AWS EKS sandbox](/concepts/sandboxes#nuon-managed-sandboxes)

<Note>
You can alternatively clone the [`example-app-configs` repository](https://github.com/nuonco/example-app-configs) which includes the `eks-simple` App Config versus creating each config file manually. This guide is meant to explain the concepts behind each config file, so you can create your own App Configs in the future.
</Note>

## Configure App

To configure the app, you will create several TOML config files.
...
Nuon provides a set of [Nuon Managed Sandboxes](/concepts/sandboxes#nuon-managed-sandboxes) that can be used to provision the infrastructure needed for your app.
The `aws-eks-sandbox` will provide everything you need to run an EKS, from the EKS cluster down to the VPC.
...
In this example, we will create a Component to install Traefik's [Whoami](https://github.com/traefik/whoami) service, a Tiny Go webserver that prints OS information and HTTP request headers.
...
[Actions](/concepts/actions) are used to perform operations on your App, such as healthchecks or running `kubectl` commands.

In the App root directory, create directory called `actions` and within it, two actions. Create a file named `healthcheck.toml` and add the following to check the ALB status and the resource types in the whoami cluster namespace.
...
Create another file named `simple_action.toml` and add the following to create a random secret in the `whoami` namespace:
...
Look at the `eks-simple` App Config in the [`example-app-configs` repository](https://github.com/nuonco/example-app-configs) for the remaining and required directory structure and files. The files created above are marked in **.
...
- Try the Coder cloud development environment app config in the [example-app-configs repository](https://github.com/nuonco/example-app-configs) to see how to deploy a more complex app. Read the blog post on [Deploying Coder with Nuon](https://nuon.co/blog/installing-coder-with-nuon/) for more details.
