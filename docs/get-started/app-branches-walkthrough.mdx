...
- `sync install configs` is skippable, and a no-op for this branch.
...
from `whoami` to `hello`. Then approve it.
...
The pause before each group is what makes the rollout reviewable. You decide group by group, with the diff in front
of you, instead of committing the whole fleet at once.
