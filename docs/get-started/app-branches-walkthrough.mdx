...
<Warning>
	...
	than an error. Copy the ID for `eks-simple` out of `nuon apps list`.

	Run `nuon apps select` with **no flags** and choose from the list: that stores the app's ID, which is what lets the
	rest of this walkthrough drop `--app-id` from `trigger` and `runs`. Do not use `nuon apps select --app eks-simple` —
	it succeeds, but stores the name, and the branch commands then fail as though the branch did not exist.
</Warning>
