...
- `building components and sandbox` rebuilds only what changed. The sandbox half of it needs
  [sandbox builds](/guides/sandbox-builds) enabled for your org; without it, components only.
...
<Steps>
  ...
  <Step title="Open a pull request">
    ...
    headed `## Nuon Preview — main` (the branch's `name`, not the app's) with a table of what changed, plus a commit
    status named `nuon/<org>/<app>/main preview (plan-only)` linked to the preview run. Push another commit and the same comment is
    updated in place.
  </Step>
</Steps>
