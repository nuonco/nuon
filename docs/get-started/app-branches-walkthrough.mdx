...
Either route sets the same label — but install config files are **not** applied by `nuon apps sync` (it prints a
deprecation notice and skips them). They are applied with their own command:
...
install group plan** action. Read the diff. For the change above, installs that inherit the default show `sub_domain`
moving from `whoami` to `hello`; installs that pin `sub_domain` explicitly show no change for it (see the note in
Preview a Change). Then approve it.
