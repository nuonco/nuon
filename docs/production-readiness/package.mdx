...
You package an App for Nuon by creating an [App Config](/concepts/apps).
...
[Inputs](/concepts/app-inputs) are how your [App](/concepts/apps) will be configured during deployment.
...
If your customer needs to configure passwords, private tokens, or other sensitive values, define an input as a [Secret](/concepts/app-secrets). Secret values are saved to the target platform's secret store in the customer's account, and are never stored or accessed by the Nuon control plane.

### App Configuration

While you can use [Inputs](/concepts/app-inputs) for your App's own internal configuration and preferences -- such as setting an admin password or connecting to a customer database -- we recommend against this if possible.
Allowing your customer to update settings like this within your app will likely provide a better experience.

## Sandbox

The [Sandbox](/concepts/sandboxes) is where you define the environment your App will run in, and therefore acts as part of the contract you are making with your customer regarding access and security.
...
See the [Sandboxes](/concepts/sandboxes#nuon-managed-sandboxes) page to learn how to use these.
...
[Components](/concepts/components) are how you model your app's architecture in Nuon.
...
In the [App Config](/concepts/apps) you can use Outputs from one Component as Inputs to another to integrate them.
...
You will typically need to manage DNS with your components to host your App at a private or public URL.
...
Beyond just defining a [Component](/concepts/components) for resources like these, you may want to use [Actions](/concepts/actions) to implement things like schema migrations and configuration updates.
