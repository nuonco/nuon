...
This kicks off a provision workflow. Both the CloudFormation stack and the Terraform tfvars are templated with the install ID, region, and other settings. The provision workflow's "await install stack" step exposes both via tabs ("CloudFormation" and "Terraform"), which can be shared with your customer.
