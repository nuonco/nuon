...
branch    = "main"
[[components.vars]]
function_name = "widgets"
image_uri = "{{.nuon.components.docker_image.image.repository.uri}}:{{.nuon.components.docker_image.image.tag}}"
```
...
lambda_function_arn = "{{.nuon.components.lambda_function.outputs.lambda_function_arn}}"
zone_id = "{{.nuon.install.sandbox.outputs.public_domain.zone_id}}"
```
