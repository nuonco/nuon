...
- Four [Terraform components](/guides/terraform-components)
- An [Install](/guides/create-installs), using our [AWS ECS sandbox](/guides/sandboxes#aws-ecs)
...
This is a [Docker Build component](/guides/docker-build-components) that will build the widgets service and create a Docker image containing it. When released, it will sync the image to each install's ECR so Lambda can pull it when executing.
