...
You will deploy an API service to ECS Fargate, and expose it to the internet with an ALB. The API will present info about the install it is running in, allowing you to inspect it and verify that everything is working.
...
Install the CLI, sign up for an account, and create an org. If you haven't done this yet, refer to our [Quickstart](/get-started/quickstart).
...
Use our turnkey ECS sandbox. It will provide provide everything you need to run ECS services, from the ECS Fargate cluster down to the VPC.
...
Initial buids for each component will also have been created. Verify with the CLI that they were successful.
