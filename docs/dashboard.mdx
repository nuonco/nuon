...
Org admins can create, list, and delete tokens here. Each token is durable (valid one year by
default), scoped to the current org, and shown only once at creation — copy it somewhere safe before
closing the dialog. Deleting a token revokes it immediately.
