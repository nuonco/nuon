...
- **App-wide matrix rules** (`operation_roles.toml`): Define a central lookup table that maps entity
  ...
  `nuon::action:*`)
- **Runtime role override**: Pass `--role-name <name>` via the CLI or use the role selector in the dashboard to
...
1. Runtime override (CLI `--role-name` flag or dashboard selection)
