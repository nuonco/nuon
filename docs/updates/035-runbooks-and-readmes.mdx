...
Operational procedures shouldn't live in someone's head or a stale wiki page — they should run. Runbooks let you define a named, ordered sequence of steps — the on-call restart, a version upgrade, a database migration, a recovery procedure — and run it against an install on demand, instead of translating prose into commands by hand at the worst possible hour.
...
Trigger and watch runs from the new **Runbooks** tab in the dashboard, or from the CLI with `nuon runbooks --install-id <id>`. See the [Runbooks guide](/guides/runbooks) for the full configuration reference, or the [`programmable-runbook-and-readme`](https://github.com/nuonco/example-app-configs/tree/main/programmable-runbook-and-readme) example app config for a complete, runnable demo of runbooks and rendered READMEs together.
