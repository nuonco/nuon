...
Now that you're logged in you can create your first organization. We'll create a "sandbox" org, which means it won't create real cloud resources. This will save you time and money, and can be a useful way to test out app configurations even after you create real org.
...
<Note>
You don't need to connect to Github for this guide, since we're just deploying an open-source Docker image.
</Note>
...
This will create a new build of your component, and release it. A release will create a deploy for each install.

You can use the CLI to inspect the state of builds and releases.

<CodeGroup>
```sh list builds
nuon builds list -c http_bin
```

```sh view build logs
nuon builds logs -b {build_id} -c http_bin
```

```sh list deploys
nuon installs list-deploys -i {install_id}
```
