...
| `manual-run` | `nuon apps branches trigger`, or the API, or the dashboard | Same steps as a `git-run`. Add `--preview` and it still plans every group and waits at each approval gate — it just applies nothing. |
