...
created at all, so a pull request cannot reach an install even in principle.
...
Nothing in a branch run reaches an install unseen. Each `plan install group` step computes a diff
