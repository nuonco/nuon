...
boost: 4
keywords: ["app branches", "app branch", "what are app branches", "continuous delivery", "git push deploy", "gitops", "deployment groups", "install groups", "install_groups", "label selector", "staged rollout", "progressive rollout", "canary", "plan and approve", "PR preview", "pull request preview", "branch run", "git-run", "git-preview-run", "manual-run", "install version history", "app config version", "fleet deployment", "roll out to many installs"]
---
...
<Note>
  ...
  describe, rather than replacing them.
</Note>
...
<Frame caption="An app branch's deployment plan: the stage group deploys first, then production, each selecting its installs by label.">
  <img src="/images/concepts/app-branches/deployment-plan-graph.png" alt="A branch's deployment plan in the Nuon dashboard: a stage group selecting installs labelled env=stage flowing into a production group selecting env=prod, with each group's installs listed below" />
</Frame>
...
Merging a pull request starts a `git-run` through the push it makes to the tracked branch; the close event itself is
ignored.
...
- **By labels** — a `label_selector` of key/value pairs. Any install carrying all of those labels is in the group, and
  membership updates itself as you label and unlabel installs. Prefer this for most groups.
...
Because the gate is an approval rather than a timer, you set the pace: approve the next group in the dashboard, or
...
- a comment titled `## Nuon Preview — <BranchName>` (the branch's `name`, so usually `main`), containing a table of
  ...
  in place on every later push rather than added to.
...
computes a diff per install in that group, showing exactly what would change, and holds the rollout there.

The gate is acted on in your org: someone reads the per-install diff in the dashboard and approves or skips the
group, or your own automation advances it through the API. It is the same plan-and-approve review that
[workflows](/concepts/workflows) and [approvals](/concepts/workflows#workflow-approvals) give a single install,
applied to a whole group and driven from your git history.
