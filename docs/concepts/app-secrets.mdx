...
be used to configure components using [variables](/guides/using-variables).
...
You can configure a component using an app secret by using [template vars](/guides/using-variables). To reference any secret,
...
1. Accepting a `sensitive` [app-input](/concepts/app-inputs) provided by the customer during installation.
2. Reading a customer provisioned secret, such as from [AWS Secrets Manager](https://aws.amazon.com/es/secrets-manager/).
3. Dynamically generating + managing secrets using a [Terraform component](/guides/terraform-components).
