...
<Note>
	...
	CloudFormation template using their cloud credentials, the customer (not the vendor) can enter the secret values at that time. Neither Nuon nor the vendor will ever see the secret values, as they are not stored in the Nuon data plane.
</Note>
