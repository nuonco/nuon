...
The sandbox sets up your application's base **compute** environment — for example a Kubernetes (EKS, AKS, GKE) or ECS cluster — so your app has a consistent, isolated place to run across every install. In a shared-responsibility model, the sandbox can also build on infrastructure a customer already provides, such as an existing cluster.

<Note>
The install's **network (VPC/VNet), the [Runner](/concepts/runners), and the Runner's permissions are provisioned by the install [Stack](/concepts/stacks)** — the Terraform or CloudFormation/Bicep the customer deploys first — not by the sandbox. The sandbox runs inside the network the Stack creates.
</Note>
