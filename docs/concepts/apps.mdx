...
Apps are versions of your application code and infrastructure that can be deployed into a customer cloud account.
...
Each App is represented as multiple configuration `.toml` files and collectively known as a an `App Config`.
...
Inputs allow you to define different inputs that customers can provide to a App. This enables you to create "configuration" knobs that your customers can manage to control your application.
...
Components are used to connect your existing application and infrastructure code to your Nuon App.
...
Apps allow you to create dependencies between components and expose a rich configuration language to share outputs
...
When running an App in an [Install](/concepts/installs), you often need to document important info. It can also be nice to have some level of programmability, such being able to render a button that will trigger an [Action](/concepts/actions). Additionally, this documentation must be rendered per-Install because certain values -- such as URLs -- will be different for each Install.
...
**Create an App** - Create an app with no configuration files. e.g., so you can sync existing config files later. For consistency, use the same name as the directory you created above.
