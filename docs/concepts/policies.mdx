...
| `helm_chart` | Helm chart components | OPA | Build, Deploy | Kubernetes AdmissionReview |
...
Unlike components, policies do not need to be built—they are evaluated directly during build and deploy workflows.
...
- **`helm_chart`**: Policies evaluate the rendered Kubernetes manifests after chart templating
...
<Note>
  `helm_chart` components support both build-time and deploy-time policy evaluation. 
  Build-time catches issues early during the build phase, while deploy-time validates 
  the final manifests before deployment to the cluster.
</Note>
