...
keywords: ["actions", "what are actions", "health checks", "cron schedule", "action triggers", "day 2 operations", "database migrations", "kubectl", "bash scripts", "operational runbooks", "post-deploy", "pre-provision", "break glass actions", "remediation actions", "one-off commands", "ad-hoc operations", "scripted operations", "container actions", "image-backed actions", "action image"]
---
...
By default an action's steps run directly on the install [runner](/concepts/runners) VM, so they are
limited to the tooling that VM carries. An action can instead declare a container `image`, in which case
every step runs inside that image and ships with whatever tools it needs:
...
The image can be a public reference, or the `image.ref` output of a
[container image component](/guides/container-image-components) when the tools image is private. See
[Container actions](/guides/container-actions) for how to configure one.
