...
The `nuon` CLI is the fastest path for engineers embedding install management in scripts, CI jobs, or a provisioning workflow.

<Note>
[CLI Reference documentation can be found here.](/cli) This page is specific to Install Management from the CLI.
</Note>
...
Install lifecycle events run as [workflows](/concepts/workflows). The CLI ships with a full-screen TUI for monitoring and interacting with them:
