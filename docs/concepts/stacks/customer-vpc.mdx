...
Point `vpc_nested_template_url` at the `byo-vpc/default` template. It accepts existing VPC and subnet IDs as parameters instead of creating them:
...
The customer enters their VPC and subnet IDs when they deploy the Stack. From there, the vendor's `sandbox.toml` wires them into the [Sandbox](/concepts/sandboxes) so it attaches to the customer's subnets instead of creating its own:
