---
title: 'Inputs'
description: "Inputs allow you to define customer provided values that can be used to configure your App."
---
...
Inputs are configured in a `inputs.toml` file inside the root of your App Config directory.
...
Inputs can be used to configure both Sandboxes and Components. All Inputs are made available via
[variable](/guides/using-variables) configuration.

Inputs are defined in a `inputs.toml` file inside the root of your App Config directory.
...
For example, to use the `root_domain` Input, as a variable to configure the Sandbox:
...
```toml inputs.toml
#:schema http://localhost:8081/v1/general/config-schema?source=inputs
[[group]]
name         = "db"
...
Inputs are commonly used to allow a customer to provide access to existing resources. For example, an Input can be
used for:
...
You can even use Inputs to manage resources from [third party cloud providers](/concepts/platforms#supporting-custom-platforms).
