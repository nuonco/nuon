...
| Install Config Sync | The process of reconciling Installs with per-install TOML config files tracked in git. Enabled by setting one of `installs_connected_repo` / `installs_public_repo` on the App Branch, plus `installs_directory`. Each sync records a version and diff per Install. |
