...
| PR Preview | A plan-only App Branch Run triggered by opening or updating a pull request against a tracked branch. Nuon posts the resulting config diff as a pull request comment and sets a `nuon/preview` commit status. Requires a repository connected through the Nuon GitHub App. |
| Install Config Sync | The process of reconciling Installs with per-install TOML config files tracked in git, from the App Branch's `installs_directory`. Each sync records a version and diff per Install. |
