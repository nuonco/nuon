...
| Install Config Sync | The process of reconciling Installs with per-install TOML config files tracked in git. Enabled by setting one of `installs_connected_repo` / `installs_public_repo` on the App Branch; `installs_directory` names the directory and defaults to `installs`. Each sync records a version and diff per Install. |
