...
keywords: ["roles", "RBAC", "access control", "permissions", "org admin", "read-only", "team", "service account", "API token", "OIDC", "stack", "runner"]
---
...
The **Stack** role is reserved for the machine accounts Nuon provisions for install stacks managed with the Terraform module. It is assigned automatically, is never user-selectable, and — unlike the org-wide roles — grants access scoped to a single install. See [Stack service accounts](/concepts/service-accounts#stack-service-accounts) for details.
