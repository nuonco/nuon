...
You can create and manage [Installs](/concepts/installs) by creating `<install>.toml` files in a `installs` directory. Install config must
declare a `name` field and any relevant install-specific configuration.
