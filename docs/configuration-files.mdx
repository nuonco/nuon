...
You can also manage [Installs](/concepts/installs) with config files. This
allows you to manage installs in your VCS, offering a more streamlined
experience than the dashboard. By defining these configurations in code,
you can automate Install updates with your CI system.

An install config file requires a name, the aws account region, and
non-sensitive input values. You can also configure auto-approval.

All input values should be defined as strings. They will be parsed into the
correct type by Nuon. If you define an input multiple times, the last value
will be used. To avoid storing sensitive input values in plain text, they
are excluded from input config files. They will not be included in generated
files, and will not be synced.

```toml installs/example.toml
#:schema https://api.nuon.co/v1/general/config-schema?type=install

name = "<install-name>"

approval_option = "approve-all" # enabled auto-approval, which you typicall need for automation
...
You can generate a config file for an Install using the Dashboard or the CLI.

<Tabs>
<Tab title="Dashboard">
To generate an install config using the Dashboard, navigate to the install's page, and open the "Manage" menu. First, you need to enable config syncing.

![Enable config sync](/images/enable-config-sync.png)

Then, you can generate a config. Save this to a file locally, and you are ready to sync it using the CLI.

![Generate install config](/images/generate-install-config.png)
</Tab>
<Tab title="CLI">
You can generate a config using the CLI with the `installs generate-config` command.

```bash
mkdir -p installs
touch installs/<install-name>.toml
nuon installs generate-config -i <install-name> > <install-name>.toml
```
</Tab>
</Tabs>
...
Once you have an install config, you can sync it using the CLI. You can sync
either an individual file, or an entire directoy.
...
<Note>
Because the app name is required for syncing, you cannot sync a directory of install
configs for different apps. If you are syncing a directory, they must be the same app.
</Note>
