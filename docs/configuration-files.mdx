...
Since configs can get complex as your application grows, we follow a structured approach to organize your configuration files.
Create a `components` directory in your project root with individual files for each component. e.g., `components/helm_deploy.toml`.
...
in an `installs` directory managed separately from the app configs. An Install config must declare a `name` field and
