...
Feature-flag and management-policy checks remain in the handlers. Release discovery, release files, packages, and
...
workflow. It also creates the initial `InstallManagementPolicyVersion`, ensures the dedicated portal service account,
and binds its install-scoped role. The response includes the install, policy, and non-secret service-account metadata
...
- `InstallManagementPolicyVersion`, which records connected policy and customer/vendor authority;
