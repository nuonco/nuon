data "aws_iam_policy_document" "odr_default" {
}

resource "aws_iam_policy" "odr_default" {
  name   = "odr-default-${local.vars.id}"
  policy = data.aws_iam_policy_document.odr_default.json
}

module "odr_default_iam_role" {
  ...
  role_name = "odr-default-${local.vars.id}"
  role_path = "/nuon/"
  ...
  role_policy_arns = {
    custom = aws_iam_policy.odr_default.arn
  }
}
