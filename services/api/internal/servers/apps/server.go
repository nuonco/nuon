...
import (
	...
	connectv1 "github.com/powertoolsdev/mono/pkg/types/api/app/v1/appv1connect"
...
type server struct {
	...
	Svc          services.AppService   `validate:"required"`
...
func WithHTTPMux(mux *http.ServeMux) serverOption {
	return func(s *server) error {
		path, handler := connectv1.NewAppsServiceHandler(s, connect.WithInterceptors(s.Interceptors...))
		mux.Handle(path, handler)
		return nil
	}
}

func WithService(svc services.AppService) serverOption {
	return func(s *server) error {
		s.Svc = svc
		return nil
	}
