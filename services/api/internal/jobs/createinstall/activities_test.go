...
func Test_ActivityTriggerInstallJob(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"happy path": {
			...
			mockMgr: func(ctl *gomock.Controller) *workflows.MockInstallWorkflowManager {
				...
				wkflowmgr.EXPECT().Provision(gomock.Any(), install, app.OrgID.String(), sandboxVersion).Return(nil)
				return wkflowmgr
			},
		},
		"repo error": {
	...
	for name, test := range tests {
		t.Run(name, func(t *testing.T) {
			...
			_, err := act.TriggerInstallJob(context.Background(), installID.String())
