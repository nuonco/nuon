...
import { createBranchConfig, getAppInstalls, getVCSConnectionRepos } from '@/lib'
import {
  ...
  const handleSubmit = async () => {
    ...
    const { error: createError } = await createBranchConfig({
      ...
      request: {
        ...
        install_groups: installGroups.map((g) => ({
          name: g.name,
          ...
          {vcsConnections.length === 0 ? (
            ...
            <>
              {vcsConnections.length > 1 && (
                <div>
                  <label htmlFor="vcs-connection" className="block text-sm font-medium mb-2">
                    VCS Connection
                  </label>
                  <select
                    id="vcs-connection"
              ...
              <div>
                <label htmlFor="git-branch" className="block text-sm font-medium mb-2">
                  Branch
                </label>
                {loadingBranches ? (
                  <div className="px-3 py-2 text-sm text-gray-500">
              ...
              <div>
                <label htmlFor="path-filter" className="block text-sm font-medium mb-2">
            ...
            <div className="space-y-4">
              {installGroups.map((group, idx) => (
                ...
                >
                  ...
                  <div>
                    <label className="block text-sm font-medium mb-2">Installs</label>
                    <select
                      multiple
