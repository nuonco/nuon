...
export const EditInstallGroupsPanel = ({
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    const sourceGroup = groups.find((g) => g.install_ids.includes(activeInstallId))
        ...
        setGroups(
          groups.map((g) => {
            if (g.id === sourceGroup.id) {
              return {
                ...g,
                install_ids: g.install_ids.filter((id) => id !== activeInstallId),
              }
            }
            if (g.id === targetGroup.id && !g.install_ids.includes(activeInstallId)) {
              return {
                ...g,
    ...
    if (overGroupId === 'unassigned' && sourceGroup) {
      setGroups(
        groups.map((g) =>
          g.id === sourceGroup.id
            ? { ...g, install_ids: g.install_ids.filter((id) => id !== activeInstallId) }
            : g
        )
      )
    }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      console.log('API endpoint:', `POST /v1/apps/${appId}/branches/${branch.id}/configs`)
  ...
  return (
    ...
    >
      ...
      {loadingInstalls ? (
        ...
        <Banner theme="info">
          No installs found for this app. Create installs first to configure deployment
          groups.
        </Banner>
      ) : (
        <DndContext
