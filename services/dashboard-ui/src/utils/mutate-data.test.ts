import '@test/mock-fetch-options'
import { expect, test } from 'vitest'
import type { TOrg } from "../types"
import { mutateData } from './mutate-data'

test('mutateData should return an new org object when provided the orgs path', async () => {
  const spec = await mutateData<TOrg>({
    data: { name: 'test' },
    path: "orgs",
  })

  expect(spec).toHaveProperty('id')
  expect(spec).toHaveProperty('created_at')
  expect(spec).toHaveProperty('updated_at')
  expect(spec).toHaveProperty('name')
})

test('mutateData should throw an error with the default error message when it can not create an orgs', async () => {
  try {
    await mutateData<TOrg>({
      data: { name: 'test' },
      path: "orgs",      
    })
  } catch (error) {
    expect(error).toMatchInlineSnapshot(`[Error: Encountered an issue retrieving this information, please refresh the page to try again.]`)
  }
})


test('mutateData should throw an error with a custom error message when it can not create an orgs', async () => {
  try {
    await mutateData<TOrg>({
      data: { name: 'test' },
      errorMessage: "Custom error message!",
      path: "orgs",      
    })
  } catch (error) {
    expect(error).toMatchInlineSnapshot(`[Error: Custom error message!]`)
  }
})
