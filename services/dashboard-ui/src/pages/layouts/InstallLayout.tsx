...
export default function InstallLayout() {
  ...
  const {
    ...
    path: `/api/ctl-api/v1/installs/${installId}`,
    shouldPoll: true,
    pollInterval: 20000,
  })
    ...
    return (
      <div className="flex items-center justify-center h-full p-8">
        <div className="text-center">
          <p className="text-red-600 mb-4">Failed to load install</p>
  ...
  const isThirdLevel = pathSegments.length > 4
  ...
  return (
    ...
    >
      <PageSidebarProvider initIsPageSidebarOpen={false}>
        <ToastProvider>
          <SurfacesProvider>
            <PageLayout>
              {isThirdLevel ? (
                <PageContent className="border-t" isScrollable variant="secondary">
                  <SubNav
                    basePath={`/${orgId}/installs/${installId}`}
                    links={[
                      {
                        path: `/`,
