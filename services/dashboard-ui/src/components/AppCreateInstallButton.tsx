      ...
      >
        ...
        >
          ...
          <div className="flex flex-col gap-8 px-6 max-w-3xl">
            ...
            {platform === 'aws' ? <AWSFields /> : <AzureFields />}
            {inputConfig ? <InputConfigs inputConfig={inputConfig} /> : null}
          </div>
          ...
          <div className="flex gap-3 justify-end border-t w-full p-6 p-6">
            ...
            <Button
              ...
              disabled={isLoading}
            >
              {isLoading ? <SpinnerSVG /> : <Cube size="16" />} Create Install
            </Button>
          </div>
        </form>
      </Modal>
...
const InputConfigs: FC<{ inputConfig: TAppInputConfig }> = ({
  inputConfig,
}) => {
  return (
    <>
      {inputConfig?.input_groups
        ? inputConfig?.input_groups?.map((group, i) => (
            <InputGroupFields key={group.id} groupInputs={group} />
          ))
        : null}
    </>
  )
        ...
        Boolean(input?.default === 'true' || input?.default === 'false') ? (
          ...
          >
            ...
            <CheckboxInput
              ...
              defaultChecked={Boolean(input?.default === 'true')}
