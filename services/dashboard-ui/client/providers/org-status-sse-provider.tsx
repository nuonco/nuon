...
import type { TOrg, TWorkflow, TWorkflowStepApproval } from '@/types'
...
export function OrgStatusSSEProvider({ children }: { children: ReactNode }) {
  const { org } = useOrg()
  const queryClient = useQueryClient()

  const sseUrl = org?.id ? `/api/orgs/${org.id}/status/sse` : undefined
        ...
        try {
          ...
          if (runnerId) {
            queryClient.setQueryData(
              ['runner-heartbeat', org?.id, runnerId],
              data
            )
          }
        } catch {}
