...
Visual and UX guidance for `client/lite/`. Engineering conventions live in
[DEV.md](./DEV.md), copy rules in [COPY.md](./COPY.md), and per-component rules
in each component's `Overview` story.

The premise of Lite is **less surface area**. Every rule here exists to make the
app smaller and more consistent, not more expressive. When a choice is between
"more visual variety" and "one fewer thing to learn", take the second.
...
`solid` with `blur="lg"` and `shadow="floating"`. `Card` has no padding *prop
override* via className: change `padding`, don't fight it with `!p-*`.
...
Status is the app's most important visual signal, so it gets exactly one system.
...
`Status` has four variants: `chip` (tinted pill, the default for a resource's
state), `inline` (icon plus label, no fill), `dot` (dense contexts such as table
rows) and `icon` (icon only, tooltip carries the label).

- **Status is not a `Badge`.** `Badge` is metadata (labels, counts,
  classifications). Reaching for a `Badge` to show state is the single most
  common mistake here.
- Colour is never the only carrier — each theme has a distinct icon.
- Six themes is the whole vocabulary. Do not invent a seventh.

## Button emphasis

- **`secondary` is the default.** Use it for standalone actions that must stay
  findable without surrounding context.
- **`primary` is the one action a page wants you to take.** At most one per view.
- **`ghost` is the lowest emphasis.** Use it inside framed chrome — toolbars,
  grouped controls, table rows, surface footers.
- **`danger`** for destructive confirmation only, not for the button that opens
  the confirmation.
- Do not use a ghost button as the only action in a section or page.
- In a toggle group, the selected option is `secondary` and the rest are `ghost`.
- A row of related toolbar controls must not become a row of competing
  `secondary` buttons.

Two sizes, `md` (default) and `sm`. `sm` is for dense chrome, not for making an
...
1. **Main sidebar** — the org's top-level areas.
2. **Resource context nav** — within an install, an app branch, or org settings.

Anything that wants a third level is a signal the information architecture is
wrong, not that the app needs another nav.

**Do not create a list page for every noun in the data model.** The question a
page answers is "what exists and what happened", not "what tables are in the
API". Detail *panels* for resources; detail *pages* for runs.

Task flows split by weight:

- **Full-screen wizard** (`FocusShell`) for creation that spans real decisions —
  create app and connect a branch, create an install and watch its first
  provision.
- **Modal** for creation that is one form — API token, service account, webhook,
  Slack channel, trust policy.
