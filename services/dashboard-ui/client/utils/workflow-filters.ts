...
export const typeFilterParameter = (
  ...
  const types = (Object.keys(WORKFLOW_TYPE_GROUPS) as TWorkflowType[]).filter(
    (type) => groups.has(WORKFLOW_TYPE_GROUPS[type])
  )
...
export const datePresetFilterParameter = (
  ...
  return widest
    ? (now.minus({ hours: DATE_PRESET_HOURS[widest] }).toUTC().toISO() ??
        undefined)
    : undefined
}

export const datePresetQueryParameter = (
  value: string | null,
  now: DateTime = DateTime.now()
) => {
  const valid = new Set(Object.keys(WORKFLOW_DATE_LABELS))
  const selected = (value?.split(',') ?? []).filter(
    (preset): preset is TWorkflowDatePreset => valid.has(preset)
  )
  return datePresetFilterParameter(selected, now)
}
...
const readOptions = <T extends string>(
  ...
  return new Set(
    (searchParams.get(key)?.split(',') ?? []).filter((value): value is T =>
      valid.has(value as T)
    )
  )
}

export const readWorkflowFilters = (
  searchParams: URLSearchParams,
  owner: TWorkflowOwner
) => {
  const status = readOptions(searchParams, 'status', workflowStatusOptions())
  const type = readOptions(searchParams, 'type', workflowTypeOptions(owner))
  const date = readOptions(
    searchParams,
    'since',
    Object.keys(WORKFLOW_DATE_LABELS) as TWorkflowDatePreset[]
  )
  const preview = readOptions(
    searchParams,
    'preview',
  ...
  return {
    ...
    api: {
      ...
      type: typeFilterParameter(type),
      preview: owner === 'app' ? previewFilterParameter(preview) : undefined,
    },
    filtered:
      !!search ||
      status.size > 0 ||
