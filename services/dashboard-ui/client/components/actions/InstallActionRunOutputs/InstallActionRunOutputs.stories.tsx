...
const mockRunWithOutputs = {
  steps: [
    ...
    },
    {
      ...
      step_id: 'step-2',
      status: 'finished',
      execution_duration: 30000000000,
    },
  ],
  config: {
    steps: [
  ...
  steps: [
    {
      ...
      step_id: 'step-1',
      status: 'finished',
      execution_duration: 10000000000,
    },
  ],
  config: {
  ...
  steps: [
    {
      ...
      step_id: 's-1',
      status: 'finished',
      execution_duration: 12000000000,
    },
    {
      id: 'sr-2',
      step_id: 's-2',
      status: 'finished',
      execution_duration: 87000000000,
    },
    {
      id: 'sr-3',
      step_id: 's-3',
      status: 'finished',
      execution_duration: 34000000000,
    },
    {
      id: 'sr-4',
      step_id: 's-4',
    ...
    },
    {
      ...
      step_id: 's-5',
      status: 'finished',
      execution_duration: 120000000000,
    },
    {
      id: 'sr-6',
      step_id: 's-6',
      status: 'finished',
      execution_duration: 22000000000,
    },
  ],
  config: {
    steps: [
  ...
  outputs: {
    steps: {
      ...
      'deploy-eks': {
        ...
        cluster_endpoint:
          'https://ABCDEF1234567890.gr7.us-west-2.eks.amazonaws.com',
        cluster_version: '1.29',
        cluster_status: 'ACTIVE',
        oidc_provider_arn:
          'arn:aws:iam::123456789012:oidc-provider/oidc.eks.us-west-2.amazonaws.com/id/ABCDEF',
        node_groups: {
          'workers-general': {
            ...
            status: 'ACTIVE',
            node_group_arn:
              'arn:aws:eks:us-west-2:123456789012:nodegroup/prod/workers-general/abc123',
          },
          'workers-gpu': {
            ...
            status: 'ACTIVE',
            node_group_arn:
              'arn:aws:eks:us-west-2:123456789012:nodegroup/prod/workers-gpu/def456',
          },
        },
        cluster_security_group_id: 'sg-0a1b2c3d4e5f6a7b8',
        service_role_arn:
          'arn:aws:iam::123456789012:role/eks-service-role-prod',
      ...
      'configure-dns': {
        ...
        acm_certificate_arn:
          'arn:aws:acm:us-west-2:123456789012:certificate/abc-def-ghi',
        certificate_status: 'ISSUED',
        validation_method: 'DNS',
      },
      'run-migrations': {
        error:
          'Migration 20240115_add_indexes failed: lock timeout exceeded after 30s on table "events"',
      ...
      'deploy-services': {
        deployments: {
          'api-server': {
            image:
              '123456789012.dkr.ecr.us-west-2.amazonaws.com/api-server:v2.14.3',
          ...
          'web-frontend': {
            image:
              '123456789012.dkr.ecr.us-west-2.amazonaws.com/web-frontend:v3.8.1',
            replicas: { desired: 2, ready: 2, available: 2 },
            namespace: 'production',
            service_url:
              'http://web-frontend.production.svc.cluster.local:3000',
            external_url: 'https://app.prod.acme-corp.io',
            status: 'Running',
          },
          'worker-processor': {
            image:
              '123456789012.dkr.ecr.us-west-2.amazonaws.com/worker:v2.14.3',
          ...
          'grpc-gateway': {
            image:
              '123456789012.dkr.ecr.us-west-2.amazonaws.com/grpc-gateway:v1.5.0',
            replicas: { desired: 2, ready: 2, available: 2 },
            namespace: 'production',
            service_url:
              'http://grpc-gateway.production.svc.cluster.local:9090',
        ...
        ingress: {
          alb_arn:
            'arn:aws:elasticloadbalancing:us-west-2:123456789012:loadbalancer/app/prod-ingress/abc123',
      ...
      'health-check': {
        checks: {
          'api-server': {
            endpoint: 'https://api.prod.acme-corp.io/healthz',
            status: 200,
            latency_ms: 23,
          },
          'web-frontend': {
            endpoint: 'https://app.prod.acme-corp.io/health',
            status: 200,
            latency_ms: 45,
          },
          'grpc-gateway': {
            endpoint:
              'grpc://grpc.prod.acme-corp.io:443/grpc.health.v1.Health/Check',
            status: 200,
            latency_ms: 12,
          },
          database: {
            endpoint:
              'postgres://prod-db.cluster-abc123.us-west-2.rds.amazonaws.com:5432',
            status: 200,
            latency_ms: 8,
          },
          redis: {
            endpoint: 'redis://prod-cache.abc123.use2.cache.amazonaws.com:6379',
            status: 200,
            latency_ms: 3,
          },
        },
