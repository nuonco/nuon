...
export const LabelsPlusDefault = () => (
  <div className="p-4">
    <DeploymentPlanGraph
      config={mockConfigDefault}
      installsById={mockInstalls}
      orgId="org123"
    />
  </div>
...
function buildLargePlan(
  ...
  const install_groups = specs.map((spec, gi) => {
    const labelKey = spec.labelKey ?? `group-${gi}`
    ...
    return {
      ...
      name: spec.name,
      order: gi,
      label_selector: { match_labels: { [labelKey]: 'true' } },
...
const largePlan = buildLargePlan([
  { name: 'Canary', count: 4, labelKey: 'canary' },
  { name: 'Production US', count: 30, maxParallel: 4, labelKey: 'prod-us' },
  { name: 'Production EU', count: 18, maxParallel: 3, labelKey: 'prod-eu' },
  { name: 'Staging', count: 9, maxParallel: 2, labelKey: 'staging' },
])
