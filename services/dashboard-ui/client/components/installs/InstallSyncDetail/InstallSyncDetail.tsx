...
import type { TAPIError, TAppInstallConfigSync, TWorkflowStep } from '@/types'
import { getStatusTheme } from '@/utils/status-utils'

export interface IInstallSyncDetail {
    ...
    <>
      ...
      {approvalStep && workflowId && (
        <ApprovalBanner
          ...
          orgId={orgId}
          syncQueryKey={['app-install-sync', orgId, appId, syncId]}
        />
      )}
              ...
              <div className="flex flex-col gap-1 min-w-0">
                {sync.vcs_connection_commit.message && (
                  <Text variant="body" weight="strong" className="truncate">
                    {sync.vcs_connection_commit.message.split('\n')[0]?.trim()}
                  </Text>
                )}
