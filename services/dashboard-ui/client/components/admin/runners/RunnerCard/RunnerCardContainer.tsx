...
export const RunnerCardContainer = ({
  ...
  onAction,
}: RunnerCardContainerProps) => {
  const { addToast } = useToast()
  const { user } = useAuth()
  const adminEmail = user?.email ?? ''
  const runnerId = runner.id

  const { mutate: gracefulShutdown, isPending: isGracefulLoading } =
    useMutation({
      mutationFn: () => adminGracefulRunnerShutdown({ runnerId, adminEmail }),
      onSuccess: () => {
        addToast(
          <Toast heading="Runner shutting down" theme="success">
            <Text>Graceful shutdown initiated.</Text>
          </Toast>
        )
        onAction?.()
      },
      onError: () => {
        addToast(
          <Toast heading="Shutdown failed" theme="error">
            <Text>Failed to initiate graceful shutdown.</Text>
          </Toast>
        )
      },
    })
  ...
  const { mutate: forceShutdown, isPending: isForceLoading } = useMutation({
    ...
    onSuccess: () => {
      addToast(
        <Toast heading="Runner force stopped" theme="success">
          <Text>Force shutdown initiated.</Text>
        </Toast>
      )
      onAction?.()
    },
    onError: () => {
      addToast(
        <Toast heading="Force shutdown failed" theme="error">
          <Text>Failed to force shutdown runner.</Text>
        </Toast>
      )
    },
  })

  const { mutate: invalidateToken, isPending: isInvalidateLoading } =
    useMutation({
      mutationFn: () => adminInvalidateRunnerToken({ runnerId, adminEmail }),
      onSuccess: () => {
        addToast(
          <Toast heading="Token invalidated" theme="success">
            <Text>Runner token has been invalidated.</Text>
          </Toast>
        )
        onAction?.()
      },
      onError: () => {
        addToast(
          <Toast heading="Token invalidation failed" theme="error">
            <Text>Failed to invalidate runner token.</Text>
          </Toast>
        )
      },
    })
