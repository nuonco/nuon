...
export const maxReleaseFilePreviewBytes = 5 * 1024 * 1024

export const releaseFileEntryCanPreview = (entry?: TReleaseFileEntry) =>
  [entry?.current, entry?.previous].every(
    (version) => !version?.size || version.size <= maxReleaseFilePreviewBytes
  )
...
const previewFile = (
  ...
  if (version.size && version.size > maxReleaseFilePreviewBytes) return null
  if (
    isSource &&
...
const FilePreview = ({
  ...
  const canPreview = releaseFileEntryCanPreview(entry)
  ...
  return (
    <div className="flex flex-col gap-4 min-w-0">
      ...
      {!canPreview ? (
        <Text theme="neutral">
          This file is larger than 5 MiB and cannot be previewed. Its digest is
        ...
        <Text theme="neutral">
          This file has no text preview. Its digest is still available for
          verification.
        </Text>
      )}
    </div>
