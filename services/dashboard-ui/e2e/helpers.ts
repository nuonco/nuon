...
async function api(apiPath: string, options: RequestInit = {}) {
  ...
  if (!res.ok) {
    throw new Error(
      `Public API ${apiPath} failed (${res.status}): ${await res.text()}`
    )
  }
  return res
}
...
export async function createTriggerableBranch(): Promise<{
  ...
  await api(`/v1/apps/${app.id}/branches/${branch.id}/configs`, {
    method: 'POST',
    body: JSON.stringify({
      install_groups: [{ name: 'default', default: true, order: 0 }],
    }),
  })
...
export async function waitForComponentBuildActive(
  ...
  const comps = (await (await api(`/v1/apps/${app.id}/components`)).json()) as {
    id: string
  }[]
