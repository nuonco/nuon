...
const STORY =
  "/?story=branches--previewconfigsection--edit-modal&mode=preview";

test.describe("PreviewConfig form behavior", () => {
  test.beforeEach(async ({ page }) => {
    await page.goto(STORY, { waitUntil: "domcontentloaded" });
    await page
      .getByRole("button", { name: "Edit preview settings" })
      .click();
  ...
  test("build-only mode hides the default install", async ({ page }) => {
    const dialog = page.getByRole("dialog");

    await expect(dialog.getByLabel("Default install")).toBeVisible();
    await dialog.getByLabel("Build only").check();
