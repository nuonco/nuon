...
func (p *portalServer) stepResult(w http.ResponseWriter, r *http.Request) {
	...
	result, ok, err := p.readStepResult(r.Context(), id)
	if err != nil {
...
func (p *portalServer) readStepResult(ctx context.Context, id string) (*decodedStepResult, bool, error) {
	raw, ok, err := p.store.Get(ctx, statestore.StepResultKey(id))
	if err == nil && !ok {
		if runID, stepID, found := strings.Cut(id, "--"); found && runID != "" && stepID != "" {
			raw, ok, err = p.store.Get(ctx, operation.RunStepResultKey(runID, stepID))
		}
	}
	if err != nil {
	...
	var stackCandidate operation.StackCandidate
	if err := json.Unmarshal(raw, &stackCandidate); err == nil && stackCandidate.StackName != "" && stackCandidate.ChangeSetName != "" {
		return &decodedStepResult{Success: stackCandidate.Status != "FAILED", Kind: "cloudformation", Content: raw}, true, nil
	}
	var result struct {
		Success                   bool            `json:"success"`
	...
	if err := json.Unmarshal(raw, &result); err != nil {
		return nil, false, fmt.Errorf("decode step result %s: %w", id, err)
	}
	...
	if err != nil {
		return nil, false, fmt.Errorf("decode step result %s contents: %w", id, err)
	}
	return &decodedStepResult{Success: result.Success, Kind: classifyResultContent(content), Content: content}, true, nil
}
...
func classifyResultContent(content json.RawMessage) string {
	...
	switch {
	case probe.StackName != "" && probe.ChangeSetName != "" && probe.Changes != nil:
