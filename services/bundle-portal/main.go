...
func run(logger *zap.Logger) error {
	var cfg config
	flag.StringVar(&cfg.controlPlaneURL, "control-plane-url", "", "ctl-api base URL for connected mode")
	flag.StringVar(&cfg.orgID, "org-id", "", "Nuon organization ID for connected mode")
	flag.StringVar(&cfg.installID, "install-id", "", "customer-managed install ID for connected mode")
	...
	baseStore, err := operationstate.New(ctx, cfg.state, cfg.profile, cfg.region)
	if err != nil {
		return err
	}
	runnerStore := operationstate.WithLegacyOperationsRead(operationstate.WithPrefix(baseStore, operationstate.RunnerNamespace))
	controlStore := operationstate.WithLegacyOperationsRead(operationstate.WithPrefix(baseStore, operationstate.ControlNamespace))
	store := operationstate.ReadOverlay(runnerStore, controlStore, operationstate.WithLegacyOperationsRead(operationstate.Legacy(baseStore)))
	stackStore := store
	if parent, ok := stackOutputsRoot(cfg.state); ok {
		stackStore, err = operationstate.New(ctx, parent, cfg.profile, cfg.region)
		if err != nil {
			return err
		}
		stackStore = operationstate.WithLegacyOperationsRead(stackStore)
	}
	...
	if cfg.stackName != "" {
		...
		portal.deploymentID = cfg.deploymentID
		portal.cloudProvider = "aws"
		portal.cloudAccountID = stringValue(identity.Account)
		portal.cloudRegion = awsCfg.Region
...
func (c config) validate() error {
	...
	connectedValues := 0
	for _, value := range []string{c.controlPlaneURL, c.orgID, c.installID, c.apiToken} {
		if value != "" {
			connectedValues++
		}
	}
	if connectedValues > 0 {
		if connectedValues != 4 {
			return fmt.Errorf("connected mode requires --control-plane-url, --org-id, --install-id, and --api-token together")
		}
		if c.hasOfflineOptions() {
			return fmt.Errorf("connected mode flags cannot be combined with offline state or AWS bundle flags")
		}
...
func (c config) hasOfflineOptions() bool {
	return c.state != "" || c.localState != "" || c.region != "" || c.profile != "" || c.stackName != "" || len(c.bundleArchive) > 0 || c.bucketPrefix != "" || c.deploymentID != "" || c.bundleKey != "" || c.bundleURI != "" || c.runnerImage != "" || c.ecrRegistry != "" || c.stackOutputsKey != "" || c.installInputsURI != ""
}
...
func runConnected(ctx context.Context, cfg config, logger *zap.Logger) error {
	...
	portal, err := newPortalServer(nil, nil, token, "", hosts, logger)
	if err != nil {
		return err
	}
	portal.branding = cfg.branding()
