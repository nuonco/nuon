...
func TestPortalCombinesInstallationUpgradeAndDay2Runs(t *testing.T) {
	stateDir := t.TempDir()
	...
	day2Started := upgradeFinished.Add(time.Minute)
	...
	writeStateObject(t, stateDir, day2.RunStatusKey("day2-run"), day2.RunStatus{
		RunID: "day2-run", RefID: "restart", RefKind: day2.RefKindAction, RefName: "Restart", Source: day2.SourcePortal,
		Status: day2.RunStatusFinished, StartedAt: day2Started,
	})

	portal, err := newPortalServer(day2state.NewLocal(stateDir), day2state.NewLocal(deploymentDir), "secret", "operator", map[string]bool{"127.0.0.1:1234": true}, zaptest.NewLogger(t))
	require.NoError(t, err)
	runs, err := portal.listRuns(httptest.NewRequest("GET", "http://127.0.0.1:1234/api/runs", nil))
	require.NoError(t, err)
	require.Len(t, runs, 3)
	require.Equal(t, "day2-run", runs[0].RunID)
