...
type portalRunStep struct {
	...
	Drift           *day2.DriftResult          `json:"drift,omitempty"`
}

func day2PortalRun(run day2.RunStatus) portalRun {
	result := portalRun{
...
func readStatusObject(r *http.Request, store day2state.State, key string) (*statestore.Status, error) {
...
func (p *portalServer) bootstrapStatuses(r *http.Request) ([]statestore.Status, error) {
	...
	add := func(store day2state.State, key string) error {
...
func (p *portalServer) listRuns(r *http.Request) ([]portalRun, error) {
	...
	for _, key := range keys {
		...
		byID[run.RunID] = day2PortalRun(run)
	}
