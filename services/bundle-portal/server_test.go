...
func testPortal(t *testing.T) (*portalServer, string) {
	...
	writeStateObject(t, dir, airgap.RunnerHeartbeatKey, airgap.RunnerHeartbeat{
		RunnerID: "airgap-dep-1", SessionID: "session-1", Version: "v1", BundleDigest: "sha256:bundle",
		Capabilities: []string{airgap.RunnerCapabilityCandidateArtifactPlans}, StartedAt: now.Add(-time.Minute), ObservedAt: now,
	})
	writeStateObject(t, dir, day2.RunStatusKey("run-1"), day2.RunStatus{RunID: "run-1", RefID: "restart-api", StartedAt: now})
	writeStateObject(t, dir, day2.BundleKey, day2.BundleInfo{
		SchemaVersion: day2.SchemaVersion,
		DeploymentID:  "dep-1",
		BundleDigest:  "sha256:bundle",
		ActivatedAt:   now,
		Target:        &day2.BundleTarget{OS: "linux", Architecture: "arm64"},
		Verification:  day2.BundleVerification{BlobsVerified: true, EnvelopeParsed: true},
		TotalSize:     300,
		Contents: []day2.BundleContent{
			{Kind: day2.BundleContentKindComponent, Name: "api", Detail: "helm_chart", Digest: "sha256:a", Size: 100},
			{Kind: day2.BundleContentKindImage, Name: "nginx", Detail: "docker.io/nginx", Digest: "sha256:i", Size: 200},
		},
	})
	writeStateObject(t, dir, day2.BundleHistoryKey("sha256:bundle"), day2.BundleInfo{
		SchemaVersion: day2.SchemaVersion, DeploymentID: "dep-1", BundleDigest: "sha256:bundle", ActivatedAt: now,
		Contents: []day2.BundleContent{{Kind: day2.BundleContentKindComponent, Name: "api", Digest: "sha256:a", ConfigDigest: "sha256:config-a"}},
	})
	writeStateObject(t, dir, day2.BundleHistoryKey("sha256:old"), day2.BundleInfo{
		SchemaVersion: day2.SchemaVersion, DeploymentID: "dep-1", BundleDigest: "sha256:old", ActivatedAt: now.Add(-time.Hour),
		Contents: []day2.BundleContent{{Kind: day2.BundleContentKindComponent, Name: "api", Digest: "sha256:old-a", ConfigDigest: "sha256:old-config-a"}},
	})
	p, err := newPortalServer(day2state.NewLocal(dir), nil, "secret", "operator", map[string]bool{"127.0.0.1:1234": true}, zaptest.NewLogger(t))
	require.NoError(t, err)
	return p, dir
}
...
func TestBundleHistoryIncludesInventoryComparisons(t *testing.T) {
	...
	require.Equal(t, []day2.BundleChange{{
		Kind: day2.BundleContentKindComponent, Name: "api", Change: day2.BundleChangeChanged,
...
func TestUploadBundleCandidateStreamsArchiveToStager(t *testing.T) {
	...
	p.stageBundle = func(_ context.Context, path, name string, progress func(bundleupgrade.Progress)) (*bundleupgrade.Result, error) {
		...
		return &bundleupgrade.Result{Candidate: day2.BundleCandidate{Bundle: day2.BundleInfo{BundleDigest: "sha256:next"}}}, nil
	}
...
func TestPlanBundleCandidateStackPersistsOnlyAfterRequest(t *testing.T) {
	p, dir := testPortal(t)
	activeBefore, found, err := p.store.Get(context.Background(), day2.BundleKey)
	require.NoError(t, err)
	require.True(t, found)
	digest := "sha256:0123456789abcdef"
	writeStateObject(t, dir, day2.StagedCandidateKey, day2.BundleCandidate{
		SchemaVersion: 1, PreviousDigest: "sha256:bundle", StagedAt: time.Now().UTC(),
		Bundle: day2.BundleInfo{BundleDigest: digest},
	...
	_, found, err = p.store.Get(context.Background(), day2.StackCandidateKey)
	require.NoError(t, err)
	...
	require.Eventually(t, func() bool {
		raw, found, err := p.store.Get(context.Background(), day2.StackCandidateKey)
		if err != nil || !found {
			return false
		}
		var planned day2.StackCandidate
	...
	require.Eventually(t, func() bool {
		...
		for _, run := range runs {
			if run.RefKind == "bundle-plan" && run.BundleDigest == digest {
				return run.Status == day2.RunStatusFinished && run.FinishedAt != nil && len(run.Steps) == 1
			}
		}
		return false
	}, time.Second, 10*time.Millisecond)
	activeAfter, found, err := p.store.Get(context.Background(), day2.BundleKey)
	require.NoError(t, err)
	require.True(t, found)
...
func TestPlanBundleCandidateStackDispatchesCompleteCandidatePlanRun(t *testing.T) {
	...
	recordKey := day2.CandidateStageKey(digest, time.Now().UTC())
	candidate := day2.BundleCandidate{
		SchemaVersion: 1, PreviousDigest: "sha256:bundle", StagedAt: time.Now().UTC(),
		Bundle: day2.BundleInfo{DeploymentID: "dep-1", BundleDigest: digest},
		Changes: []day2.BundleChange{
			{Kind: day2.BundleContentKindSandbox, Name: "terraform", Change: day2.BundleChangeChanged, PlanStepID: "sandbox-plan"},
			{Kind: day2.BundleContentKindComponent, Name: "api", Change: day2.BundleChangeChanged, PlanStepID: "deploy-api-plan"},
		},
		Deployment: &day2.BundleDeploymentAssets{
	...
	require.Eventually(t, func() bool {
		keys, err := p.store.List(context.Background(), day2.RequestsPrefix)
		...
		var planRequest day2.Request
		if json.Unmarshal(raw, &planRequest) != nil {
			return false
		}
		if planRequest.RefKind != day2.RefKindBundlePlan || planRequest.RefID != recordKey || planRequest.RunID == "" || planRequest.CandidateRecordKey != recordKey {
			return false
		}
		require.Equal(t, []string{"sandbox-plan", "deploy-api-plan"}, planRequest.PlanStepIDs)
		runRaw, found, err := p.store.Get(context.Background(), day2.RunStatusKey(planRequest.RunID))
		if err != nil || !found {
			return false
		}
		var run day2.RunStatus
		return json.Unmarshal(runRaw, &run) == nil && run.RefID == recordKey && run.Status == day2.RunStatusInProgress && len(run.Steps) == 3 && run.Steps[0].Status == day2.RunStatusFinished
	}, time.Second, 10*time.Millisecond)
}
...
func TestBundleUsesAppendOnlyStageWhenMutablePointerIsStale(t *testing.T) {
	...
	writeStateObject(t, dir, day2.StagedCandidateKey, day2.BundleCandidate{
		Bundle: day2.BundleInfo{BundleDigest: "sha256:stale"}, StagedAt: stagedAt.Add(-time.Minute),
	})
	writeStateObject(t, dir, day2.CandidateStageKey("sha256:new", stagedAt), day2.BundleCandidate{
		Bundle: day2.BundleInfo{BundleDigest: "sha256:new"}, StagedAt: stagedAt,
	})
	...
	require.Equal(t, day2.CandidateStageKey("sha256:new", stagedAt), recordKey)
}
...
func TestPortalRunnerHeartbeatUnknownWhenAbsent(t *testing.T) {
	p, err := newPortalServer(day2state.NewLocal(t.TempDir()), nil, "secret", "operator", map[string]bool{"127.0.0.1:1234": true}, zaptest.NewLogger(t))
	require.NoError(t, err)
...
func TestPortalRejectsCandidatePlansFromIncompatibleRunner(t *testing.T) {
	p, dir := testPortal(t)
	writeStateObject(t, dir, airgap.RunnerHeartbeatKey, airgap.RunnerHeartbeat{RunnerID: "old-runner"})

	err := p.requireRunnerCapability(context.Background(), airgap.RunnerCapabilityCandidateArtifactPlans)
...
func TestPortalPlanEndpoint(t *testing.T) {
	...
	path := filepath.Join(dir, filepath.FromSlash(day2.JobPlanKey("run-1--drift-1")))
	require.NoError(t, os.MkdirAll(filepath.Dir(path), 0o700))
...
func TestPortalBundleEndpoint(t *testing.T) {
	p, dir := testPortal(t)
	writeStateObject(t, dir, day2.CandidateKey, day2.BundleCandidate{
		SchemaVersion: day2.SchemaVersion, PreviousDigest: "sha256:bundle",
		Bundle: day2.BundleInfo{SchemaVersion: day2.SchemaVersion, DeploymentID: "dep-1", BundleDigest: "sha256:next"},
	})
	request := httptest.NewRequest(http.MethodGet, "http://127.0.0.1:1234/api/bundle", nil)
	response := httptest.NewRecorder()
	p.handler().ServeHTTP(response, request)
	...
	var payload struct {
		Active    *day2.BundleInfo      `json:"active"`
		Candidate *day2.BundleCandidate `json:"candidate"`
		History   []day2.BundleInfo     `json:"history"`
	}
	require.NoError(t, json.Unmarshal(response.Body.Bytes(), &payload))
...
func TestPortalBundleEndpointShowsStagedCandidateAndStackChanges(t *testing.T) {
	p, dir := testPortal(t)
	writeStateObject(t, dir, day2.CandidateKey, day2.BundleCandidate{
		SchemaVersion: day2.SchemaVersion, PreviousDigest: "sha256:old",
		Bundle: day2.BundleInfo{SchemaVersion: day2.SchemaVersion, DeploymentID: "dep-1", BundleDigest: "sha256:bundle"},
	})
	writeStateObject(t, dir, day2.StagedCandidateKey, day2.BundleCandidate{
		SchemaVersion: day2.SchemaVersion, PreviousDigest: "sha256:bundle",
		Bundle: day2.BundleInfo{SchemaVersion: day2.SchemaVersion, DeploymentID: "dep-1", BundleDigest: "sha256:next"},
	})
	writeStateObject(t, dir, day2.StackCandidateKey, day2.StackCandidate{
		SchemaVersion: day2.SchemaVersion, BundleDigest: "sha256:next", StackName: "install-stack", ChangeSetName: "candidate-next",
		Status: "CREATE_COMPLETE", ExecutionStatus: "AVAILABLE",
		Changes: []day2.StackChange{{Action: "Modify", LogicalResourceID: "Runner", ResourceType: "AWS::AutoScaling::LaunchConfiguration", Replacement: "True"}},
	})
	request := httptest.NewRequest(http.MethodGet, "http://127.0.0.1:1234/api/bundle", nil)
	response := httptest.NewRecorder()
	p.handler().ServeHTTP(response, request)
	...
	var payload struct {
		...
		Candidate          *day2.BundleCandidate `json:"candidate"`
		StackCandidate     *day2.StackCandidate  `json:"stack_candidate"`
	}
	require.NoError(t, json.Unmarshal(response.Body.Bytes(), &payload))
	require.Equal(t, day2.StagedCandidateKey, payload.CandidateRecordKey)
	require.Equal(t, "sha256:next", payload.Candidate.Bundle.BundleDigest)
...
func TestPortalClearsBundleCandidateWithoutDeletingHistory(t *testing.T) {
	...
	candidate := day2.BundleCandidate{
		...
		StagedAt:       stagedAt,
		Bundle:         day2.BundleInfo{SchemaVersion: day2.SchemaVersion, DeploymentID: "dep-1", BundleDigest: digest},
	}
	stageKey := day2.CandidateStageKey(digest, stagedAt)
	writeStateObject(t, dir, day2.StagedCandidateKey, candidate)
	...
	require.NoError(t, err)
	_, err = os.Stat(filepath.Join(dir, filepath.FromSlash(day2.StagedCandidateKey)))
	require.NoError(t, err)

	keys, err := p.store.List(context.Background(), day2.CandidateStagesPrefix)
	require.NoError(t, err)
	var dismissal day2.BundleCandidateDismissal
	dismissalCount := 0
	...
	writeStateObject(t, dir, day2.CandidateStageKey(digest, restaged.StagedAt), restaged)
	request = httptest.NewRequest(http.MethodGet, "http://127.0.0.1:1234/api/bundle", nil)
	response = httptest.NewRecorder()
	p.handler().ServeHTTP(response, request)
	require.Equal(t, http.StatusOK, response.Code)
...
func TestPortalRejectsClearingStaleBundleCandidate(t *testing.T) {
	...
	writeStateObject(t, dir, day2.StagedCandidateKey, day2.BundleCandidate{
		SchemaVersion: day2.SchemaVersion,
		StagedAt:      time.Now().UTC(),
		Bundle:        day2.BundleInfo{BundleDigest: "sha256:next"},
	})
...
func TestPortalApprovesMatchingPlannedBundleCandidate(t *testing.T) {
	p, dir := testPortal(t)
	candidate := day2.BundleCandidate{
		SchemaVersion: day2.SchemaVersion, PreviousDigest: "sha256:bundle", StagedAt: time.Now().UTC(),
		Bundle:  day2.BundleInfo{SchemaVersion: day2.SchemaVersion, DeploymentID: "dep-1", BundleDigest: "sha256:next"},
		Changes: []day2.BundleChange{{Kind: day2.BundleContentKindComponent, Name: "api", Change: day2.BundleChangeChanged, PlanStepID: "deploy-api-plan", ApplyStepID: "deploy-api-apply"}},
	}
	writeStateObject(t, dir, day2.CandidateKey, candidate)
	...
	p.handler().ServeHTTP(response, request)
	require.Equal(t, http.StatusOK, response.Code, response.Body.String())
	_, err := os.Stat(filepath.Join(dir, filepath.FromSlash(day2.CandidateApprovalKey("sha256:next"))))
	require.NoError(t, err)
}
...
func TestPortalApprovesTerraformSandboxPlanSeparately(t *testing.T) {
	...
	writeStateObject(t, dir, day2.CandidateKey, day2.BundleCandidate{
		SchemaVersion: day2.SchemaVersion, PreviousDigest: "sha256:bundle",
		Bundle:  day2.BundleInfo{SchemaVersion: day2.SchemaVersion, DeploymentID: "dep-1", BundleDigest: "sha256:next"},
		Changes: []day2.BundleChange{{Kind: day2.BundleContentKindSandbox, Name: "terraform", Change: day2.BundleChangeChanged, PlanStepID: "sandbox-plan", ApplyStepID: "sandbox-apply"}},
	})
	writeStateObject(t, dir, "status.json", statestore.Status{
	...
	p.handler().ServeHTTP(response, request)
	require.Equal(t, http.StatusOK, response.Code, response.Body.String())
	_, err := os.Stat(filepath.Join(dir, filepath.FromSlash(day2.CandidateSandboxApprovalKey("sha256:next"))))
	require.NoError(t, err)
}
...
func TestPortalRejectsStaleOrUnplannedBundleCandidateApproval(t *testing.T) {
	...
	writeStateObject(t, dir, day2.CandidateKey, day2.BundleCandidate{
		SchemaVersion: day2.SchemaVersion, PreviousDigest: "sha256:bundle",
		Bundle:  day2.BundleInfo{SchemaVersion: day2.SchemaVersion, DeploymentID: "dep-1", BundleDigest: "sha256:next"},
		Changes: []day2.BundleChange{{Kind: day2.BundleContentKindComponent, Name: "api", Change: day2.BundleChangeChanged, PlanStepID: "deploy-api-plan"}},
	})
...
func TestPortalReportAndStackOutputs(t *testing.T) {
	...
	p, err := newPortalServer(day2state.NewLocal(dir), day2state.NewLocal(stackDir), "secret", "operator", map[string]bool{"127.0.0.1:1234": true}, zaptest.NewLogger(t))
	require.NoError(t, err)
	h := p.handler()
...
func TestPortalNamespacedStateOwnership(t *testing.T) {
	...
	base := day2state.NewLocal(t.TempDir())
	runner := day2state.WithPrefix(base, day2state.RunnerNamespace)
	control := day2state.WithPrefix(base, day2state.ControlNamespace)
	read := day2state.ReadOverlay(runner, control, day2state.Legacy(base))
	catalog, err := json.Marshal(day2.Catalog{
		SchemaVersion: day2.SchemaVersion,
		DeploymentID:  "dep-1",
		BundleDigest:  "sha256:bundle",
		Refs:          []day2.CatalogRef{{ID: "restart-api", Kind: day2.RefKindAction, Name: "Restart API"}},
	})
	require.NoError(t, err)
	require.NoError(t, runner.Put(ctx, day2.CatalogKey, catalog))
	...
	controlKeys, err := control.List(ctx, day2.RequestsPrefix)
	require.NoError(t, err)
	require.Len(t, controlKeys, 1)
	runnerKeys, err := runner.List(ctx, day2.RequestsPrefix)
	require.NoError(t, err)
