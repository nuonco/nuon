...
type portalServer struct {
	...
	controlStore            day2state.State
	stackStore              day2state.State
	csrf                    string
	requestedBy             string
...
func newPortalServer(store, stackStore day2state.State, csrf, requestedBy string, hosts map[string]bool, logger *zap.Logger) (*portalServer, error) {
	...
	return &portalServer{store: store, controlStore: store, stackStore: stackStore, csrf: csrf, requestedBy: requestedBy, hosts: hosts, assets: assets, index: index, logger: logger}, nil
}
...
func (p *portalServer) clearBundleCandidate(w http.ResponseWriter, r *http.Request) {
	...
	dismissal := day2.BundleCandidateDismissal{
	...
	if err := p.controlStore.PutIfAbsent(r.Context(), day2.CandidateDismissalKey(input.BundleDigest, dismissedAt), raw); err != nil {
...
type bundleHistoryComparison struct {
	...
	Changes        []day2.BundleChange `json:"changes"`
}

func (p *portalServer) bundle(w http.ResponseWriter, r *http.Request) {
	active, ok, err := p.store.Get(r.Context(), day2.BundleKey)
	if err != nil {
		writeAPIError(w, err, http.StatusInternalServerError)
		return
	}
	...
	keys, err := p.store.List(r.Context(), day2.BundlesPrefix)
	if err != nil {
		writeAPIError(w, err, http.StatusInternalServerError)
		return
	}
	history := make([]day2.BundleInfo, 0)
	...
	for i := 0; i+1 < len(history); i++ {
		...
		if comparison.Available {
			comparison.Changes = day2.CompareBundleContents(history[i+1], history[i])
		}
	...
	stackCandidate, stackCandidateFound, err := p.store.Get(r.Context(), day2.StackCandidateKey)
	if err != nil {
		writeAPIError(w, err, http.StatusInternalServerError)
		return
	}
	if !stackCandidateFound {
		...
		var decoded day2.StackCandidate
...
func (p *portalServer) approveBundleCandidate(w http.ResponseWriter, r *http.Request) {
	...
	raw, found, err := p.store.Get(r.Context(), day2.CandidateKey)
	if err != nil {
		writeAPIError(w, err, http.StatusInternalServerError)
		return
	}
	...
	activeRaw, found, err := p.store.Get(r.Context(), day2.BundleKey)
	if err != nil || !found {
	...
	if status.BundleDigest != candidate.Bundle.BundleDigest || !status.ApprovalRequired {
		writeAPIError(w, fmt.Errorf("candidate is not awaiting approval"), http.StatusConflict)
		return
	}
	...
	sandboxPlanFound := false
	for _, change := range candidate.Changes {
		if status.ApprovalPhase == "sandbox" {
			if change.Kind != day2.BundleContentKindSandbox || change.Change == day2.BundleChangeUnchanged {
				continue
			}
			if change.Change != day2.BundleChangeChanged || change.Name != "terraform" || change.PlanStepID == "" || stepStatuses[change.PlanStepID] != "finished" {
				writeAPIError(w, fmt.Errorf("terraform sandbox plan %q has not finished", change.PlanStepID), http.StatusConflict)
				return
			}
			sandboxPlanFound = true
			continue
		}
		if change.Kind != day2.BundleContentKindComponent || change.Change == day2.BundleChangeUnchanged {
			continue
		}
		if change.Change == day2.BundleChangeRemoved {
			writeAPIError(w, fmt.Errorf("component removals are not supported"), http.StatusConflict)
			return
		}
		if change.PlanStepID == "" || stepStatuses[change.PlanStepID] != "finished" {
			writeAPIError(w, fmt.Errorf("component plan %q has not finished", change.PlanStepID), http.StatusConflict)
			return
		}
	}
	if status.ApprovalPhase == "sandbox" && !sandboxPlanFound {
		writeAPIError(w, fmt.Errorf("terraform sandbox plan not found"), http.StatusConflict)
		return
	}
	approvalKey := day2.CandidateApprovalKey(candidate.Bundle.BundleDigest)
	if status.ApprovalPhase == "sandbox" {
		approvalKey = day2.CandidateSandboxApprovalKey(candidate.Bundle.BundleDigest)
	}
	approval := []byte(fmt.Sprintf(`{"approved_at":%q,"approved_by":%q}`, time.Now().UTC().Format(time.RFC3339Nano), p.requestedBy))
	if err := p.controlStore.PutIfAbsent(r.Context(), approvalKey, approval); err != nil && !errors.Is(err, day2state.ErrObjectExists) {
		writeAPIError(w, err, http.StatusInternalServerError)
		return
	}
	writeJSON(w, map[string]string{"bundle_digest": candidate.Bundle.BundleDigest, "status": "approved", "phase": status.ApprovalPhase})
}
...
func (p *portalServer) planBundleCandidateStack(w http.ResponseWriter, r *http.Request) {
	...
	if err := p.requireRunnerCapability(r.Context(), airgap.RunnerCapabilityCandidateArtifactPlans); err != nil {
	...
	if raw, ok, err := p.store.Get(r.Context(), day2.StackCandidateKey); err != nil {
		writeAPIError(w, err, http.StatusInternalServerError)
		return
	} else if ok {
		var existing day2.StackCandidate
	...
	steps := []day2.RunStep{{ID: "install-stack-plan", Name: "Plan install stack", Kind: "cloudformation-plan", Status: day2.RunStatusInProgress, StartedAt: &startedAt}}
	for _, id := range planStepIDs {
		name, kind := candidatePlanStep(candidate, id)
		steps = append(steps, day2.RunStep{ID: id, Name: name, Kind: kind, Status: "available"})
	}
	planRun := day2.RunStatus{
		RunID: "bundle-plan-" + ulid.Make().String(), RefID: candidateRecordKey, RefKind: "bundle-plan",
		RefName: "Bundle deployment plan", Source: day2.SourcePortal, Status: day2.RunStatusInProgress,
	...
	if err := p.controlStore.Put(r.Context(), day2.RunStatusKey(planRun.RunID), planRunRaw); err != nil {
		writeAPIError(w, err, http.StatusInternalServerError)
		return
	}
	if err := p.controlStore.Put(r.Context(), day2.StackCandidateKey, raw); err != nil {
		finishedAt := time.Now().UTC()
		planRun.Status, planRun.Error, planRun.FinishedAt = day2.RunStatusFailed, err.Error(), &finishedAt
		planRun.Steps[0].Status, planRun.Steps[0].Error, planRun.Steps[0].FinishedAt = day2.RunStatusFailed, err.Error(), &finishedAt
		if failedRaw, marshalErr := json.Marshal(planRun); marshalErr == nil {
			_ = p.controlStore.Put(r.Context(), day2.RunStatusKey(planRun.RunID), failedRaw)
		}
		writeAPIError(w, err, http.StatusInternalServerError)
		return
	}
...
func (p *portalServer) requireRunnerCapability(ctx context.Context, capability string) error {
	raw, found, err := p.store.Get(ctx, airgap.RunnerHeartbeatKey)
	if err != nil {
	...
	var heartbeat airgap.RunnerHeartbeat
...
func candidatePlanStepIDs(candidate day2.BundleCandidate) ([]string, error) {
	ids := make([]string, 0)
	for _, change := range candidate.Changes {
		switch {
		case change.Kind == day2.BundleContentKindSandbox && change.Change != day2.BundleChangeUnchanged:
			if change.Change != day2.BundleChangeChanged || change.PlanStepID == "" {
		...
		case change.Kind == day2.BundleContentKindComponent && change.Change == day2.BundleChangeRemoved:
			return nil, fmt.Errorf("component removal for %q cannot be planned", change.Name)
		case change.Kind == day2.BundleContentKindComponent && change.Change != day2.BundleChangeUnchanged:
...
func candidatePlanStep(candidate day2.BundleCandidate, id string) (string, string) {
	for _, change := range candidate.Changes {
		if change.PlanStepID != id {
			continue
		}
		if change.Kind == day2.BundleContentKindSandbox {
...
func (p *portalServer) generateStackPlan(candidate day2.BundleCandidate, candidateRecordKey string, template []byte, pending day2.StackCandidate, planRun day2.RunStatus, planStepIDs []string) {
	...
	if err != nil {
		...
		planned = &pending
		planRun.Status = day2.RunStatusFailed
		planRun.FinishedAt = &finishedAt
		planRun.Error = err.Error()
		planRun.Steps[0].Status = day2.RunStatusFailed
		planRun.Steps[0].Error = err.Error()
		for i := 1; i < len(planRun.Steps); i++ {
			planRun.Steps[i].Status = day2.StepStatusDiscarded
		}
		...
		planRun.Steps[0].Status = day2.RunStatusFinished
		if len(planStepIDs) == 0 {
			planRun.Status = day2.RunStatusFinished
			planRun.FinishedAt = &finishedAt
		}
	}
	...
	if err := p.controlStore.Put(persistCtx, day2.StackCandidateKey, raw); err != nil {
		p.logger.Error("persist install stack plan", zap.Error(err))
	}
	if err == nil {
		if resultErr := p.controlStore.Put(persistCtx, day2.RunStepResultKey(planRun.RunID, "install-stack-plan"), raw); resultErr != nil {
	...
	if err := p.controlStore.Put(persistCtx, day2.RunStatusKey(planRun.RunID), planRunRaw); err != nil {
	...
	request := day2.Request{
		SchemaVersion: day2.SchemaVersion, DeploymentID: candidate.Bundle.DeploymentID,
		BundleDigest: candidate.Bundle.BundleDigest, RefID: candidateRecordKey,
		RefKind: day2.RefKindBundlePlan, RunID: planRun.RunID,
		CandidateArchiveKey: candidate.Deployment.CandidateBundleKey, CandidateRecordKey: candidateRecordKey,
		PlanStepIDs: planStepIDs, DispatchID: planRun.RunID, Source: day2.SourcePortal,
	...
	if putErr := p.controlStore.PutIfAbsent(persistCtx, day2.RequestKey(request.DispatchID), requestRaw); putErr != nil && !errors.Is(putErr, day2state.ErrObjectExists) {
		p.failBundlePlanRun(planRun, putErr)
	}
}

func (p *portalServer) failBundlePlanRun(run day2.RunStatus, err error) {
	finishedAt := time.Now().UTC()
	run.Status, run.Error, run.FinishedAt = day2.RunStatusFailed, err.Error(), &finishedAt
	...
	if marshalErr == nil {
		_ = p.controlStore.Put(context.Background(), day2.RunStatusKey(run.RunID), raw)
	}
	p.logger.Error("dispatch candidate bundle plan", zap.Error(err))
}

func (p *portalServer) latestBundleCandidate(ctx context.Context) (day2.BundleCandidate, bool, error) {
	candidate, _, found, err := p.latestBundleCandidateRecord(ctx)
	return candidate, found, err
}

func (p *portalServer) latestBundleCandidateRecord(ctx context.Context) (day2.BundleCandidate, string, bool, error) {
	var latest day2.BundleCandidate
	latestDismissals := make(map[string]time.Time)
	latestKey := ""
	found := false
	activeDigest := ""
	if raw, ok, err := p.store.Get(ctx, day2.BundleKey); err != nil {
		return day2.BundleCandidate{}, "", false, err
	} else if ok {
		var active day2.BundleInfo
		if err := json.Unmarshal(raw, &active); err != nil {
			return day2.BundleCandidate{}, "", false, fmt.Errorf("decode active bundle: %w", err)
		}
		activeDigest = active.BundleDigest
	}
	keys := []string{day2.CandidateKey, day2.StagedCandidateKey}
	stageKeys, err := p.store.List(ctx, day2.CandidateStagesPrefix)
	if err != nil {
		return day2.BundleCandidate{}, "", false, err
	}
	for _, key := range stageKeys {
		...
		if strings.Contains(key, "/dismissed/") && strings.HasSuffix(key, ".json") {
			...
			if err := json.Unmarshal(raw, &dismissal); err != nil {
				return day2.BundleCandidate{}, "", false, fmt.Errorf("decode %s: %w", key, err)
			}
	...
	if found {
		if dismissedAt, dismissed := latestDismissals[latest.Bundle.BundleDigest]; dismissed && !dismissedAt.Before(latest.StagedAt) {
			return day2.BundleCandidate{}, "", false, nil
		}
	}
...
func (p *portalServer) runnerHeartbeat(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Cache-Control", "no-store")
	raw, ok, err := p.store.Get(r.Context(), airgap.RunnerHeartbeatKey)
	if err == nil && !ok {
		raw, ok, err = p.store.Get(r.Context(), airgap.LegacyRunnerHeartbeatKey)
	}
	if err != nil {
		writeAPIError(w, err, http.StatusInternalServerError)
		return
...
func (p *portalServer) runControl(w http.ResponseWriter, r *http.Request) {
	...
	if err := p.controlStore.PutIfAbsent(r.Context(), statestore.InstallControlKey(id, input.Action), raw); err != nil {
		if errors.Is(err, day2state.ErrObjectExists) {
			writeJSON(w, request)
			return
		}
		writeAPIError(w, err, http.StatusInternalServerError)
...
func (p *portalServer) plan(w http.ResponseWriter, r *http.Request) {
	...
	p.object(day2.JobPlanKey(id))(w, r)
}

// stepPlan serves the late-bound rendered plan the airgapped runner persisted
...
func (p *portalServer) dispatch(w http.ResponseWriter, r *http.Request) {
	...
	if err := p.controlStore.PutIfAbsent(r.Context(), day2.RequestKey(id), raw); err != nil {
		writeAPIError(w, err, http.StatusInternalServerError)
		return
	}
