...
func TestConfigConnectedMode(t *testing.T) {
	connected := config{controlPlaneURL: "https://api.example.com", orgID: "org-1", installID: "ins-1", apiToken: "standard-token"}
	require.NoError(t, connected.validate())
	require.True(t, connected.connected())
	require.EqualError(t, (config{controlPlaneURL: "https://api.example.com"}).validate(), "connected mode requires --control-plane-url, --org-id, --install-id, and --api-token together")
	connected.localState = "/tmp/state"
	require.EqualError(t, connected.validate(), "connected mode flags cannot be combined with offline state or AWS bundle flags")
}
