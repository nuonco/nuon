...
const ManifestLineDiff = ({
  payloads,
  changedOnly = true,
}: {
  payloads: Array<{ line: string; type: string | number }>;
  changedOnly?: boolean;
}) => {
  const [showContext, setShowContext] = useState(false);
  const firstChangedLine = useRef<HTMLSpanElement>(null);
  const hiddenContext = changedOnly
    ? payloads.filter(
        (payload) =>
          payload.type !== 1 &&
          payload.type !== 2 &&
  ...
  return (
    <div className="manifest-line-diff-wrap">
      {hiddenContext > 0 && (
        <button
          type="button"
          className="plan-diff-context"
          onClick={() => {
            const next = !showContext;
            setShowContext(next);
            if (next)
              requestAnimationFrame(() =>
                firstChangedLine.current?.scrollIntoView({ block: "center" }),
              );
          }}
        >
          {showContext
            ? "Hide unchanged lines"
            : `Show ${hiddenContext} unchanged ${hiddenContext === 1 ? "line" : "lines"}`}
        </button>
      )}
      <pre className="manifest-line-diff">
        {visible.map((payload, index) => {
          ...
          return (
            <span
              ref={index === firstChangedIndex ? firstChangedLine : undefined}
              key={index}
              className={removed ? "removed" : added ? "added" : "context"}
            >
              <b>{removed ? "−" : added ? "+" : " "}</b>
              {payload.line}
              {"\n"}
...
const stepResultLabel = (kind: TStepResult["kind"]) =>
  kind === "terraform"
    ? "terraform plan computed by the customer-managed runner"
    : kind === "helm"
      ? "helm diff computed by the customer-managed runner"
      : kind === "kubernetes_manifest"
        ? "kubernetes manifest diff computed by the customer-managed runner"
        : "execution result recorded by the customer-managed runner";

const StepPlanPreview = ({
  step,
}: {
  step: TStatusStep & { job_id?: string };
}) => {
  ...
  return (
    <div>
      ...
      >
        ...
        <span style={{ fontSize: 12, color: "var(--muted)" }}>
          {finished
            ? "late-bound plan rendered by the customer-managed runner"
            : "late-bound plan rendered by the customer-managed runner — the real terraform plan / diff appears once the step’s plan phase completes"}
        </span>
        <a
          className="link"
          style={{ marginLeft: "auto", fontSize: 12 }}
    ...
    {changes.length === 0 ? (
      ...
      <div className="candidate-plan-result-list">
        {changes.map((change) => {
          ...
          return (
            <article
              className="candidate-plan-result"
              key={`${change.kind}-${change.name}`}
            >
              <header className="candidate-plan-result-header">
                <span>
                  <ContentKindIcon kind={change.kind} />
                  <strong>{change.name}</strong>
                  <small>
                    {change.detail || change.kind.replaceAll("-", " ")}
                  </small>
                </span>
              ...
              <div className="candidate-plan-result-body">
                {step?.job_id ? (
                  ...
                  <div className="notice">
                    Waiting for the candidate runner to start this plan.
                  </div>
                ) : (
                  <div className="notice">
                    No candidate deployment plan has run for this change.
                  </div>
                )}
              </div>
            </article>
    ...
    <main>
      ...
      <div className="tiles">
        <div className="tile">
          ...
          <div className="tile-note">reported by customer-managed runner</div>
        </div>
        <div className="tile">
          <div className="tile-top">
...
const Components = () => {
  ...
  return (
    <main>
      <PageHeader
        title="Components"
        subtitle="Deployed components, their live health, and the operation refs that operate them."
      />
      <section className="card">
        <State
          error={health.error}
          ...
          emptyText="No health reports yet — the customer-managed runner publishes them on its health interval"
        />
        {components.length > 0 && (
          <div className="table-wrap">
            <table>
...
const ComponentDetail = () => {
  ...
  return (
    <main>
      ...
      <PageHeader
        title={name}
        subtitle={`${component?.component_type ?? "component"} · reported by the customer-managed runner`}
      />
      ...
      <div className="tiles">
        ...
        <div className="tile">
          <div className="tile-top">
            <span className="tile-label">Operation refs</span>
            <Zap />
          </div>
        ...
        <div className="stack">
          ...
          <section className="card">
            ...
            <State
              ...
              emptyText="No operation runs reference this component yet"
            />
            <div className="activity">
    ...
    <main>
      ...
      <section className="card">
        ...
        <State
          ...
          emptyText="No sandbox steps found — the customer-managed runner records them during bootstrap"
        />
        {rows.length > 0 && (
          <div className="table-wrap">
            <table>
...
const StackPage = () => {
  ...
  return (
    <main>
      <PageHeader
        title="Stack"
        subtitle="Install stack readiness and the outputs the customer-managed runner late-binds into every plan."
      />
      ...
      <section className="card">
        ...
        {missing && (
          <div className="state">
            ...
            provisioning finishes; the customer-managed runner polls for the
            same document before it runs any plan.
          </div>
        )}
        {entries.length > 0 && (
          <div className="table-wrap">
...
const HistoryEntry = ({
  ...
  const groups = [
    ...
    },
    {
      ...
      changes: changes.filter(
        (change) => change.kind === "action" || change.kind === "runbook",
      ),
    },
    {
      label: "Packaging",
        ...
        <div className="activity-item">
          ...
          <span className="activity-body">
            ...
            <code>
              ...
              {comparison?.available && impactSummary
                ? ` · ${impactSummary}`
                : ""}
            </code>
          </span>
          {comparison && (
            ...
            >
              ...
              {comparison.available ? "View changes" : "diff unavailable"}
            </button>
          )}
      ...
      {!hideDetails && expanded && comparison?.available && (
        <div className="history-diff">
          ...
          <div className="history-change-summary">
            {counts.changed > 0 && (
              <Badge theme="info">{counts.changed} changed</Badge>
            )}
            {counts.added > 0 && (
                    ...
                    <table>
                      ...
                      <tbody>
                        {group.changes.map((change, index) => {
                          ...
                          const componentDiff =
                            change.kind === "component" &&
                            (change.previous_component_definition ||
                              change.candidate_component_definition);
                          const runbookDiff =
                            change.kind === "runbook" &&
                            (change.previous_runbook_definition ||
                              change.candidate_runbook_definition);
                            ...
                            <Fragment key={key}>
                              <tr>
                                ...
                                <td>
                                  ...
                                  runbookDiff ? (
                                    <button
                                      type="button"
                                      className="link history-plan-button"
                                      onClick={() =>
                                        setExpandedPlan(
                                          planExpanded ? null : key,
                                        )
                                      }
                                    >
                                      <ChevronRight
                                        className={
                                          planExpanded ? "expanded-chevron" : ""
                                        }
                                      />
                                      {planExpanded
                                        ? "Hide diff"
                              ...
                              {planExpanded &&
                                (step ||
                                  ...
                                  actionDiff ||
                                  runbookDiff) && (
                                  <tr className="candidate-plan-row">
                                    <td colSpan={4}>
                                      {actionDiff ? (
                                        <ActionDefinitionDiff
                                          before={
                                            change.previous_action_definition
                                          }
                                          after={
...
const StackChangesTable = ({
  candidate,
  heading = true,
}: {
  candidate: TStackCandidate;
  heading?: boolean;
}) => {
  ...
  const materialChanges = changes.filter(
    (change) => !dependencyReevaluations.includes(change),
  );
  const rows = (items: TStackChange[]) =>
    items.map((change, index) => {
      ...
      return (
        <Fragment key={key}>
          <tr>
            <td>
              <span className="strong">{change.logical_resource_id}</span>
            </td>
            <td>
              <code>{change.resource_type}</code>
            </td>
            <td>
              <Badge
                theme={
            ...
            <td>
              {change.details
                ?.map((detail) => detail.name || detail.attribute)
                .filter(Boolean)
                .join(", ") ||
                change.scope?.join(", ") ||
                "—"}
            ...
            <td>
              {propertyChanges.length > 0
                ? `${propertyChanges.length} value ${propertyChanges.length === 1 ? "change" : "changes"}`
                : "evaluation only"}
            </td>
          </tr>
          <tr className="stack-property-row">
            <td colSpan={6}>
              <StackResourceChanges change={change} />
            </td>
          </tr>
        </Fragment>
      );
  ...
  return (
    <div>
      ...
      {materialChanges.length > 0 ? (
        ...
        <div className="notice candidate-notice">
          No template property values change in this plan.
        </div>
      )}
      {dependencyReevaluations.length > 0 && (
        <details className="stack-reevaluations">
          <summary>
            <span>
              <strong>Dependency reevaluations</strong>
              <small>
                CloudFormation revisited these resources, but their template
                property values do not change.
              </small>
            </span>
            <span>
              <Badge>{dependencyReevaluations.length}</Badge>
              <ChevronRight />
            </span>
          </summary>
...
const StackResourceChanges = ({ change }: { change: TStackChange }) => (
  <div className="stack-resource-detail">
    {(change.property_changes ?? []).length > 0 ? (
      <StackPropertyChanges
        changes={change.property_changes ?? []}
        truncated={change.property_changes_truncated}
      />
    ) : (
    ...
    {(change.details ?? []).length > 0 && (
      <div className="stack-change-reasons">
        <div className="stack-change-reasons-heading">
          CloudFormation evaluation
        </div>
        {(change.details ?? []).map((detail, index) => (
          <div
            className="stack-change-reason"
            key={`${detail.attribute}-${detail.name}-${index}`}
          >
...
const StackPropertyChanges = ({
  changes,
  truncated,
}: {
  changes: TStackPropertyChange[];
  truncated?: boolean;
}) => (
  <div className="stack-property-diff">
    ...
    {changes.map((change) => (
      <section className="stack-property-change" key={change.path}>
        <code className="stack-property-name">
          {change.path.replace(/^Properties\./, "")}
        </code>
    ...
    {truncated && (
      <div className="notice">
        Additional property changes were omitted from the persisted comparison.
      </div>
    )}
  </div>
);

const ActionDefinitionDiff = ({
  before,
  after,
}: {
  before?: TBundleActionDefinition;
  after?: TBundleActionDefinition;
}) => {
  const beforeSteps = new Map(
    (before?.steps ?? []).map((step) => [step.name, step]),
  );
  const afterSteps = new Map(
    (after?.steps ?? []).map((step) => [step.name, step]),
  );
  const names = Array.from(
    new Set([
      ...Array.from(beforeSteps.keys()),
      ...Array.from(afterSteps.keys()),
    ]),
  ).filter(
    (name) =>
      JSON.stringify(beforeSteps.get(name)) !==
      JSON.stringify(afterSteps.get(name)),
  );
  const { steps: _beforeSteps, ...beforeSettings } = before ?? {};
  const { steps: _afterSteps, ...afterSettings } = after ?? {};
  const settingChanges = definitionPropertyChanges(
    beforeSettings,
    afterSettings,
  );
  return (
    <div className="action-definition-diff">
      <div className="history-stack-heading">
        ...
        <Badge mono>
          {settingChanges.length} settings · {names.length} steps changed
        </Badge>
      </div>
      {settingChanges.length > 0 && (
        <StackPropertyChanges changes={settingChanges} />
      )}
      {names.length > 0 ? (
        names.map((name) => (
          <ActionStepDiff
            key={name}
            name={name}
            before={beforeSteps.get(name)}
            after={afterSteps.get(name)}
          />
        ))
      ) : settingChanges.length === 0 ? (
        <div className="notice">
          The canonical action definition is unchanged.
        </div>
      ) : null}
    </div>
  );
...
const ActionStepDiff = ({
  name,
  before,
  after,
}: {
  name: string;
  before?: TBundleActionStep;
  after?: TBundleActionStep;
}) => {
  ...
  return (
    <section className="action-step-diff">
      ...
      {fields.length > 0 ? (
        ...
        <div className="notice">
          The persisted step definition is unchanged.
        </div>
      )}
    </section>
  );
...
const RunbookDefinitionDiff = ({
  before,
  after,
}: {
  before?: TBundleRunbookDefinition;
  after?: TBundleRunbookDefinition;
}) => {
  ...
  return (
    <div className="action-definition-diff">
      <div className="history-stack-heading">
        ...
        <Badge mono>
          {changes.length} changed {changes.length === 1 ? "field" : "fields"}
        </Badge>
      </div>
      {changes.length > 0 ? (
        ...
        <div className="notice">
          The canonical runbook definition is unchanged.
        </div>
      )}
    </div>
  );
  ...
  return (
    <div className="action-definition-diff">
      ...
      {!comparisonAvailable && payloads.length > 0 && (
        <div className="notice">
          The active bundle predates canonical component definitions. Showing
          the uploaded TOML snapshot; a line diff is unavailable for this
          upgrade.
        </div>
      )}
      {payloads.length > 0 ? (
        <ManifestLineDiff
          payloads={payloads}
          changedOnly={comparisonAvailable}
        />
      ) : (
        <div className="notice">
          The canonical component definition is unchanged.
        </div>
      )}
    </div>
  );
...
const tomlScalar = (value: unknown): string => {
  ...
  if (value && typeof value === "object") {
    return `{ ${Object.entries(value)
      ...
      .map(([key, child]) => `${tomlKey(key)} = ${tomlScalar(child)}`)
      .join(", ")} }`;
  }
  return "";
};

const componentDefinitionToml = (
  ...
  const writeTable = (value: Record<string, unknown>, path: string[]) => {
    ...
    const emptyTables = entries.filter(
      ([, child]) =>
        child &&
        typeof child === "object" &&
        !Array.isArray(child) &&
    ...
    [...scalars, ...emptyTables]
      ...
      .forEach(([key, child]) => {
        lines.push(
          `${tomlKey(key)} = ${child && typeof child === "object" && !Array.isArray(child) ? "{}" : tomlScalar(child)}`,
        );
      });
    tables
      .sort(([left], [right]) => left.localeCompare(right))
...
const definitionPropertyChanges = (
  before: unknown,
  after: unknown,
  path = "definition",
): TStackPropertyChange[] => {
...
const BundleChangeDescription = ({ change }: { change: TBundleChange }) => {
  ...
  const actionStepsUnchanged =
    ...
    change.previous_action_definition &&
    change.candidate_action_definition &&
    JSON.stringify(change.previous_action_definition) ===
      JSON.stringify(change.candidate_action_definition);
            ...
            : change.kind === "stack-asset"
              ...
              : change.kind === "runner-binary"
                ...
                : change.kind === "runner-image"
                  ? "Runner image"
                  : "Bundle content";
  const detail = actionStepsUnchanged
        ...
        {contentChanged && (
          <span>
            {actionStepsUnchanged
              ? "bundle metadata"
              : configDuplicatesContent
                ? "definition"
                : "content"}{" "}
            <DigestChange
              before={change.previous_digest}
              after={change.candidate_digest}
            />
          </span>
        )}
        {configChanged && !configDuplicatesContent && (
          <span>
            config{" "}
            <DigestChange
              before={change.previous_config_digest}
              after={change.candidate_config_digest}
            />
          </span>
        )}
      </span>
...
const Bundles = () => {
  ...
  const bundle = useQuery({
    ...
    refetchInterval: (query) => {
      ...
      if (data?.candidate && !data.stack_candidate?.runner_activated_at)
        return 5000;
      return false;
    },
  });
  ...
  const [openChangeGroups, setOpenChangeGroups] = useState<string[]>([
    "Components",
  ]);
  const [expandedChange, setExpandedChange] = useState<string | null>(null);
  const [expandedHistory, setExpandedHistory] = useState<string | null>(null);
  const [bundleView, setBundleView] = useState<"review" | "active" | "history">(
    "review",
  );
  ...
  const [bundleUploadProgress, setBundleUploadProgress] = useState({
    loaded: 0,
    total: 0,
  });
  ...
  const uploadBundle = useMutation({
    mutationFn: (file: File) =>
      uploadBundleCandidate(file, (loaded, total) =>
        setBundleUploadProgress({ loaded, total }),
      ),
    onMutate: () => {
  ...
  const bundleUploadStatus = useQuery({
    ...
    enabled:
      ...
      bundleUploadProgress.total > 0 &&
      bundleUploadProgress.loaded >= bundleUploadProgress.total,
    refetchInterval: 750,
  });
  ...
  const stackCandidate =
    candidateRecordKey &&
    stagedStack?.candidate_record_key === candidateRecordKey
      ? stagedStack
      : null;
  ...
  const stackPending =
    ...
    !stackPlanRunning &&
    !stackPlanFailed &&
    !stackCandidate.runner_activated_at;
  const history = bundle.data?.history ?? [];
  const candidatePlanRun = runs.data?.find(
    (run) =>
      run.ref_kind === "bundle-plan" && run.ref_id === candidateRecordKey,
  );
  ...
  const changeCounts = candidateChanges.reduce(
    (counts, change) => ({
      ...counts,
      [change.change]: counts[change.change] + 1,
    }),
    { changed: 0, added: 0, removed: 0, unchanged: 0 },
  );
    ...
    .reduce(
      (total, change) =>
        ...
        (candidateContents.find(
          (content) =>
            content.kind === change.kind && content.name === change.name,
        )?.size ?? 0),
      0,
    );
  ...
  const changeGroups = [
    {
      label: "Components",
      description: "Application workloads and configuration",
      kinds: ["component"],
    },
    {
      label: "Operations",
      description: "Actions and runbooks",
      kinds: ["action", "runbook"],
    },
    {
      label: "Infrastructure",
      description: "Sandbox and install stack",
      kinds: ["sandbox", "stack-asset"],
    },
    {
      label: "Runtime",
      description: "Runner and packaged artifacts",
      kinds: ["runner-image", "runner-binary", "image"],
    },
  ]
    .map((group) => ({
      ...group,
      changes: visibleChanges.filter((change) =>
        group.kinds.includes(change.kind),
      ),
    }))
    .filter((group) => group.changes.length > 0);
  const statusMatchesCandidate =
    !!candidate &&
    status.data?.bundle_digest === candidate.bundle.bundle_digest;
  ...
  const plansFinished = plannedChanges.every(
    (change) =>
      change.plan_step_id &&
      candidatePlanSteps.find((step) => step.id === change.plan_step_id)
        ?.status === "finished",
  );
    ...
    <section className="card bundle-history-card">
      <div className="card-header">
        <div>
          ...
          <p>
            Select an activation to review what changed from the bundle it
            replaced.
          </p>
        </div>
      </div>
    ...
    <main>
      <PageHeader
        ...
        actions={
          <button
            type="button"
            className="primary"
            onClick={() => setShowBundleUpload(true)}
          >
            <UploadCloud /> Upload bundle
          </button>
        }
      />
      ...
      {!bundle.isLoading && !bundle.error && !active && (
        <div className="notice" style={{ margin: "0 0 20px" }}>
          ...
          The customer-managed runner publishes the inventory when it activates
          a bundle; runners older than this portal never publish it.
        </div>
      )}

      <div
        className="bundle-workspace-tabs"
        role="tablist"
        aria-label="Bundle workspace"
      >
        <button
          type="button"
          className={bundleView === "review" ? "active" : ""}
          onClick={() => setBundleView("review")}
        >
          <Package /> Review upload{" "}
          {candidate && (
            <Badge theme="info">
              {changeCounts.changed + changeCounts.added + changeCounts.removed}
            </Badge>
          )}
        </button>
        <button
          type="button"
          className={bundleView === "active" ? "active" : ""}
          onClick={() => setBundleView("active")}
        >
          <ShieldCheck /> Active bundle
        </button>
        <button
          type="button"
          className={bundleView === "history" ? "active" : ""}
          onClick={() => setBundleView("history")}
        >
          <Clock /> History
        </button>
        {active && (
          <span className="active-bundle-summary">
            <Badge theme="success" mono>
              active
            </Badge>
            <code>{shortDigest(active.bundle_digest)}</code>
            <span>activated {formatTime(active.activated_at)}</span>
          </span>
        )}
      </div>
      ...
      {showBundleUpload && (
        <div
          className="bundle-upload-backdrop"
          role="presentation"
          onMouseDown={() =>
            !uploadBundle.isPending && setShowBundleUpload(false)
          }
        >
          <section
            className="card bundle-upload-dialog"
            role="dialog"
            aria-modal="true"
          ...
          >
            <div className="card-header">
              <div>
                <h3 id="bundle-upload-title">
                  <UploadCloud /> Upload bundle
                </h3>
                <p>
                  Compare a bundle archive with the active installation before
                  planning or deploying it.
                </p>
              </div>
              <button
                type="button"
                ...
                onClick={() => setShowBundleUpload(false)}
              >
                <XCircle />
              </button>
            </div>
            <label
              ...
              onDragEnter={(event) => {
                ...
                setDraggingBundle(true);
              }}
              onDragOver={(event) => event.preventDefault()}
              onDragLeave={() => setDraggingBundle(false)}
              onDrop={(event) => {
                event.preventDefault();
                setDraggingBundle(false);
                selectBundle(event.dataTransfer.files);
              }}
            >
              <input
                type="file"
                ...
                disabled={uploadBundle.isPending}
                onChange={(event) => selectBundle(event.target.files)}
              />
              {uploadBundle.isPending ? (
              ...
              <strong>
                {uploadBundle.isPending
                  ? bundleUploadProgress.total > 0 &&
                    bundleUploadProgress.loaded >= bundleUploadProgress.total
                    ? "Upload complete · Processing bundle"
                    : "Uploading bundle…"
                  : "Drop bundle here or choose a file"}
              </strong>
              <span>
                {uploadBundle.isPending && bundleUploadProgress.total > 0
                  ? bundleUploadProgress.loaded < bundleUploadProgress.total
                    ? `${Math.min(100, Math.round((bundleUploadProgress.loaded / bundleUploadProgress.total) * 100))}% · ${formatBytes(bundleUploadProgress.loaded)} of ${formatBytes(bundleUploadProgress.total)}`
                    : bundleUploadStatus.data?.detail ||
                      "Waiting for backend processing status…"
                  : "Compressed .tar.zst bundle, up to 5 GiB"}
              </span>
            </label>
            {uploadBundle.error && (
      ...
      {showClearCandidate && candidate && (
        <div
          ...
          role="presentation"
          onMouseDown={() =>
            !clearCandidate.isPending && setShowClearCandidate(false)
          }
        >
          <section
            className="card bundle-clear-dialog"
            role="dialog"
            aria-modal="true"
            aria-labelledby="bundle-clear-title"
            onMouseDown={(event) => event.stopPropagation()}
          >
            <div className="card-header">
              <div>
                <h3 id="bundle-clear-title">
                  <XCircle /> Clear staged bundle?
                </h3>
                <p>
                  This removes it from the review queue without deleting its
                  archive, plan run, or audit record.
                </p>
              </div>
            </div>
            ...
            {clearCandidate.error && (
              <div className="notice error">{clearCandidate.error.message}</div>
            )}
            <div className="bundle-clear-actions">
              <button
                type="button"
                className="secondary"
                disabled={clearCandidate.isPending}
                onClick={() => setShowClearCandidate(false)}
              >
                Cancel
              </button>
              <button
                ...
                disabled={clearCandidate.isPending}
                onClick={() =>
                  clearCandidate.mutate(candidate.bundle.bundle_digest)
                }
              >
                {clearCandidate.isPending ? (
                  <Loader2 className="spin" />
                ) : (
                  <XCircle />
                )}{" "}
      ...
      {bundleView === "review" && !candidate && !bundle.isLoading && (
        <section className="card bundle-empty-review">
          <Package />
          <h3>No bundle awaiting review</h3>
          <p>Upload a bundle to compare it with the active installation.</p>
          <button
            type="button"
            className="primary"
            onClick={() => setShowBundleUpload(true)}
        ...
        <>
          <div className="review-toolbar">
            <div
              ...
              role="tablist"
              aria-label="Upload review"
            >
              <button
                type="button"
                className={reviewView === "changes" ? "active" : ""}
              ...
              <button
                type="button"
                className={reviewView === "plan" ? "active" : ""}
                onClick={() => setReviewView("plan")}
              >
                Deployment plan{" "}
                <Badge
                  theme={
                    stackPlanFailed ? "error" : stackCandidate ? "info" : ""
                  }
                >
                  {stackPlanRunning
                    ? "running"
            ...
            <button
              type="button"
              className="secondary danger"
              onClick={() => setShowClearCandidate(true)}
            >
              <XCircle /> Clear staged bundle
            </button>
          </div>
            ...
            <section className="card candidate-card">
                ...
                <div>
                  ...
                  <p className="candidate-file">
                    {candidate.archive_name && (
                      <>
                        <FileArchive />
                        <strong>{candidate.archive_name}</strong>
                      </>
                    )}
                    {candidate.archive_size != null && (
                      <span>{formatBytes(candidate.archive_size)}</span>
                    )}
              ...
              <div className="candidate-digests">
                <div>
                  <span>Active</span>
                  <code>
                    {active?.bundle_digest ?? candidate.previous_digest}
                  </code>
                </div>
                <ChevronRight />
                <div>
              ...
              {(candidate.bundle.release || candidate.bundle.package) && (
                <dl className="candidate-identity">
                  {candidate.bundle.release && (
                    <div>
                      <dt>Release</dt>
                      <dd>
                        <code>{candidate.bundle.release.id}</code>
                        <span title={candidate.bundle.release.digest}>
                          {shortDigest(candidate.bundle.release.digest)}
                        </span>
                      </dd>
                    </div>
                  )}
                  {candidate.bundle.package && (
                    <div>
                      <dt>Package</dt>
                      <dd>
                        <code>{candidate.bundle.package.id}</code>
                        <span>
                          {candidate.bundle.package.format} ·{" "}
                          {candidate.bundle.package.target}
                        </span>
                      </dd>
                    </div>
                  )}
              ...
              <div className="candidate-summary">
                {(["changed", "added", "removed", "unchanged"] as const).map(
                  (change) => (
                    <div key={change}>
                      <strong>{changeCounts[change]}</strong>
                      <span>{change}</span>
                    </div>
                  ),
                )}
              ...
              {hasRemovals && (
                <div className="notice error candidate-notice">
                  Component removals are not supported. Remove the deleted
                  components manually or stage a candidate without removals
                  before deploying.
                </div>
              )}
              <div className="candidate-table-heading">
                ...
                <button
                  type="button"
                  className={`chip ${showUnchanged ? "on" : ""}`}
                ...
                {changeGroups.map((group) => (
                  ...
                  >
                    ...
                    >
                      ...
                      <span>
                        <Badge>{group.changes.length}</Badge>
                        <ChevronRight />
                      </span>
                    </summary>
                      ...
                      {group.changes.map((change, index) => {
                        ...
                        const runbookDiff =
                          ...
                          (change.previous_runbook_definition ||
                            change.candidate_runbook_definition);
                        const expandable =
                          !!componentDiff || !!actionDiff || !!runbookDiff;
                        const expanded = expandedChange === key;
                        return (
                          <Fragment key={key}>
                            <button
                              type="button"
                              className={`bundle-change-row ${change.change === "unchanged" ? "quiet-row" : ""}`}
                              disabled={!expandable}
                              onClick={() =>
                                expandable &&
                                setExpandedChange(expanded ? null : key)
                              }
                            >
                              <span className="bundle-change-name">
                                <ContentKindIcon kind={change.kind} />
                                <span>
                                  <strong>{change.name}</strong>
                                  {change.detail && (
                              ...
                              {expandable && (
                                <ChevronRight
                                  className={expanded ? "expanded-chevron" : ""}
                                />
                              )}
                            </button>
                            {expanded &&
                              (componentDiff || actionDiff || runbookDiff) && (
                                <div className="bundle-change-detail">
                                  {actionDiff ? (
                                    <ActionDefinitionDiff
                                      before={change.previous_action_definition}
                                      after={change.candidate_action_definition}
                                    />
                                  ) : runbookDiff ? (
                                    <RunbookDefinitionDiff
                                      before={
            ...
            <section className="card deployment-plan-card">
              <div>
                <div className="deployment-plan-heading">
                  <span>
                    <Layers /> <strong>Deployment plan</strong>
                    <small>
                      Run environment-specific plans only when you are ready.
                    </small>
                  </span>
                    ...
                    {stackCandidate && (
                      <Badge
                        theme={
                          stackPlanFailed
                            ? "error"
                            : stackCandidate.stack_applied_at
                              ? "success"
                              : "info"
                        }
                        mono
                ...
                <div className="deployment-plan-body">
                  {stackPending && (
                    <div className="stack-apply-callout">
                      <div className="stack-apply-callout-heading">
                        <span>
                          <Terminal />
                          <strong>Next step: Apply the install stack</strong>
                        </span>
                        <Badge mono>required</Badge>
                      </div>
                      <p>
                        The deployment plan is ready. The portal does not
                        execute this CloudFormation change set; run the command
                        below to apply it and activate the candidate runner.
                      </p>
                        ...
                        <button
                          ...
                          onClick={async () => {
                            await navigator.clipboard.writeText(
                              "nuon-bundle upgrade apply-stack --yes",
                            );
                            setApplyCommandCopied(true);
                            window.setTimeout(
                              () => setApplyCommandCopied(false),
                              2000,
                            );
                          }}
                        >
                          {applyCommandCopied ? <CheckCircle2 /> : <Copy />}{" "}
                          {applyCommandCopied ? "Copied" : "Copy"}
                        </button>
                      </div>
                      <small>
                        This page will detect activation automatically and then
                        show the bundle deployment approval.
                      </small>
                    </div>
                  )}
                  {candidatePlanRun && (
                    <Link
                      ...
                      to={`/runs?run=${encodeURIComponent(candidatePlanRun.run_id)}`}
                    >
                      <PlayCircle /> View plan run in Runs
                    </Link>
                  )}
                  {!stackCandidate || stackPlanFailed ? (
                    <button
                      ...
                      onClick={() =>
                        planStack.mutate(candidate.bundle.bundle_digest)
                      }
                    >
                      {planStack.isPending ? (
                        <Loader2 className="spin" />
                      ...
                      {planStack.isPending
                        ? "Starting plan…"
                        : stackPlanFailed
                          ? "Retry deployment plan"
                          : "Run deployment plan"}
                    </button>
                  ) : stackPlanRunning ? (
                    <div className="notice candidate-notice">
                      CloudFormation is evaluating the candidate template. You
                      can continue reviewing the bundle diff while this runs.
                    </div>
                  ) : (
                    <>
                      <div className="candidate-table-heading">
                        <div>
                          <h3>Install stack plan</h3>
                          <code>{stackCandidate.change_set_name}</code>
                        </div>
                      </div>
                      {stackCandidate.no_op ? (
                        <div className="notice candidate-notice">
                          The candidate does not change any CloudFormation
                          resources.
                        </div>
                      ) : (
                        <StackChangesTable
                  ...
                  {stackCandidate?.runner_activated_at &&
                    ...
                    !plansFinished && (
                      <div className="notice candidate-notice">
                        {approvalPhase === "sandbox"
                          ? "Deploy becomes available after the sandbox Terraform plan finishes."
                          : "Deploy becomes available after all changed and added component plans finish."}
                      </div>
                    )}
                  {stackCandidate?.runner_activated_at &&
                    !hasRemovals &&
                    ...
                    (!statusMatchesCandidate ||
                      status.data?.approval_required !== true) && (
                      <div className="notice candidate-notice">
                        Waiting for the runner to request approval.
                      </div>
                    )}
                  {stackCandidate && !stackPlanRunning && !stackPending && (
                    <button
                      ...
                      onClick={() =>
                        approve.mutate(candidate.bundle.bundle_digest)
                      }
                    >
                      {approve.isPending ? (
                        <Loader2 className="spin" />
                      ...
                      {approvalPhase === "sandbox"
                        ? "Approve & Deploy Sandbox"
                        : "Approve & Deploy Bundle"}
                    </button>
                  )}
                  {approve.error && (
                    <div className="notice error candidate-notice">
          ...
          <section className="card">
            <div className="card-header" style={{ display: "block" }}>
              ...
              <p className="panel-desc">
                Runs inside the customer-managed runner. Progress and drift
                verdicts appear under Runs.
              </p>
            </div>
            {message && (
              <div
      ...
      <ul className="drift-resource-list">
        {created.map((event) => (
          <li className="drift-resource-item" key={event.logical_resource_id}>
            <button
              ...
              onClick={() =>
                setExpanded((current) => {
                  ...
                  if (next.has(event.logical_resource_id!))
                    next.delete(event.logical_resource_id!);
              ...
              title={
                stack.data?.resources?.[event.logical_resource_id!]
                  ? "Show template properties"
                  : "Template properties unavailable"
              }
            >
              <ChevronRight className="drift-resource-chevron" />
            ...
            {expanded.has(event.logical_resource_id!) &&
              stack.data?.resources?.[event.logical_resource_id!] && (
                <div className="drift-resource-detail">
                  <PlanResourceDiff
                    resource={{
...
const Stage = ({ step, steps }: { step: TRunStep; steps: TRunStep[] }) => {
  ...
  const applied =
    ...
    steps.some(
      (candidate) =>
        candidate.id === applyID && candidate.status === "finished",
    );
  return (
    <div className="stage">
...
const runPhases = (run: TRun): TRunPhase[] => {
  ...
  for (const step of run.steps ?? []) {
    ...
    if (identity.includes("install-stack") || step.kind === "cloudformation") {
      ...
      ["installation", "upgrade", "bundle-plan"].includes(run.ref_kind)
    ) {
...
const phaseStatus = (phase: TRunPhase) => {
  if (phase.steps.some((step) => statusTheme(step.status) === "error"))
    return "failed";
  if (phase.steps.some((step) => statusTheme(step.status) === "info"))
    return "in-progress";
  if (
    phase.steps.every(
      (step) =>
        statusTheme(step.status) === "success" ||
...
const RunDetail = ({ runId, initial }: { runId: string; initial: TRun }) => {
  ...
  const selectedStep =
    selectedPhase?.steps.find((step) => step.id === selectedStepID) ??
    (selectedPhase ? preferredPhaseStep(selectedPhase) : undefined);
  const control = useMutation({
    mutationFn: (action: "retry" | "user-skip" | "cancel") =>
            ...
            <div className="stage-meta" style={{ marginTop: 8 }}>
              ...
              {run.bundle_digest && (
                <span title={run.bundle_digest}>
                  bundle{" "}
                  <code>
                    {run.bundle_digest.replace("sha256:", "").slice(0, 12)}…
                  </code>
                </span>
              )}
              {run.result_directive && (
                <span>
                  directive <code>{run.result_directive}</code>
                </span>
              )}
            </div>
          </div>
          {run.ref_kind !== "bundle-plan" &&
            (pendingRetry || run.status === "in-progress") && (
              <div className="actions run-controls">
                ...
                {pendingRetry && (
                  <button
                    onClick={() => control.mutate("user-skip")}
                    disabled={control.isPending}
                  >
                    Skip failed step
                  </button>
            ...
            <ol>
              {run.events?.map((event) => (
                <li key={event.sequence}>
                  <code>{event.sequence}</code> {formatTime(event.created_at)} —{" "}
                  {event.status.status}
                  {event.status.result_directive
                    ? ` (${event.status.result_directive})`
                    : ""}
                </li>
              ))}
          ...
          <>
            <div className="card-header run-inspector-header">
              <div>
                ...
                <p>
                  {selectedPhase.steps.length} recorded{" "}
                  {selectedPhase.steps.length === 1 ? "step" : "steps"}
                </p>
              </div>
              <div className="run-step-tabs">
                {selectedPhase.steps.map((step) => (
                  ...
                  >
                    <StageIcon status={step.status} />
                    {step.name
                      .replace(" create-apply-plan", " plan")
                      .replace(" apply-plan", " apply")}
                  </button>
                ))}
              </div>
            </div>
...
const Runs = () => {
  ...
  const filtered = sorted.filter((run) => {
    if (filter === "install")
      return ["installation", "upgrade"].includes(run.ref_kind);
    if (filter === "plan") return run.ref_kind === "bundle-plan";
    if (filter === "action")
      return !["installation", "upgrade", "bundle-plan"].includes(run.ref_kind);
    if (filter === "failed") return statusTheme(run.status) === "error";
    return true;
  });
  const selected =
    filtered.find((run) => run.run_id === selectedID) ?? filtered[0];
        ...
        <section className="card runs-history">
          <div className="card-header runs-history-header">
            <div>
              <h3>Run history</h3>
              <p>
                {filtered.length} of {sorted.length} runs
              </p>
            </div>
            <div className="run-filters">
              {(
                ["all", "install", "plan", "action", "failed"] as RunFilter[]
              ).map((value) => (
                <button
                  type="button"
                ...
                <tbody>
                  {filtered.map((run) => (
                    ...
                    >
                      ...
                      <td>
                        <span className="run-table-muted">
                          {run.finished_at
...
const isOperationSource = (source?: string) =>
  source === "operation";
const sourceLabel = (source?: string) =>
  isOperationSource(source) ? "Operations" : source;

const sourceBadgeTheme = (source?: string) =>
  isOperationSource(source) ? "brand" : source === "install" ? "info" : "";
...
const LogStream = ({ job }: { job: TJobLogSummary }) => {
  ...
  const filtered = entries.filter((entry) => {
    const matchesQuery =
      !query || entryText(entry).includes(query.toLowerCase());
    const matchesLevel =
        ...
        <div className="card-header log-overview-header">
          <div>
            <h3>
              ...
              {!job.logs_available && (
                <Badge theme="warn">logs not synced</Badge>
              )}
            </h3>
            <div
              className="stage-meta log-overview-meta"
              style={{ marginTop: 8 }}
            >
              <span>
              ...
              {job.source && (
                <span>
                  source <code>{sourceLabel(job.source)}</code>
                </span>
              )}
          ...
          <div className="log-overview-actions">
            {job.run_id && (
              <Link
                ...
                to={`/runs?run=${encodeURIComponent(job.run_id)}`}
              >
                <ScrollText /> View run
              </Link>
            )}
            {job.logs_available && (
              <a
                className="secondary-link"
                href={jobLogDownloadURL(job.job_id)}
                download
              >
      ...
      <section className="card log-output">
        <div className="card-header log-output-header">
          <div>
            <h3>Log output</h3>
            <p>
              {log.data?.total ?? 0} recorded lines
              {log.data?.truncated
        ...
        <div className="search-row log-search-row">
          ...
          <input
            ...
            placeholder="Filter messages and structured fields…"
          />
          <span className="polling" style={{ whiteSpace: "nowrap" }}>
        ...
        <State
          ...
          emptyText={
            !job.logs_available
              ? "This job ran, but its log file was not synced."
              : query || level !== "all"
          ...
          <div className="log-stream">
            {filtered.map((entry, index) => (
              <div className="log-row" key={index}>
                <span
                  className="log-time"
                  title={entry.time ? formatDate(entry.time) : undefined}
                >
                  {formatLogTime(entry.time)}
                </span>
                <span className={`log-level ${levelClass(entry.level)}`}>
                  {entry.raw ? "—" : entry.level}
                </span>
...
type LogJobFilter = "all" | "install" | "operation" | "failed";
...
const Logs = () => {
  ...
  const filteredJobs = jobs.filter((job) => {
    const matchesQuery =
      !jobQuery ||
      `${job.job_id} ${job.name ?? ""} ${job.ref_name ?? ""} ${job.run_id ?? ""}`
        .toLowerCase()
        .includes(jobQuery.toLowerCase());
    if (!matchesQuery) return false;
    if (jobFilter === "install") return job.source === "install";
    if (jobFilter === "operation") return isOperationSource(job.source);
    if (jobFilter === "failed") return statusTheme(job.status) === "error";
    return true;
  });
  const selected =
    filteredJobs.find((job) => job.job_id === selectedID) ?? filteredJobs[0];
  ...
  return (
    <main>
      <PageHeader
        title="Logs"
        subtitle="Job logs synced by the customer-managed runner: install steps and operation runs."
      />
        ...
        <section className="card logs-history">
          <div className="card-header logs-history-header">
            ...
            <div className="log-job-filters">
              {(
                ["all", "install", "operation", "failed"] as LogJobFilter[]
              ).map((value) => (
                <button
                  type="button"
                  ...
                  {filteredJobs.map((job) => (
                    ...
                    >
                      <td>
                        ...
                        {!job.logs_available && (
                          <span className="subtle">log file not synced</span>
                        )}
                      </td>
                      <td>
                        <span className="run-table-muted">
                          {job.ref_name ?? job.run_id ?? "—"}
                        </span>
                      </td>
                      <td>
                        <span className="run-table-muted">
    ...
    <div className="offline-portal">
      <div className="shell">
        <aside className="sidebar">
          <div className="brand">
            ...
            <span>
              <span className="brand-name">
                {branding.data?.name ?? "Deployment portal"}
              </span>
              <span className="brand-sub">customer-managed runner</span>
            </span>
          </div>
          <nav>
            <NavLink end to="/">
          ...
          {branding.data?.support_url ? (
            <a
              className="support-link"
              href={branding.data.support_url}
              target="_blank"
              rel="noreferrer"
            >
              <ArrowUpRight /> Get support
            </a>
          ) : null}
        </aside>
        <div className="main-col">
          <header className="topbar">
            ...
            <div className="top-pills">
              <span className="top-pill">customer-managed</span>
              <RunnerPill />
            </div>
          </header>
          <Routes>
...
export const App = () => {
  const mode = useQuery({
    ...
    queryFn: getPortalMode,
    staleTime: Infinity,
  });
  if (mode.isLoading) return <div className="state">Loading portal…</div>;
  if (mode.error)
    return <div className="notice error">{mode.error.message}</div>;
  return mode.data?.mode === "connected" ? <ConnectedApp /> : <Layout />;
};
