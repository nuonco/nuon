...
export const getPortalMode = () =>
  request<TPortalMode>("/api/mode/capabilities");
...
export const installationRegistrationDownloadURL = () =>
  "/api/installation-registration";
export const supportSnapshotDownloadURL = (includeState: boolean) =>
  `/api/support-snapshot?include_state=${includeState}`;
export const getRuns = () => request<TRun[]>("/api/runs");
export const getRun = (id: string) =>
  request<TRun>(`/api/runs/${encodeURIComponent(id)}`);
export const controlRun = (
  id: string,
  action: "retry" | "user-skip" | "cancel",
) =>
...
export const approveBundleCandidate = (bundleDigest: string) =>
  request<{ bundle_digest: string; status: string }>(
    "/api/bundle-candidate/approve",
    {
      method: "POST",
      headers: csrfHeaders(),
  ...
  new Promise<TBundleCandidate>((resolve, reject) => {
    ...
    request.onload = () => {
      ...
      try {
        ...
        ) as TBundleCandidate & { error?: string };
      } catch {
      ...
      if (request.status < 200 || request.status >= 300) {
        reject(
          new Error(body?.error ?? `${request.status} ${request.statusText}`),
        );
        return;
      }
      if (!body) {
