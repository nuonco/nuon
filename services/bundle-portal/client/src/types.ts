...
// The late-bound composite plan the airgapped runner renders for a bootstrap
...
export type TRun = {
  ...
  finished_at?: string;
  result_directive?: string;
  events?: Array<{ schema_version: number; sequence: number; created_at: string; status: { status: string; failed_step?: string; result_directive?: string } }>;
  steps?: TRunStep[];
};
