...
SANDBOX=$(curl -s --X POST --data '{"name":"aws-eks","description":"standard sandbox"}' $INTERNAL_BASE_URL/v1/sandboxes)
echo $SANDBOX
SANDBOX_ID=$(echo $SANDBOX | jq -r .id)

echo "creating sandbox release..."
SANDBOX_RELEASE=$(curl -s -X POST --data '{"version":"08e7f11","terraform_version":"v1.5.3"}' "$INTERNAL_BASE_URL/v1/sandboxes/$SANDBOX_ID/release")
