...
`))
	...
	contents, source, err := (&Activities{cfg: ctlCfg, db: db}).compileRootTemplate(context.Background(), appCfg, "inl-synthetic", "runner-v1")
	require.NoError(t, err)
...
func TestValidateRunnerNestedTemplateCustomerManagedCompatible(t *testing.T) {
	require.NoError(t, validateRunnerNestedTemplateCustomerManagedCompatible([]byte(`
Parameters:
  RunnerApiUrl:
    Type: String
...
`)))
	...
	err := validateRunnerNestedTemplateCustomerManagedCompatible([]byte(`
Parameters:
  RunnerApiToken:
    Type: String
`))
	require.ErrorContains(t, err, "RunnerApiToken")
	require.ErrorContains(t, err, "offline-compatible")
}
