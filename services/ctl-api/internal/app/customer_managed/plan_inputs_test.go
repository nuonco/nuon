...
func inputEnvelope(inputs []customermanaged.InputSpec, plan string) *customermanaged.Envelope {
	return &customermanaged.Envelope{
...
func TestRewriteInputPlaceholdersExactLeaf(t *testing.T) {
	...
	envelope := inputEnvelope([]customermanaged.InputSpec{{Name: "region", Default: "us-west-2"}}, `{"vars":{"region":"us-west-2"}}`)
...
func TestRewriteInputPlaceholdersEmbeddedValue(t *testing.T) {
	db := planInputsTestDB(t)
	envelope := inputEnvelope([]customermanaged.InputSpec{{Name: "domain", Default: "example.com"}}, `{"url":"https://api.example.com/v1"}`)
...
func TestRewriteInputPlaceholdersSkipsGenericValues(t *testing.T) {
	db := planInputsTestDB(t)
	envelope := inputEnvelope([]customermanaged.InputSpec{
...
func TestRewriteInputPlaceholdersRejectsBakedSecret(t *testing.T) {
	db := planInputsTestDB(t)
	envelope := inputEnvelope([]customermanaged.InputSpec{{Name: "token", Default: "secret-value", Secret: true}}, `{"authorization":"Bearer secret-value"}`)
...
func TestRewriteInputPlaceholdersUsesLatestInstallValue(t *testing.T) {
	...
	envelope := inputEnvelope([]customermanaged.InputSpec{{Name: "region", Default: "us-west-2"}}, `{"region":"eu-central-1"}`)
