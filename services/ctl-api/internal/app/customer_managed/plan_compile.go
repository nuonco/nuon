...
const compilePlanEnvelopeSource = "ctl-api customer_managed_bundle compile"
...
func CompilePlanEnvelope(ctx context.Context, db *gorm.DB, v *validator.Validate, orgID, appID string, cfg *app.AppConfig, sandboxBuildID string, componentBuildIDs map[string]string, runbooks []customermanaged.RunbookTemplate, report *QualificationReport) (*customermanaged.Envelope, error) {
	if cfg == nil {
	...
	templateReferences := templateReferenceData(cfg, cfgJSON, appConfigJSON)
	componentRefs = extractComponentOutputRefs(templateReferences)
	if err := validateComponentOutputRefs(componentRefs, connections, report); err != nil {
		return nil, err
	}
	...
	st, stateData, err := compileState(orgID, appID, installID, templateReferences, inputs, componentRefs, needsSandboxCluster(connections, templateReferences), roleConfigNames(cfg.BreakGlassConfig.Roles), roleConfigNames(cfg.PermissionsConfig.CustomRoles), cfg.StackConfig.Type)
	if err != nil {
		return nil, err
	}
	...
	for _, connection := range order {
		...
		steps = append(steps, customermanaged.Step{ID: syncID, Name: connection.ComponentName + " sync", JobType: "oci-sync", JobOperation: "exec", JobGroup: "sync", DependsOn: []string{previous}, CompositePlan: syncComposite})
		...
		steps = append(steps,
			customermanaged.Step{ID: planID, Name: connection.ComponentName + " create-apply-plan", JobType: jobType, JobOperation: "create-apply-plan", JobGroup: "deploy", DependsOn: []string{syncID}, CompositePlan: deployComposite},
			customermanaged.Step{ID: applyID, Name: connection.ComponentName + " apply-plan", JobType: jobType, JobOperation: "apply-plan", JobGroup: "deploy", DependsOn: []string{planID}, PlanFromStep: planID, CompositePlan: deployComposite},
		)
		previous = applyID
	...
	bindings := make([]customermanaged.OutputBinding, 0, len(componentRefs))
	for _, ref := range componentRefs {
		...
		bindings = append(bindings, customermanaged.OutputBinding{Token: ref.token(), ComponentName: ref.Component, StepID: stepID, OutputPath: ref.Path})
	}
...
func compileInstall(orgID, appID, appConfigID, installID string) *app.Install {
	install := &app.Install{ID: installID, OrgID: orgID, AppID: appID, AppConfigID: appConfigID, Name: "customer_managed"}
...
func templateReferenceData(cfg *app.AppConfig, sources ...[]byte) []byte {
	result := make([]byte, 0)
	appendSource := func(source []byte) {
	...
	for _, actionCfg := range cfg.ActionWorkflowConfigs {
		for _, stepCfg := range actionCfg.Steps {
			appendSource([]byte(stepCfg.Command))
			appendSource([]byte(stepCfg.InlineContents))
			for _, value := range stepCfg.EnvVars {
				if value != nil {
					appendSource([]byte(*value))
				}
			}
		}
	}
...
func compileState(orgID, appID, installID string, raw []byte, inputs []customermanaged.InputSpec, componentRefs []componentOutputRefKey, seedSandboxCluster bool, breakGlassRoles, customRoles []string, stackType app.StackType) (*statepkg.State, map[string]any, error) {
	...
	for _, path := range sortedKeys(sandboxPaths) {
		token := "__NUON_CUSTOMER_MANAGED_SANDBOX_" + strings.ReplaceAll(path, ".", "_") + "__"
		if err := seedPlaceholderPath(sandboxOutputs, path, token); err != nil {
			return nil, nil, fmt.Errorf("sandbox output references: %w", err)
		}
	}
	sandboxOutputs["ecr"] = map[string]any{"repository_url": "__NUON_CUSTOMER_MANAGED_SANDBOX_ecr_repository_url__", "registry_url": "__NUON_CUSTOMER_MANAGED_SANDBOX_ecr_registry_url__"}
	...
	if seedSandboxCluster {
		sandboxOutputs["cluster"] = map[string]any{"name": "__NUON_CUSTOMER_MANAGED_CLUSTER_id__", "endpoint": "__NUON_CUSTOMER_MANAGED_CLUSTER_endpoint__", "certificate_authority_data": "__NUON_CUSTOMER_MANAGED_CLUSTER_ca_data__"}
	}
	inputValues := map[string]string{}
	for _, input := range inputs {
		inputValues[input.Name] = customermanaged.InputPlaceholder(input.Name)
	}
...
func seedRoleARNOutputs(st *statepkg.State, stackOutputs map[string]any, breakGlassRoles, customRoles []string) error {
	...
	seed := func(key string, names []string) error {
		...
		for i, name := range names {
			...
			arns[rendered] = fmt.Sprintf("__NUON_CUSTOMER_MANAGED_STACK_%s_%d__", key, i)
		}
...
func compileCloudAccount(stackType app.StackType) *statepkg.CloudAccount {
	...
	case app.StackTypeAzure:
		return &statepkg.CloudAccount{Azure: &statepkg.AzureCloudAccount{Location: "__NUON_CUSTOMER_MANAGED_STACK_resource_group_location__"}}
	case app.StackTypeGCP:
		return &statepkg.CloudAccount{GCP: &statepkg.GCPCloudAccount{ProjectID: "__NUON_CUSTOMER_MANAGED_STACK_project_id__", Region: "__NUON_CUSTOMER_MANAGED_STACK_region__"}}
	default:
		return &statepkg.CloudAccount{AWS: &statepkg.AWSCloudAccount{Region: "__NUON_CUSTOMER_MANAGED_STACK_region__"}}
	}
}
...
func compileStack(cfg *app.AppConfig, installID string) *app.InstallStack {
	...
	case app.StackTypeAzure:
		outputs.AzureStackOutputs = &app.AzureStackOutputs{
			SubscriptionID:              "__NUON_CUSTOMER_MANAGED_STACK_subscription_id__",
			SubscriptionTenantID:        "__NUON_CUSTOMER_MANAGED_STACK_subscription_tenant_id__",
			MaintenanceIdentityClientID: "__NUON_CUSTOMER_MANAGED_STACK_maintenance_identity_client_id__",
		}
	case app.StackTypeGCP:
		outputs.GCPStackOutputs = &app.GCPStackOutputs{
			ProjectID:          "__NUON_CUSTOMER_MANAGED_STACK_project_id__",
			Region:             "__NUON_CUSTOMER_MANAGED_STACK_region__",
			ProvisionSAEmail:   "__NUON_CUSTOMER_MANAGED_STACK_provision_sa_email__",
			DeprovisionSAEmail: "__NUON_CUSTOMER_MANAGED_STACK_deprovision_sa_email__",
			MaintenanceSAEmail: "__NUON_CUSTOMER_MANAGED_STACK_maintenance_sa_email__",
		}
	default:
		outputs.AWSStackOutputs = &app.AWSStackOutputs{
			AccountID:             "__NUON_CUSTOMER_MANAGED_STACK_account_id__",
			Region:                "__NUON_CUSTOMER_MANAGED_STACK_region__",
			VPCID:                 "__NUON_CUSTOMER_MANAGED_STACK_vpc_id__",
			ProvisionIAMRoleARN:   "__NUON_CUSTOMER_MANAGED_STACK_provision_iam_role_arn__",
			DeprovisionIAMRoleARN: "__NUON_CUSTOMER_MANAGED_STACK_deprovision_iam_role_arn__",
			MaintenanceIAMRoleARN: "__NUON_CUSTOMER_MANAGED_STACK_maintenance_iam_role_arn__",
			RunnerIAMRoleARN:      "__NUON_CUSTOMER_MANAGED_STACK_runner_iam_role_arn__",
		}
	}
...
func compileRoleSelection(cfg *app.AppConfig, stack *app.InstallStack) *operationroles.RoleSelection {
	...
	arn := "__NUON_CUSTOMER_MANAGED_STACK_maintenance_role__"
	switch {
...
func compileOrgRegistry(orgID, appID string) *configs.OCIRegistryRepository {
	return &configs.OCIRegistryRepository{Plugin: "oci", RegistryType: configs.OCIRegistryTypePrivateOCI, Repository: orgID + "/" + appID, LoginServer: "__NUON_CUSTOMER_MANAGED_VENDOR_REGISTRY__"}
}
...
func compileActionTemplates(cfg *app.AppConfig, planner *planpkg.Planner, l *zap.Logger, installID string, stateData map[string]any, stack *app.InstallStack, role *operationroles.RoleSelection, report *QualificationReport) ([]customermanaged.ActionTemplate, error) {
	result := make([]customermanaged.ActionTemplate, 0, len(cfg.ActionWorkflowConfigs))
	for _, actionCfg := range cfg.ActionWorkflowConfigs {
		if step := firstGitSourcedActionStep(actionCfg); step != "" {
			addQualificationWarning(report, "action.git_source_excluded", "action:"+actionCfg.ActionWorkflowID,
				fmt.Sprintf("action %s was excluded from the bundle: step %s sources its contents from Git, and portable bundle actions must be inline-only", actionCfg.ActionWorkflow.Name, step))
			continue
		}
		...
		result = append(result, customermanaged.ActionTemplate{ID: actionCfg.ActionWorkflowID, Name: actionCfg.ActionWorkflow.Name, CronSchedule: cron, JobType: "actions-workflow", JobGroup: "actions", JobOperation: "exec", CompositePlan: composite})
	}
	sort.Slice(result, func(i, j int) bool { return result[i].ID < result[j].ID })
	return result, nil
}

func compileDriftTemplates(steps []customermanaged.Step, components []customermanaged.ComponentSpec) ([]customermanaged.DriftTemplate, error) {
	byName := map[string]customermanaged.ComponentSpec{}
	for _, component := range components {
		byName[component.ComponentName] = component
	}
	result := []customermanaged.DriftTemplate{}
	for _, step := range steps {
		...
		result = append(result, customermanaged.DriftTemplate{ID: "drift-" + component.ComponentID, ComponentID: component.ComponentID, ComponentName: name, JobType: step.JobType, JobGroup: step.JobGroup, JobOperation: "create-apply-plan", CompositePlan: plan})
	}
	return result, nil
}
