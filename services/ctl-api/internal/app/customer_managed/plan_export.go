...
const planEnvelopeSource = "ctl-api customer_managed_bundle publish"
...
func ExportPlanEnvelope(ctx context.Context, db *gorm.DB, blobRead bool, orgID, appID, appConfigID string, runbooks []customermanaged.RunbookTemplate, report *QualificationReport) (*customermanaged.Envelope, error) {
	...
	if len(installs) == 0 {
		return nil, fmt.Errorf("portable bundle requires an install of app config %s with executed plans; none found", appConfigID)
	}

	var steps []customermanaged.Step
...
func exportInstallSteps(ctx context.Context, db *gorm.DB, blobRead bool, installID string) ([]customermanaged.Step, error) {
	...
	steps := make([]customermanaged.Step, 0, len(selected))
	for i, item := range selected {
		step := customermanaged.Step{
			ID:            item.job.ID,
...
func exportComponents(ctx context.Context, db *gorm.DB, installID, appConfigID string) ([]customermanaged.ComponentSpec, error) {
...
func componentSpecs(installComponents []app.InstallComponent, connections []app.ComponentConfigConnection) []customermanaged.ComponentSpec {
	...
	specs := make([]customermanaged.ComponentSpec, 0, len(installComponents))
	for _, ic := range installComponents {
		spec := customermanaged.ComponentSpec{
...
func exportInputSpecs(ctx context.Context, db *gorm.DB, appConfigID string) ([]customermanaged.InputSpec, error) {
	...
	specs := make([]customermanaged.InputSpec, 0, len(config.AppInputs))
	for _, in := range config.AppInputs {
		specs = append(specs, customermanaged.InputSpec{
			Name:        in.Name,
			Type:        string(in.Type),
