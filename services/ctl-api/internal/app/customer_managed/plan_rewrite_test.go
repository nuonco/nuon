...
func rewriteTestEnvelope(t *testing.T, steps ...customermanaged.Step) *customermanaged.Envelope {
	t.Helper()
	return &customermanaged.Envelope{Version: "v0", OrgID: "org1", AppID: "app1", InstallID: "inl1", Steps: steps}
}
...
func TestRewriteEnvelopeForBundleSandboxSource(t *testing.T) {
	...
	envelope := rewriteTestEnvelope(t, customermanaged.Step{ID: "job1", JobType: "sandbox-terraform", JobGroup: "sandbox", CompositePlan: json.RawMessage(plan)})
...
func TestRewriteEnvelopeForBundleRetagsSyncSources(t *testing.T) {
	...
	envelope := rewriteTestEnvelope(t, customermanaged.Step{ID: "job1", JobType: "oci-sync", JobGroup: "sync", CompositePlan: json.RawMessage(plan)})
