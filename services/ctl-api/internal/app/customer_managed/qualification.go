	...
	for i := range cfg.ActionWorkflowConfigs {
		...
		for j := range a.Steps {
			...
			if s.ConnectedGithubVCSConfig != nil || s.PublicGitVCSConfig != nil {
				if hasCron {
					add("action_step.git_unsupported", step, "cron action steps must be inline for portable bundles; use inline_contents")
				} else {
					warn("action_step.git_excluded", step, "Git-backed non-cron action is excluded from the portable bundle")
				}
