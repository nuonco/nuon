...
func createCustomerManagedInstallTables(t *testing.T, svc *service) {
	...
	require.NoError(t, svc.db.Exec(`CREATE TABLE install_operating_models (
		id text primary key, created_by_id text, created_at datetime, org_id text, install_id text unique,
		connectivity text, release_selection text, command_authority text, approval_authority text, telemetry text
	)`).Error)
	...
	require.NoError(t, svc.db.Exec(`CREATE TABLE install_release_deployments (
		...
		previous_release_id text, install_app_config_version_id text, operating_model_id text, method text, actor text, executor text, operation_id text, plan_digest text,
...
func TestRegisterInstallAtomicallyCreatesDisconnectedRecordsAndIsIdempotent(t *testing.T) {
	...
	require.Equal(t, app.InstallConnectivityDisconnected, first.OperatingModel.Connectivity)
	require.Equal(t, app.InstallAuthorityCustomer, first.OperatingModel.CommandAuthority)
	...
	for model, expected := range map[any]int64{
		...
		&app.InstallOperatingModel{}: 1, &app.InstallReleaseDeployment{}: 1,
	} {
		var count int64
