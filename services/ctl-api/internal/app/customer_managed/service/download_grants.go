...
// @Summary create a download grant for a published portable bundle
...
func (s *service) CreateDownloadGrant(ctx *gin.Context) {
	...
	if err != nil {
		...
		ctx.Error(fmt.Errorf("unable to create portable bundle download grant: %w", err))
		return
	}
...
func (s *service) createDownloadGrant(ctx context.Context, orgID, appID, bundleID string) (*downloadGrantResponse, error) {
	...
	if bundle.Status != app.CustomerManagedBundleStatusActive {
		return nil, conflictError{message: "portable bundle is not active or does not have a verified replica"}
	}
	var replicas []app.CustomerManagedBundleTransportReplica
	result := s.db.WithContext(ctx).
		Where(app.CustomerManagedBundleTransportReplica{OrgID: orgID, BundleID: bundle.ID}).
		Order("verified_at DESC").Order("created_at DESC").Order("id ASC").Find(&replicas)
	if result.Error != nil {
		return nil, result.Error
	}
	var replica *app.CustomerManagedBundleTransportReplica
