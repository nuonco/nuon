...
func TestCreateBlobGrantsReturnsMetadataAndGrants(t *testing.T) {
	...
	require.NoError(t, svc.db.Model(&app.CustomerManagedBundle{ID: bundle.ID}).Update("oci_index_digest", "sha256:"+indexDigest).Error)
...
func TestCreateBlobGrantsHandlerValidatesRequest(t *testing.T) {
	...
	digest := strings.Repeat("a", 64)
	bundle := seedBundle(t, svc, strings.Repeat("1", 26), "org-a", "app-a", digest, now)
	require.NoError(t, svc.db.Model(&app.CustomerManagedBundle{ID: bundle.ID}).Update("oci_index_digest", "sha256:"+digest).Error)
