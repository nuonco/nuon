...
func (s *service) PortalGetRelease(ctx *gin.Context) {
	...
	release, err := s.releases.GetReleaseCore(ctx, portal.Install.OrgID, portal.Install.AppID, ctx.Param("release_id"))
	if err != nil {
		ctx.Error(err)
		return
	}
...
func (s *service) PortalGetReleaseFileContent(ctx *gin.Context) {
	...
	content, err := s.releases.GetReleaseFileContentCore(ctx, portal.Install.OrgID, portal.Install.AppID, release.ID, ctx.Query("path"))
	if err != nil {
		ctx.Error(err)
		return
	}
