...
func (s *service) syncBundleHistory(ctx context.Context, install *app.Install, history []operation.BundleInfo) error {
	...
	return s.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.Where(app.InstallOperatingModel{OrgID: install.OrgID, InstallID: install.ID}).First(&operatingModel).Error; err != nil {
			return fmt.Errorf("load install operating model: %w", err)
		}
		...
		for _, info := range sorted {
			...
			deployment := app.InstallReleaseDeployment{
				...
				OperatingModelID: operatingModel.ID, Method: app.InstallDeploymentMethodDisconnectedLocal,
