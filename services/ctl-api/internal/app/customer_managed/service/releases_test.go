...
func TestSemanticReleaseDigestIsCanonicalAndPackageIndependent(t *testing.T) {
	...
	amd64PackageDigest := releasePackageDigest(digest, "sha256:runtime-v1", createReleasePackageRequest{Format: app.ReleasePackageFormatPortableOCI, TargetPlatform: "linux/amd64"})
	arm64PackageDigest := releasePackageDigest(digest, "sha256:runtime-v1", createReleasePackageRequest{Format: app.ReleasePackageFormatPortableOCI, TargetPlatform: "linux/arm64"})

	require.NotEqual(t, amd64PackageDigest, arm64PackageDigest)
...
func TestBundleTargetRequiresOSAndArchitecture(t *testing.T) {
	target, err := bundleTarget("linux/arm64")
	require.NoError(t, err)
	require.Equal(t, "linux", target.OS)
	require.Equal(t, "arm64", target.Architecture)

	_, err = bundleTarget("linux")
	require.ErrorContains(t, err, "expected os/architecture")
	_, err = bundleTarget("linux/amd64/extra")
	require.ErrorContains(t, err, "expected os/architecture")
}
