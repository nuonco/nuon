...
func (s *service) CreateReleaseUpdate(ctx *gin.Context) {
	...
	if err := s.db.WithContext(ctx).Where(app.InstallOperatingModel{OrgID: org.ID, InstallID: install.ID}).First(&operatingModel).Error; err != nil {
		ctx.Error(fmt.Errorf("load install operating model: %w", err))
		return
	}
	if operatingModel.Connectivity != app.InstallConnectivityConnected || operatingModel.ReleaseSelection != app.InstallReleaseSelectionVendor || operatingModel.ApprovalAuthority != app.InstallAuthorityCustomer {
		ctx.Error(stderr.ErrInvalidRequest{Err: fmt.Errorf("install operating model does not allow vendor-proposed, customer-approved release updates")})
		return
	}
	...
	update, _, err := s.installsHelpers.StartInstallAppConfigUpdate(ctx, installshelpers.StartInstallAppConfigUpdateInput{
		...
		OperatingModelID: operatingModel.ID, ReleaseComponentBuildIDs: release.ComponentBuildIDs,
...
func (s *service) validateReleaseBuilds(ctx *gin.Context, release app.AppRelease) error {
	...
	for _, connection := range cfg.ComponentConfigConnections {
		...
		if _, err := s.releases.ActiveBuildForConnection(ctx, release.OrgID, buildID, connection); err != nil {
			return fmt.Errorf("release component build %s is not active or does not belong to component %s: %w", buildID, connection.ComponentID, err)
		}
	}
	return nil
}
...
func (s *service) PortalListReleaseUpdates(ctx *gin.Context) {
	...
	if err := s.db.WithContext(ctx).
		Preload("Workflow").
		Where(app.InstallAppConfigVersion{OrgID: portal.Install.OrgID, InstallID: portal.Install.ID}).
