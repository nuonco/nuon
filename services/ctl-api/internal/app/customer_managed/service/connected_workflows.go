...
func (s *service) PortalListWorkflows(ctx *gin.Context) {
	portal, ok := s.connectedPortal(ctx)
	if !ok {
		return
	}
	var workflows []app.Workflow
	if err := s.db.WithContext(ctx).Where(app.Workflow{OwnerID: portal.Install.ID, OwnerType: "installs"}).Order("created_at DESC").Limit(100).Find(&workflows).Error; err != nil {
		ctx.Error(err)
		return
	}
...
func (s *service) PortalGetWorkflow(ctx *gin.Context) {
	portal, ok := s.connectedPortal(ctx)
	if !ok {
		return
	}
	workflow, err := s.portalWorkflow(ctx, portal, ctx.Param("workflow_id"))
	if err != nil {
		ctx.Error(err)
		return
	}
...
func (s *service) PortalGetWorkflowStepLogs(ctx *gin.Context) {
	portal, ok := s.connectedPortal(ctx)
	if !ok {
		return
	}
	...
	if err := s.chDB.WithContext(ctx).
		Where(app.OtelLogRecord{OrgID: portal.Install.OrgID, LogStreamID: step.LogStream.ID}).
		Order("timestamp ASC").
		Limit(500).
...
func (s *service) PortalRetryWorkflowStep(ctx *gin.Context) {
	portal, ok := s.connectedPortal(ctx)
	if !ok {
		return
	}
...
func (s *service) PortalGetApprovalContents(ctx *gin.Context) {
	portal, ok := s.connectedPortal(ctx)
	if !ok {
		return
	}
	_, step, approval, err := s.portalApproval(ctx, portal)
	if err != nil {
		ctx.Error(err)
		return
	}
...
type portalApprovalResponseRequest struct {
	ResponseType app.WorkflowStepResponseType `json:"response_type"`
	Note         string                       `json:"note"`
}

func (s *service) PortalCreateApprovalResponse(ctx *gin.Context) {
	portal, ok := s.connectedPortal(ctx)
	if !ok {
		return
	}
	...
	var req portalApprovalResponseRequest
	...
	workflow, step, approval, err := s.portalApproval(ctx, portal)
	if err != nil {
		ctx.Error(err)
		return
	}
	...
	requestCtx, err := s.connectedWorkflowContext(ctx, portal)
	if err != nil {
		ctx.Error(err)
		return
	}
	...
	signal := &workflowstepapprovalresponse.Signal{
		InstallID: portal.Install.ID, InstallWorkflowID: workflow.ID, WorkflowStepID: step.ID,
		ApprovalID: approval.ID, ApprovalResponseID: response.ID, ResponseType: response.Type,
	}
	queue, err := s.queueClient.GetQueueByOwnerAndName(requestCtx, portal.Install.ID, "installs", installshelpers.InstallSignalsQueueName)
	if err != nil {
		ctx.Error(err)
		return
	}
...
func (s *service) portalWorkflow(ctx context.Context, portal *portalContext, workflowID string) (*app.Workflow, error) {
	...
	err := s.db.WithContext(ctx).
		...
		Where(app.Workflow{ID: workflowID, OrgID: portal.Install.OrgID, OwnerID: portal.Install.ID, OwnerType: "installs"}).First(&workflow).Error
	if err != nil {
		return &workflow, err
	}
	if err := s.loadPortalStepLogStreams(ctx, portal.Install.OrgID, workflow.Steps); err != nil {
		return &workflow, err
	}
	return &workflow, nil
}

func (s *service) loadPortalStepLogStreams(ctx context.Context, orgID string, steps []app.WorkflowStep) error {
...
func (s *service) portalApproval(ctx *gin.Context, portal *portalContext) (*app.Workflow, *app.WorkflowStep, *app.WorkflowStepApproval, error) {
	workflow, err := s.portalWorkflow(ctx, portal, ctx.Param("workflow_id"))
	if err != nil {
...
func (s *service) connectedWorkflowContext(ctx context.Context, portal *portalContext) (context.Context, error) {
	return ctx, nil
}
