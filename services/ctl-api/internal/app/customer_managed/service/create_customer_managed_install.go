...
type customerManagedInstallResponse struct {
	...
	ManagementPolicy     app.InstallManagementPolicyVersion `json:"management_policy"`
...
func (s *service) CreateCustomerManagedInstall(ctx *gin.Context) {
	...
	install, workflow, err := provisioning.Prepare(ctx, s.installsHelpers, s.db, req.AppID, &installshelpers.CreateInstallParams{
		Name: req.IntendedName, AppConfigID: release.AppConfigID, Inputs: req.Inputs,
		RunnerImageURL: release.Runtime.RunnerImageURL, RunnerImageTag: release.Runtime.RunnerImageTag,
	...
	policy := app.InstallManagementPolicyVersion{
		InstallID: install.ID, Version: 1, Connectivity: app.InstallConnectivityConnected,
		ReleaseSelection: app.InstallReleaseSelectionVendor, CommandAuthority: app.InstallAuthorityCustomer,
		ApprovalAuthority: app.InstallAuthorityCustomer, Telemetry: req.Telemetry, EffectiveAt: time.Now().UTC(),
	}
	if err := s.db.WithContext(ctx).Create(&policy).Error; err != nil {
		ctx.Error(fmt.Errorf("create install management policy: %w", err))
		return
	}
	...
	ctx.JSON(http.StatusCreated, customerManagedInstallResponse{
		Install: *install, ManagementPolicy: policy, PortalServiceAccount: *portalAccount,
	})
}
