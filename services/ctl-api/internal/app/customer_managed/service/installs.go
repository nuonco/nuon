...
func (s *service) registerInstall(ctx *gin.Context, orgID string, registration customermanaged.InstallationRegistration) (*installRegistrationResponse, bool, error) {
	...
	err = s.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		operatingModel.InstallID, operatingModel.OrgID = install.ID, orgID
		...
		deployment.InstallID, deployment.OrgID, deployment.OperatingModelID = install.ID, orgID, operatingModel.ID
	...
	return &installRegistrationResponse{Install: install, Registration: record, OperatingModel: operatingModel, Deployment: deployment}, true, nil
}
