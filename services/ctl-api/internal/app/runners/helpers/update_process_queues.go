...
// UpdateProcessQueues updates given runner process queue emitters for the cron health check
// and scheduled uptime TTL emitter.
func (h *Helpers) UpdateProcessQueues(ctx context.Context, runnerID string, process *app.RunnerProcess) (*app.Queue, error) {
	q, err := h.queueClient.Update(ctx, &queueclient.UpdateQueueRequest{
		OwnerID:     runnerID,
		OwnerType:   "runners",
	...
	if _, err := h.emitterClient.UpdateEmitter(ctx, &emitterclient.UpdateEmitterRequest{
		QueueID:      q.ID,
