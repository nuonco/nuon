...
func (w *Workflows) createRunnerVMShutDownJob(ctx workflow.Context, runnerID string, metadata map[string]string) (*app.RunnerJob, error) {
	runner, err := activities.AwaitGetByRunnerID(ctx, runnerID)
	if err != nil {
		return nil, errors.Wrap(err, "unable to get runner")
	}
	...
	runnerJob, err := activities.AwaitCreateVMShutdownJob(ctx, &activities.CreateShutdownJobRequest{
		RunnerID:    runner.ID,
		OwnerID:     runner.ID,
		LogStreamID: logStream.ID,
		Metadata:    metadata,
	...
	l.Info("dispatching vm shutdown job to runner",
		zap.String("runner_id", runner.ID),
		zap.String("runner_type", string(runner.RunnerGroup.Type)),
	)
