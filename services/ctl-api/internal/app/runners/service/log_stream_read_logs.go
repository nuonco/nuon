...
func hasAnyFilter(f logFilters) bool {
	return len(f.serviceNames) > 0 ||
		len(f.scopeNames) > 0 ||
		len(f.scopeVersions) > 0 ||
		len(f.severityTexts) > 0 ||
		f.severityNumberMin > 0 ||
		f.severityNumberMax > 0 ||
		len(f.resourceSchemaURLs) > 0 ||
		len(f.scopeSchemaURLs) > 0 ||
		f.traceID != "" ||
		f.spanID != "" ||
		f.traceFlagsSet ||
		f.runnerID != "" ||
		f.runnerJobID != "" ||
		f.runnerGroupID != "" ||
		f.runnerJobExecutionID != "" ||
		f.runnerJobExecutionStep != "" ||
		len(f.tools) > 0 ||
		f.helmReleaseName != "" ||
		f.helmChartName != "" ||
		f.helmChartID != "" ||
