...
// awsRegionCertPEMs contains the AWS RSA-2048 public certificates for
// verifying instance identity document signatures, keyed by region.
// Source: https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/regions-certs.html
var awsRegionCertPEMs = map[string]string{
...
// AWSCertStore provides parsed x509 certificates for IID verification.
type AWSCertStore struct {
	certs map[string]*x509.Certificate
}

// NewAWSCertStore parses the embedded certs on startup.
func NewAWSCertStore(l *zap.Logger) (*AWSCertStore, error) {
	certs := make(map[string]*x509.Certificate)

	for region, pemData := range awsRegionCertPEMs {
	...
	return &AWSCertStore{certs: certs}, nil
}

// GetCert returns the certificate for the given AWS region.
func (s *AWSCertStore) GetCert(region string) (*x509.Certificate, error) {
