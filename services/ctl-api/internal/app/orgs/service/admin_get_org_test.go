...
func (s *AdminGetOrgTestSuite) TestAdminGetOrg() {
	testCases := []struct {
		...
		{
			name: "preloads CreatedBy relation",
			setupFunc: func() string {
				ctx := context.Background()
				ctx = cctx.SetAccountContext(ctx, s.testAcc)

				// Create a specific account for CreatedBy
				creatorAcc := &app.Account{
					ID:          domains.NewAccountID(),
					Email:       "creator@example.com",
					Subject:     "creator-subject",
					AccountType: app.AccountTypeAuth0,
				}
				err := s.service.DB.Create(creatorAcc).Error
				require.NoError(s.T(), err)
				s.T().Cleanup(func() {
					s.service.DB.Unscoped().Delete(&app.Account{}, "id = ?", creatorAcc.ID)
				})

				// Create org with CreatedBy set
				ctx = cctx.SetAccountContext(ctx, creatorAcc)
				org := &app.Org{
					ID:   domains.NewOrgID(),
					Name: "org-with-creator",
					NotificationsConfig: app.NotificationsConfig{
						InternalSlackWebhookURL: "https://hooks.slack.com/creator",
					},
				}
				err = s.service.DB.WithContext(ctx).Create(org).Error
				require.NoError(s.T(), err)
			...
			expectedCode: http.StatusOK,
			validateFunc: func(org *app.Org) {
				require.NotNil(s.T(), org.CreatedBy, "CreatedBy should be preloaded")
				require.Equal(s.T(), "creator@example.com", org.CreatedBy.Email)
			},
		},
		{
...
func (s *AdminGetOrgTestSuite) TestAdminGetOrgPreloadsCombinedRelations() {
	s.Run("returns org with all relations preloaded together", func() {
		ctx := context.Background()
		ctx = cctx.SetAccountContext(ctx, s.testAcc)
		...
		creatorAcc := &app.Account{
			ID:          domains.NewAccountID(),
			Email:       "full-creator@example.com",
			Subject:     "full-creator-subject",
			AccountType: app.AccountTypeAuth0,
		}
		err := s.service.DB.Create(creatorAcc).Error
		require.NoError(s.T(), err)
		...
		// Create org with CreatedBy
		ctx = cctx.SetAccountContext(ctx, creatorAcc)
		org := &app.Org{
			ID:   domains.NewOrgID(),
			Name: "full-preload-org",
			NotificationsConfig: app.NotificationsConfig{
				InternalSlackWebhookURL: "https://hooks.slack.com/full",
			},
		}
		err = s.service.DB.WithContext(ctx).Create(org).Error
		require.NoError(s.T(), err)
		...
		require.NoError(s.T(), err)
		s.T().Cleanup(func() {
			s.service.DB.Unscoped().Delete(&app.Runner{}, "id = ?", runner.ID)
		})

		// Make request
		rr := s.makeRequest(http.MethodGet, "/v1/orgs/admin-get?name="+org.Name)
		...
		// Validate all relations are preloaded
		require.Equal(s.T(), "full-preload-org", response.Name)

		// Verify CreatedBy preloaded
		require.NotNil(s.T(), response.CreatedBy, "CreatedBy should be preloaded")
		require.Equal(s.T(), "full-creator@example.com", response.CreatedBy.Email)

		// Verify RunnerGroup preloaded
		require.NotNil(s.T(), response.RunnerGroup, "RunnerGroup should be preloaded")
		require.Equal(s.T(), org.ID, response.RunnerGroup.OwnerID)

		// Verify Runners preloaded
		require.NotNil(s.T(), response.RunnerGroup.Runners, "Runners should be preloaded")
		require.Len(s.T(), response.RunnerGroup.Runners, 1)
		require.Equal(s.T(), "full-runner", response.RunnerGroup.Runners[0].Name)
	})
}

func (s *AdminGetOrgTestSuite) TestAdminGetOrgQueryMethods() {
	s.Run("exact match takes precedence over LIKE match", func() {
		ctx := context.Background()
		ctx = cctx.SetAccountContext(ctx, s.testAcc)

		// Create org with "test" in name
		org1 := &app.Org{
			ID:   domains.NewOrgID(),
			Name: "test-org-partial",
			NotificationsConfig: app.NotificationsConfig{
				InternalSlackWebhookURL: "https://hooks.slack.com/test1",
			},
		}
		...
		// Create org with exact name "test"
		org2 := &app.Org{
			ID:   domains.NewOrgID(),
			Name: "test",
			NotificationsConfig: app.NotificationsConfig{
				InternalSlackWebhookURL: "https://hooks.slack.com/test2",
			},
		}
		...
		// Query for "test" - should return exact match first
		rr := s.makeRequest(http.MethodGet, "/v1/orgs/admin-get?name=test")
		...
		require.Equal(s.T(), "test", response.Name)
		require.Equal(s.T(), org2.ID, response.ID)
	})
}
