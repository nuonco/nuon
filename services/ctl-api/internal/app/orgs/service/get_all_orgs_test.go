...
func (s *GetAllOrgsTestSuite) TestGetAllOrgs() {
	testCases := []struct {
		...
		{
			name: "returns all orgs with default type=real",
			setupFunc: func() []string {
				ctx := context.Background()
				ctx = cctx.SetAccountContext(ctx, s.testAcc)

				// Create orgs with default/real type
				org1 := &app.Org{
					ID:      domains.NewOrgID(),
				...
				org2 := &app.Org{
					...
					OrgType: app.OrgTypeLegacy, // "real"
					NotificationsConfig: app.NotificationsConfig{
						InternalSlackWebhookURL: "https://hooks.slack.com/foo",
					},
				}
				// Create org with different type (should be filtered out)
				org3 := &app.Org{
					ID:      domains.NewOrgID(),
					Name:    "test-sandbox-org",
				...
				// Default type=real should filter to only "real" OrgType
				return []string{org2.ID}
			},
			queryParams:   "", // Default type=real
			expectedCount: 1,
			expectedCode:  http.StatusOK,
			validateFunc: func(orgs []app.Org) {
				require.Equal(s.T(), "test-legacy-org", orgs[0].Name)
				require.Equal(s.T(), app.OrgTypeLegacy, orgs[0].OrgType)
			},
		},
		{
			name: "filters by type=sandbox",
			setupFunc: func() []string {
				ctx := context.Background()
				ctx = cctx.SetAccountContext(ctx, s.testAcc)
			...
			validateFunc: func(orgs []app.Org) {
				for _, org := range orgs {
					require.Equal(s.T(), app.OrgTypeSandbox, org.OrgType)
				}
			},
		},
		{
			name: "filters by type=integration",
			setupFunc: func() []string {
				ctx := context.Background()
				ctx = cctx.SetAccountContext(ctx, s.testAcc)
				...
				return []string{org1.ID}
			},
			queryParams:   "?type=integration",
			expectedCount: 1,
			expectedCode:  http.StatusOK,
			validateFunc: func(orgs []app.Org) {
				require.Equal(s.T(), "integration-org-1", orgs[0].Name)
				require.Equal(s.T(), app.OrgTypeIntegration, orgs[0].OrgType)
			},
		},
		{
			name: "filters by type=default",
			setupFunc: func() []string {
				ctx := context.Background()
				ctx = cctx.SetAccountContext(ctx, s.testAcc)
			...
			validateFunc: func(orgs []app.Org) {
				for _, org := range orgs {
					require.Equal(s.T(), app.OrgTypeDefault, org.OrgType)
				}
			},
		},
		{
			name: "returns all types when type=all",
			setupFunc: func() []string {
				ctx := context.Background()
				ctx = cctx.SetAccountContext(ctx, s.testAcc)
				...
				return []string{org1.ID, org2.ID, org3.ID}
			},
			queryParams:   "?type=all",
			expectedCount: 3,
			expectedCode:  http.StatusOK,
			validateFunc: func(orgs []app.Org) {
				types := make(map[app.OrgType]bool)
				for _, org := range orgs {
					types[org.OrgType] = true
				}
				require.True(s.T(), types[app.OrgTypeSandbox])
				require.True(s.T(), types[app.OrgTypeIntegration])
				require.True(s.T(), types[app.OrgTypeDefault])
			},
		},
		{
