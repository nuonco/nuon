...
func (h *Helpers) GetRecordedAppConfig(ctx context.Context, orgID, appID, appConfigID string) (*app.AppConfig, error) {
	...
	res := h.db.WithContext(ctx).
		...
		First(&appConfig)
	if res.Error != nil {
		return nil, fmt.Errorf("unable to get recorded app config for release definitions: %w", res.Error)
	}

	var state configsync.State
	if err := json.Unmarshal([]byte(appConfig.State), &state); err != nil {
		return nil, fmt.Errorf("decode recorded app config state: %w", err)
	}
...
func (h *Helpers) loadComponentConfigConnections(ctx context.Context, orgID string, components []configsync.ComponentState) ([]app.ComponentConfigConnection, error) {
	...
	for _, c := range missing {
		...
		if err != nil {
			return nil, fmt.Errorf("load recorded component config %s for release definitions: %w", c.ConfigID, err)
		}
		...
		res := h.db.WithContext(ctx).
			...
			First(&connection)
		if res.Error != nil {
			return nil, fmt.Errorf("load recorded component config %s for release definitions: %w", c.ConfigID, res.Error)
		}
