...
func (s *Signal) Execute(ctx workflow.Context) error {
	...
	appConfig, err := activities.AwaitGetAppConfigByID(ctx, s.AppConfigID)
	if err != nil {
	...
	l.Info("triggering component builds",
		...
		"component_count", len(appConfig.Components))

	// Trigger builds for each component
	for _, component := range appConfig.Components {
		if err := activities.AwaitTriggerComponentBuild(ctx, component.ID); err != nil {
