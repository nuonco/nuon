...
func (s *service) GetAppInputLatestConfig(ctx *gin.Context) {
	...
	if len(app.AppInputConfigs) < 1 {
		ctx.Error(fmt.Errorf("no app input configs found for app: %w", gorm.ErrRecordNotFound))
		return
	}

	ctx.JSON(http.StatusOK, app.AppInputConfigs[0])
}
