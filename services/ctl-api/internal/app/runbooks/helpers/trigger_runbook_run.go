...
func (h *Helpers) TriggerRunbookRun(ctx context.Context, req TriggerRunbookRunRequest) (*TriggerRunbookRunResponse, error) {
	...
	err := h.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		resp, err := h.queueClient.EnqueueSignalInTransaction(ctx, tx, &queueclient.EnqueueSignalRequest{QueueID: q.ID, Signal: executeflow.NewSignal(workflow.ID), OwnerID: workflow.ID, OwnerType: "install_workflows", DedupeKey: &dedupe, Callback: req.Callback})
		if err != nil {
			return err
		}
