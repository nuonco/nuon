...
func (s *service) verifyBearerJWT(ctx *gin.Context, source *app.EventSource) error {
	...
	provider, err := s.jwtProvider(source.AuthConfig.Issuer)
	if err != nil {
		return err
	}
	tokenValidator, err := validator.New(
		provider.KeyFunc,
		validator.RS256,
		source.AuthConfig.Issuer,
		source.AuthConfig.Audience,
		validator.WithAllowedClockSkew(time.Minute),
	)
	if err != nil {
		return fmt.Errorf("create JWT validator: %w", err)
	}
	_, err = tokenValidator.ValidateToken(ctx, token)
	return err
}
