...
// services/ctl-api/internal/app/installs/helpers, duplicated as a literal to
// avoid an import cycle (helpers imports signals via fx wiring).
...
// notify enqueues one notification-carrier signal per debounced crossing.
// Enqueue happens here rather than in the evaluator activity because only a
// workflow context can enqueue, and it targets the install-signals queue so
// notifications never queue behind the next health evaluation (the
// component-health queue runs one signal at a time).
//
// A failed enqueue is logged and skipped rather than propagated: the verdicts
// are already committed, and failing the signal would re-run the whole
// evaluation, which by then sees no transition and so would never retry the
// notification anyway.
