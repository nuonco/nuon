package installstackrunsucceeded
...
// SignalType is the queue signal type for an install stack run succeeded
// notification. Fired from the SDK-facing UpdateInstallStackVersionRun
// endpoint when a run transitions to succeeded. Notification-only.
const SignalType signal.SignalType = "install-stack-run-succeeded"
