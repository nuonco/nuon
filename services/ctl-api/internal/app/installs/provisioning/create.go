...
func Prepare(ctx context.Context, installsHelpers *helpers.Helpers, db *gorm.DB, appID string, params *helpers.CreateInstallParams) (*app.Install, *app.Workflow, error) {
	...
	return prepareWorkflow(ctx, installsHelpers, db, install, nil)
}

func PrepareFromRelease(ctx context.Context, installsHelpers *helpers.Helpers, db *gorm.DB, release *app.AppRelease, params *helpers.CreateInstallParams) (*app.Install, *app.Workflow, error) {
	install, err := installsHelpers.CreateInstallFromRelease(ctx, release, params)
	if err != nil {
		return nil, nil, err
	}
	componentBuildIDs, err := json.Marshal(release.ComponentBuildIDs)
	if err != nil {
		return nil, nil, fmt.Errorf("marshal release component builds: %w", err)
	}
	metadata := map[string]string{
		"app_release_id":              release.ID,
		"release_component_build_ids": string(componentBuildIDs),
		"release_sandbox_build_id":    release.SandboxBuildID,
	}
	return prepareWorkflow(ctx, installsHelpers, db, install, metadata)
}

func prepareWorkflow(ctx context.Context, installsHelpers *helpers.Helpers, db *gorm.DB, install *app.Install, metadata map[string]string) (*app.Install, *app.Workflow, error) {
	if metadata == nil {
		metadata = map[string]string{}
	}
	workflow, err := installsHelpers.CreateWorkflow(ctx, install.ID, app.WorkflowTypeProvision, metadata, false)
	if err != nil {
