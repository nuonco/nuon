...
// installStackVersionConfigResponse mirrors the create-run response's config
// block, but carries no run — this is a read-only fetch used by the Terraform
...
// @Description			return the rendered install-stack configuration (runner, permissions, inputs, secrets) for a stack version. Read-only and side-effect free — unlike create-run it does not start a run or change status. Public endpoint: the per-stack-version phone_home_id in the URL is the secret. Consumed by the Terraform provider's nuon_stack data source.
...
func (s *service) GetInstallStackVersionConfig(ctx *gin.Context) {
	...
	// Merge the install's latest stored input values, same as create-run: a
	// missing inputs row is not an error.
