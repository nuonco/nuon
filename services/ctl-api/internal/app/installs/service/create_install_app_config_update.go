...
func (s *service) CreateInstallAppConfigUpdate(ctx *gin.Context) {
	...
	update, _, err := s.helpers.StartInstallAppConfigUpdate(ctx, helpers.StartInstallAppConfigUpdateInput{
		InstallID: install.ID, NewAppConfigID: req.AppConfigID, PlanOnly: req.PlanOnly,
	})
	if err != nil {
