...
// @Description			Derives the install's cluster access from its current stack outputs and the chosen role, then stores it for the runner's health engine. Use when health reports unknown because the install has not been deployed since the cluster's endpoint or role changed. The runner picks the refreshed access up within a minute.
