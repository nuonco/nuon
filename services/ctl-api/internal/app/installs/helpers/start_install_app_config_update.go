...
func (h *Helpers) StartInstallAppConfigUpdate(ctx context.Context, input StartInstallAppConfigUpdateInput) (*app.InstallAppConfigVersion, *app.Workflow, error) {
	...
	if input.PolicyVersionID != "" {
		update.PolicyVersionID = generics.ToPtr(input.PolicyVersionID)
	}
	if input.AppBranchRunID != "" {
	...
	workflow, err := h.CreateWorkflow(ctx, input.InstallID, app.WorkflowTypeAppBranchConfigUpdate, metadata, input.PlanOnly)
	if err != nil {
