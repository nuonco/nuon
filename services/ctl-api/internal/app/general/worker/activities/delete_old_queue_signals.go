...
// DeleteOldQueueSignals hard-deletes a single batch of process_healthcheck queue
// signals older than the retention window, returning the rows removed so the
// caller can loop until a batch comes back smaller than BatchSize. It no-ops
// (returns 0) unless QueueSignalCleanupEnabled is set.
//
...
func (a *Activities) DeleteOldQueueSignals(ctx context.Context, req DeleteOldQueueSignalsRequest) (*DeleteOldQueueSignalsResponse, error) {
	...
	sub := a.db.Model(&app.QueueSignal{}).
		Unscoped().
		Select("id").
		Where(&app.QueueSignal{Type: processhealthcheck.SignalType}).
