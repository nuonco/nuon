...
func canonicalBundleRunbooks(runbooks []pkgReleases.RunbookTemplate) ([]pkgReleases.RunbookTemplate, string, error) {
	if len(runbooks) == 0 {
		return nil, "", nil
	}
	canonical := append([]pkgReleases.RunbookTemplate(nil), runbooks...)
...
func (s *service) ActiveBuildForConnection(ctx context.Context, orgID, buildID string, connection app.ComponentConfigConnection) (app.ComponentBuild, error) {
	return s.activeBuildForConnection(ctx, orgID, buildID, connection)
}
...
func (s *service) resolveReleasedSandboxBuild(ctx context.Context, orgID, appID string, sandboxConfig app.AppSandboxConfig) (app.AppSandboxBuild, bool, error) {
	definition, err := CanonicalObject(sandboxConfig)
	if err != nil {
	...
	if err := s.db.WithContext(ctx).
		Preload("Release").
		Where(app.AppReleaseMember{OrgID: orgID, Kind: "sandbox", ConfigDigest: ObjectDigest(definition)}).
