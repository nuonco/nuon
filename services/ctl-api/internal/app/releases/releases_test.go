...
import (
	...
	releasearchive "github.com/nuonco/nuon/pkg/releases"
...
func TestSemanticReleaseDigestIsCanonicalAndPackageIndependent(t *testing.T) {
	...
	digest := semanticReleaseDigest("config-1", members)
	require.Equal(t, digest, semanticReleaseDigest("config-1", []app.AppReleaseMember{members[1], members[0]}))
	require.NotEqual(t, digest, semanticReleaseDigest("config-2", members))
}
