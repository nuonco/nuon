...
func (s *service) createRelease(ctx context.Context, orgID, appID, appConfigID string, requestedRunbooks []customermanaged.RunbookTemplate) (*app.AppRelease, bool, error) {
	cfg, err := s.appsHelpers.GetCustomerManagedAppConfig(ctx, orgID, appID, appConfigID)
	if err != nil {
...
func CreateAppRelease(ctx context.Context, database *gorm.DB, appsHelpers *appshelpers.Helpers, blobSvc blobstore.Service, orgID, appID, appConfigID string, runbooks []customermanaged.RunbookTemplate) (*app.AppRelease, bool, error) {
	return (&service{
		db:          database,
...
func (s *service) appReleasesEnabled(ctx *gin.Context) bool {
	if s.features == nil {
		ctx.Error(fmt.Errorf("app releases feature client is unavailable"))
		return false
	}
	enabled, err := s.features.FeatureEnabled(ctx, app.OrgFeatureAppReleases)
	if err != nil {
		ctx.Error(fmt.Errorf("unable to check app releases feature: %w", err))
		return false
	}
	if !enabled {
		ctx.Error(features.ErrFeatureNotEnabled(app.OrgFeatureAppReleases))
		return false
	}
	return true
}
