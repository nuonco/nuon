...
import (
	...
	pkgReleases "github.com/nuonco/nuon/pkg/releases"
	bundle "github.com/nuonco/nuon/pkg/releases/bundle"
...
func (s *service) createRelease(ctx context.Context, orgID, appID, appConfigID string, requestedRunbooks []pkgReleases.RunbookTemplate) (*app.AppRelease, bool, error) {
	cfg, err := s.appsHelpers.GetRecordedAppConfig(ctx, orgID, appID, appConfigID)
	if err != nil {
	...
	runbooks, _, err := canonicalBundleRunbooks(requestedRunbooks)
	if err != nil {
	...
	semanticDigest := semanticReleaseDigest(cfg.ID, members)
	...
	if sourceArchive != nil {
		release.SourceFiles = (pkgReleases.ReleaseArchive{Files: sourceArchive.Files}).FileList()
	}
	return &release, true, nil
}

func CreateAppReleaseForConfig(ctx context.Context, database *gorm.DB, appsHelpers *appshelpers.Helpers, blobSvc blobstore.Service, orgID, appID, appConfigID string) (*app.AppRelease, error) {
	release, _, err := CreateAppRelease(ctx, database, appsHelpers, blobSvc, orgID, appID, appConfigID, nil)
	return release, err
}

func CreateAppRelease(ctx context.Context, database *gorm.DB, appsHelpers *appshelpers.Helpers, blobSvc blobstore.Service, orgID, appID, appConfigID string, runbooks []pkgReleases.RunbookTemplate) (*app.AppRelease, bool, error) {
	return (&service{
		db:          database,
		appsHelpers: appsHelpers,
		blobSvc:     blobSvc,
	}).createRelease(ctx, orgID, appID, appConfigID, runbooks)
}
...
func newReleaseDefinitionsBlob(members []app.AppReleaseMember, authored map[string]string, sourceArchive *pkgconfig.SourceArchive) (*blobstore.Blob, error) {
	definitions := pkgReleases.ReleaseArchive{SchemaVersion: 2, Members: make(map[string]string, len(members))}
	if sourceArchive != nil {
...
func appendAuthoredReleaseMembers(members []app.AppReleaseMember, archive *pkgconfig.SourceArchive) ([]app.AppReleaseMember, error) {
	...
	for key, path := range archive.Members {
		...
		digest := ObjectDigest(definition)
...
func (s *service) hydrateReleaseDefinitions(ctx context.Context, release *app.AppRelease) error {
	...
	var definitions pkgReleases.ReleaseArchive
...
func (s *service) releaseMembers(ctx context.Context, orgID string, cfg *app.AppConfig, selection bundleBuildSelection, runbooks []pkgReleases.RunbookTemplate) ([]app.AppReleaseMember, error) {
	...
	for _, connection := range cfg.ComponentConfigConnections {
		...
		build, err := s.activeBuildForConnection(ctx, orgID, buildID, connection)
		if err != nil {
			return nil, err
		}
		...
		members = append(members, app.AppReleaseMember{
			...
			ComponentID: connection.ComponentID, BuildID: build.ID, ConfigDigest: ObjectDigest(definition), ConfigTOML: configTOML,
			ContentDigest: ObjectDigest(map[string]string{"build_id": build.ID, "source_digest": build.SourceDigest, "source_checksum": build.SourceChecksum}),
	...
	sandboxDefinition, err := CanonicalObject(cfg.SandboxConfig)
	if err != nil {
	...
	members = append(members, app.AppReleaseMember{
		...
		BuildID: selection.sandboxBuildID, ConfigDigest: ObjectDigest(sandboxDefinition), ConfigTOML: sandboxTOML,
		ContentDigest: ObjectDigest(map[string]string{"build_id": selection.sandboxBuildID}), SourceType: cfg.SandboxConfig.Type,
	})
	for _, actionConfig := range cfg.ActionWorkflowConfigs {
		...
		definition := CanonicalActionDefinition(actionConfig, cfg.ComponentConfigConnections)
		digest := ObjectDigest(definition)
		configTOML, err := canonicalDefinitionTOML(definition)
		if err != nil {
	...
	for _, runbookConfig := range runbookConfigs {
		...
		definition := CanonicalRunbookDefinition(runbookConfig, cfg.ActionWorkflowConfigs)
		digest := ObjectDigest(definition)
		configTOML, err := canonicalDefinitionTOML(definition)
		if err != nil {
...
func semanticReleaseDigest(appConfigID string, members []app.AppReleaseMember) string {
	...
	return ObjectDigest(map[string]any{"schema_version": bundle.CurrentSchemaVersion, "app_config_id": appConfigID, "members": identity})
}
...
func (s *service) GetReleaseCore(ctx context.Context, orgID, appID, releaseID string) (*app.AppRelease, error) {
	return s.getRelease(ctx, orgID, appID, releaseID)
}
...
func (s *service) getReleaseFileContent(ctx context.Context, orgID, appID, releaseID, path string) (*ReleaseFileContentResponse, error) {
	...
	var archive pkgReleases.ReleaseArchive
	if err := json.Unmarshal([]byte(raw), &archive); err != nil {
...
func (s *service) GetReleaseFileContentCore(ctx context.Context, orgID, appID, releaseID, path string) (*ReleaseFileContentResponse, error) {
	return s.getReleaseFileContent(ctx, orgID, appID, releaseID, path)
}
