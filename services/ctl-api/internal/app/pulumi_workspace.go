...
func (r *PulumiWorkspace) Indexes(db *gorm.DB) []migrations.Index {
	return []migrations.Index{
		{
			Name: indexes.Name(db, &PulumiWorkspace{}, "org_id"),
			Columns: []string{
				"org_id",
			},
		},
...
func (r *PulumiWorkspace) BeforeCreate(tx *gorm.DB) (err error) {
	if r.ID == "" {
		r.ID = domains.NewPulumiWorkspaceID()
	}
