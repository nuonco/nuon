...
func TestNewStoreReturnsDisabledStoreWithoutBucket(t *testing.T) {
	cfg := ctlconfig.Config{BlobStorageBucket: "blob-bucket", BlobStorageRegion: "blob-region", AirgapBundleGrantTTL: time.Minute}
...
func TestNewS3RejectsInvalidResolvedConfiguration(t *testing.T) {
	base := ctlconfig.Config{AirgapBundleStorageBucket: "bucket", AirgapBundleStorageRegion: "region", AirgapBundleGrantTTL: time.Minute}
	tests := []struct {
		name   string
		mutate func(*ctlconfig.Config)
	}{
		{name: "bucket", mutate: func(c *ctlconfig.Config) { c.AirgapBundleStorageBucket = "" }},
		{name: "region", mutate: func(c *ctlconfig.Config) { c.AirgapBundleStorageRegion = "" }},
		{name: "region not inherited from blob storage", mutate: func(c *ctlconfig.Config) {
			c.AirgapBundleStorageRegion = ""
			c.BlobStorageRegion = "blob-region"
		}},
		{name: "ttl", mutate: func(c *ctlconfig.Config) { c.AirgapBundleGrantTTL = 0 }},
		{name: "ttl over seven days", mutate: func(c *ctlconfig.Config) { c.AirgapBundleGrantTTL = 7*24*time.Hour + time.Second }},
		{name: "endpoint scheme", mutate: func(c *ctlconfig.Config) { c.AirgapBundleStorageEndpoint = "ftp://host/path" }},
		{name: "endpoint host", mutate: func(c *ctlconfig.Config) { c.AirgapBundleStorageEndpoint = "https:///path" }},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			cfg := base
