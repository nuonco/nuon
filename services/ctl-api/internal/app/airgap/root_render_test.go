...
func TestRenderConfigForStackCompileRendersCloudAccountRegion(t *testing.T) {
	...
	rendered, err := renderConfigForStackCompile(cfg, "vinst0123456789abcdef", nil)
	require.NoError(t, err)
	require.Contains(t, string(rendered.PermissionsConfig.Roles[0].Policies[0].Contents), "arn:aws:secretsmanager:__NUON_AIRGAP_STACK_region__::secret:rds!*")
}
...
func TestRenderConfigForStackCompileSeedsInputPlaceholders(t *testing.T) {
	...
	rendered, err := renderConfigForStackCompile(cfg, "vinst0123456789abcdef", []runnerairgap.InputSpec{{Name: "env"}})
	require.NoError(t, err)
	require.Equal(t, "stack-"+runnerairgap.InputPlaceholder("env"), rendered.StackConfig.Name)
}
