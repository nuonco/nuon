...
type BundlePins struct {
	...
	// air-gapped runner serves the source from the bundle by tag and never
...
func RewriteEnvelopeForBundle(ctx context.Context, db *gorm.DB, envelope *runnerairgap.Envelope, pins BundlePins) error {
...
func componentIDForBuild(ctx context.Context, db *gorm.DB, orgID, buildID string, cache map[string]string) (string, error) {
	...
	if err := db.WithContext(ctx).Where(app.ComponentBuild{ID: buildID, OrgID: orgID}).First(&build).Error; err != nil {
		return "", fmt.Errorf("sync-oci source tag %s is not a component build in this org; airgap bundles can only serve component-build sources: %w", buildID, err)
	}
