...
// @Router /v1/apps/{app_id}/airgap-bundles/{bundle_id}/blob-grants [post]
func (s *service) CreateBlobGrants(ctx *gin.Context) {
	...
	if err != nil {
		...
		ctx.Error(fmt.Errorf("unable to create air-gap bundle blob grants: %w", err))
		return
	}
...
func (s *service) createBlobGrants(ctx context.Context, orgID, appID, bundleID string, digests []string) (*blobGrantsResponse, error) {
	...
	if bundle.Status != app.AirgapBundleStatusActive {
		return nil, conflictError{message: "air-gap bundle is not active"}
	}
	if bundle.OCIIndexDigest == "" {
		return nil, conflictError{message: "air-gap bundle was published before differential downloads were supported; republish the bundle to enable blob grants"}
	}
