...
func (*fakeStore) Publish(context.Context, transport.PublishRequest) (transport.Replica, error) {
...
func testService(t *testing.T) (*service, *fakeStore) {
	...
	return &service{db: database, store: store}, store
}

func seedBundle(t *testing.T, svc *service, id, orgID, appID, digest string, createdAt time.Time) app.AirgapBundle {
	t.Helper()
	bundle := app.AirgapBundle{
		...
		Status: app.AirgapBundleStatusActive, StatusDescription: "bundle published and verified",
	}
...
func TestListAndGetIsolateTenantAndExcludeReplicas(t *testing.T) {
	...
	require.NoError(t, svc.db.Create(&app.AirgapBundleArtifact{ID: strings.Repeat("5", 26), OrgID: "org-a", BundleID: wanted.ID, Kind: "image", LogicalName: "api", Digest: digest, MediaType: "application/test", Size: 4}).Error)
	require.NoError(t, svc.db.Create(&app.AirgapBundleTransportReplica{ID: strings.Repeat("6", 26), CreatedAt: now, OrgID: "org-a", BundleID: wanted.ID, Provider: "secret-provider", StorageRef: "secret-ref", StorageVersion: "v1", TransportChecksum: digest, Size: 42, VerifiedAt: &now}).Error)
...
func TestCreateDownloadGrantSelectsNewestVerifiedReplica(t *testing.T) {
	...
	newVerified := now
	for _, replica := range []app.AirgapBundleTransportReplica{
...
func TestCreateDownloadGrantFailsWithoutVerifiedReplica(t *testing.T) {
	...
	digest := strings.Repeat("a", 64)
	bundle := seedBundle(t, svc, strings.Repeat("1", 26), "org-a", "app-a", digest, now)
	require.NoError(t, svc.db.Create(&app.AirgapBundleTransportReplica{ID: strings.Repeat("2", 26), CreatedAt: now, OrgID: "org-a", BundleID: bundle.ID, Provider: "test", StorageRef: "ref", StorageVersion: "v", TransportChecksum: digest, Size: 42}).Error)
...
func TestMalformedManifestDigestIsDataError(t *testing.T) {
	...
	digest := strings.Repeat("a", 64)
	require.NoError(t, svc.db.Create(&app.AirgapBundleTransportReplica{ID: strings.Repeat("2", 26), CreatedAt: now, OrgID: "org-a", BundleID: bundle.ID, Provider: "test", StorageRef: "ref", StorageVersion: "v", TransportChecksum: digest, Size: 42, VerifiedAt: &now}).Error)
	_, err := svc.createDownloadGrant(context.Background(), "org-a", "app-a", bundle.ID)
