...
func TestCompileStateNestsNuonAndInjectsInputPlaceholders(t *testing.T) {
	...
	_, data, err := compileState("org-test", "app-test", "vinst-test", raw, []runnerairgap.InputSpec{{Name: "region"}}, nil, false, nil, nil, "")
	require.NoError(t, err)
	...
	require.Equal(t, "__NUON_AIRGAP_STACK_vpc_id__", nuon["install_stack"].(map[string]any)["outputs"].(map[string]any)["vpc_id"])
	require.Equal(t, "__NUON_AIRGAP_SANDBOX_endpoint__", nuon["sandbox"].(map[string]any)["outputs"].(map[string]any)["endpoint"])
	installSandboxOutputs := install["sandbox"].(map[string]any)["outputs"].(map[string]any)
	require.Equal(t, "__NUON_AIRGAP_SANDBOX_nuon_dns__", installSandboxOutputs["nuon_dns"])
	require.Equal(t, "__NUON_AIRGAP_SANDBOX_endpoint__", installSandboxOutputs["endpoint"])
}
...
func TestCompileStateSeedsBuiltinStackOutputsWithoutReferences(t *testing.T) {
	...
	for _, key := range builtinStackOutputKeys {
		require.Equal(t, "__NUON_AIRGAP_STACK_"+key+"__", outputs[key], key)
	}
}
...
func TestCompileStateSeedsCloudAccountByStackType(t *testing.T) {
	...
	require.Equal(t, "__NUON_AIRGAP_STACK_region__", cloud["aws"].(map[string]any)["region"])
	require.Nil(t, cloud["azure"])
	require.Nil(t, cloud["gcp"])

	out, err := render.RenderTextV2(`{{ .nuon.cloud_account.aws.region }}`, data)
	require.NoError(t, err)
	require.Equal(t, "__NUON_AIRGAP_STACK_region__", out)
	...
	require.Equal(t, "__NUON_AIRGAP_STACK_resource_group_location__", cloud["azure"].(map[string]any)["location"])

	_, data, err = compileState("org-test", "app-test", "vinst-test", []byte(`{}`), nil, nil, false, nil, nil, app.StackTypeGCP)
	require.NoError(t, err)
	cloud = data["nuon"].(map[string]any)["cloud_account"].(map[string]any)
	require.Nil(t, cloud["aws"])
	require.Equal(t, "__NUON_AIRGAP_STACK_project_id__", cloud["gcp"].(map[string]any)["project_id"])
	require.Equal(t, "__NUON_AIRGAP_STACK_region__", cloud["gcp"].(map[string]any)["region"])
}
...
func TestCompileStateSeedsRoleARNMapsFromRenderedNames(t *testing.T) {
	...
	require.Equal(t, map[string]any{
		"vinst-test-sandbox-break-glass": "__NUON_AIRGAP_STACK_break_glass_role_arns_0__",
	}, outputs["break_glass_role_arns"])
	require.Equal(t, map[string]any{
		"vinst-test-custom": "__NUON_AIRGAP_STACK_custom_role_arns_0__",
...
func TestCompileStateRoleARNMapsSupportTemplateMapOperations(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, "__NUON_AIRGAP_STACK_break_glass_role_arns_0__", out)
...
func TestCompileStackSuppliesAllAWSRolePlaceholders(t *testing.T) {
	...
	require.Equal(t, "__NUON_AIRGAP_STACK_provision_iam_role_arn__", aws.ProvisionIAMRoleARN)
	require.Equal(t, "__NUON_AIRGAP_STACK_deprovision_iam_role_arn__", aws.DeprovisionIAMRoleARN)
	require.Equal(t, "__NUON_AIRGAP_STACK_maintenance_iam_role_arn__", aws.MaintenanceIAMRoleARN)
...
func TestCompileStateSeedsNestedSandboxOutputPlaceholders(t *testing.T) {
	...
	require.Equal(t, "__NUON_AIRGAP_SANDBOX_nuon_dns_public_domain_name__", publicDomain["name"])
	require.Equal(t, "__NUON_AIRGAP_SANDBOX_nuon_dns_public_domain_zone_id__", publicDomain["zone_id"])
	require.Equal(t, "__NUON_AIRGAP_SANDBOX_endpoint__", outputs["endpoint"])
}
...
func TestCompileStateSeedsComponentOutputPlaceholders(t *testing.T) {
	...
	require.Equal(t, runnerairgap.ComponentOutputPlaceholder("certificate", "public_domain_certificate_arn"), cert["public_domain_certificate_arn"])
	lambda := components["lambda_function"].(map[string]any)["outputs"].(map[string]any)["lambda_function"].(map[string]any)
	require.Equal(t, runnerairgap.ComponentOutputPlaceholder("lambda_function", "lambda_function.lambda_function_arn"), lambda["lambda_function_arn"])
}
...
func TestCompileStateKeepsComponentPlaceholdersOutOfEmbeddedState(t *testing.T) {
	...
	for _, ref := range refs {
		token := runnerairgap.ComponentOutputPlaceholder(ref.Component, ref.Path)
...
func TestCompileDeployPlanClusterInfo(t *testing.T) {
	...
	t.Run("cluster sandbox carries cluster info with cloud auth", func(t *testing.T) {
		...
		require.Equal(t, "__NUON_AIRGAP_CLUSTER_id__", info.ID)
		require.Equal(t, "__NUON_AIRGAP_CLUSTER_endpoint__", info.Endpoint)
		require.Equal(t, "__NUON_AIRGAP_CLUSTER_ca_data__", info.CAData)
