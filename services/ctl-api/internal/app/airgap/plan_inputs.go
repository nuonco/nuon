...
func rewriteInputPlaceholders(ctx context.Context, db *gorm.DB, envelope *runnerairgap.Envelope) error {
	...
	for i := range envelope.Inputs {
		...
		if spec.Secret {
			if len(value) >= 6 && containsInputValue(plans, value) {
				return fmt.Errorf("input %q is a secret and its value is baked into the exported plans; air-gapped bundles cannot ship or accept secrets", spec.Name)
			}
			continue
		}
	...
	for _, reference := range references {
		if previous, ok := seen[reference.value]; ok {
			return fmt.Errorf("inputs %q and %q have the same reference value; air-gapped bundle inputs must be disambiguated", previous, reference.name)
		}
	...
	for _, reference := range references {
		placeholder := runnerairgap.InputPlaceholder(reference.name)
