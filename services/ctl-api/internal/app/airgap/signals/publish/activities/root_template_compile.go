...
func (a *Activities) compileRootTemplate(ctx context.Context, cfg *app.AppConfig, syntheticInstallID string) ([]byte, string, error) {
	if syntheticInstallID == "" {
		return nil, "", fmt.Errorf("synthetic install ID is required")
	}

	cfg, err := airgap.RenderConfigForStackCompile(ctx, a.db, cfg, syntheticInstallID)
	if err != nil {
	...
	input := &stacks.TemplateInput{
		...
		Settings:                     settings,
		APIToken:                     "compiled",
		RunnerEnvVars:                stacks.FormatRunnerEnvVars(&cfg.RunnerConfig, a.cfg.RunnerContainerImageTag),
