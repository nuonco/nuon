...
func exportActionTemplates(ctx context.Context, db *gorm.DB, blobRead bool, installID string, report *QualificationReport) ([]runnerairgap.ActionTemplate, error) {
	...
	for _, job := range jobs {
		...
		if gitStep != "" {
			if report != nil {
				report.Warnings = append(report.Warnings, Finding{Code: "action.git_source_excluded", Member: "runner_job:" + job.ID, Message: fmt.Sprintf("action was excluded from the bundle: step %s sources its contents from Git, and air-gap v1 actions must be inline-only", gitStep)})
			}
			continue
		}
	...
	sort.Strings(ids)
	templates := make([]runnerairgap.ActionTemplate, 0, len(ids))
	for _, id := range ids {
		item := selected[id]
		cronSchedule := ""
		...
		templates = append(templates, runnerairgap.ActionTemplate{ID: id, Name: item.workflow.Name, CronSchedule: cronSchedule, JobType: string(item.job.Type), JobGroup: string(item.job.Group), JobOperation: string(item.job.Operation), CompositePlan: item.plan})
	}
...
func exportDriftTemplates(ctx context.Context, db *gorm.DB, steps []runnerairgap.Step, components []runnerairgap.ComponentSpec) ([]runnerairgap.DriftTemplate, error) {
	componentByInstallID := make(map[string]runnerairgap.ComponentSpec, len(components))
	for _, component := range components {
		componentByInstallID[component.InstallComponentID] = component
	}
	byComponent := map[string]runnerairgap.DriftTemplate{}
	for _, step := range steps {
		...
		byComponent[component.ComponentID] = runnerairgap.DriftTemplate{ID: "drift-" + component.ComponentID, ComponentID: component.ComponentID, ComponentName: component.ComponentName, JobType: step.JobType, JobGroup: step.JobGroup, JobOperation: string(app.RunnerJobOperationTypeCreateApplyPlan), CompositePlan: plan}
	}
	...
	sort.Strings(ids)
	result := make([]runnerairgap.DriftTemplate, 0, len(ids))
	for _, id := range ids {
