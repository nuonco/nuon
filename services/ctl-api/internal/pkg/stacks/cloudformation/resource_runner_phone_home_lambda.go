...
func (a *Templates) getPhoneHomeS3Parameters() map[string]cloudformation.Parameter {
	return map[string]cloudformation.Parameter{
		phoneHomeS3BucketParam: {
			Type:        "String",
			Default:     "",
			Description: ptr("S3 bucket the phone-home Lambda writes stack outputs to, for air-gapped installs. Empty keeps the default control-plane phone-home."),
		},
		phoneHomeS3KeyParam: {
			Type:        "String",
			Default:     "",
			Description: ptr("S3 object key for the phone-home stack outputs JSON, for air-gapped installs."),
		},
	}
}
...
func (a *Templates) getRunnerPhoneHomeLambdaRole(inp *stacks.TemplateInput, t tagBuilder) *iam.Role {
	...
	// The write half of the air-gapped S3 rendezvous. IAM rejects a policy whose
