...
func residentSkipAutoRetrySteps(deployID string) []app.WorkflowStep {
	steps := residentRetrySteps(false)
	steps[0].StepTargetType = string(app.WorkflowStepTargetTypeInstallDeploys)
	steps[0].StepTargetID = deployID
	steps[0].QueueSignal = &signaldb.SignalData{Signal: &CountdownSignal{SucceedAtRetry: 1}}
	return steps
}

func (e *FlowTestSuite) waitForResidentAwaitRetry(ctx context.Context, flw *app.Workflow, stepName string) *app.WorkflowStep {
	e.waitForWorkflowParked(ctx, flw.ID)
...
func (e *FlowTestSuite) TestResidentAwaitRetryBlocksPendingSiblingInSameGroup() {
	ctx := e.service.Seed.EnsureAccount(e.T().Context(), e.T())
	ctx = e.service.Seed.EnsureOrg(ctx, e.T())
	ownerID, ownerType := newTestOwner()
	flw, queueID := e.setupGroupedFlowTest(ctx, ownerID, ownerType, []app.WorkflowStep{
		{
			Name:          "plan-awaiting-retry",
			Idx:           100,
			GroupIdx:      1,
			ExecutionType: app.WorkflowStepExecutionTypeSystem,
			Retryable:     true,
	...
	require.Eventually(e.T(), func() bool {
		steps := e.getStepsByWorkflow(ctx, flw.ID)
		for i := range steps {
			if steps[i].Name == "plan-awaiting-retry" &&
				steps[i].Status.Status == app.StatusError &&
				directive.Step(steps[i].ResultDirective) == directive.StepAwaitRetry {
				failedStep = &steps[i]
				return true
	...
	require.Never(e.T(), func() bool {
		...
		for _, step := range steps {
			if step.ID != failedStep.ID && (step.Status.Status != app.StatusPending || step.GroupRetryIdx != 0) {
				return true
			}
		}
		return false
	}, 5*time.Second, pollInterval, "pending siblings must stay blocked while the plan awaits retry")

	e.waitForQueueSignalStatus(ctx, flw.ID, "install_workflows", executeflow.SignalType, app.StatusSuccess)

	steps := e.getStepsByWorkflow(ctx, flw.ID)
	require.Len(e.T(), steps, 3, "resident scan must not clone the parked group")
	for _, step := range steps {
		if step.ID == failedStep.ID {
			continue
		}
		require.Equal(e.T(), app.StatusPending, step.Status.Status, "%s must remain blocked", step.Name)
		require.Zero(e.T(), step.GroupRetryIdx, "%s must not be a group retry", step.Name)
	}
	...
	steps = e.getStepsByWorkflow(ctx, flw.ID)
	require.Len(e.T(), steps, 4, "explicit retry should create exactly one step clone")
	for _, step := range steps {
		switch {
		case step.ID == failedStep.ID:
			require.Equal(e.T(), app.StatusDiscarded, step.Status.Status)
		case step.Name == "plan-awaiting-retry":
			require.Equal(e.T(), 1, step.RetryIndex)
			require.Equal(e.T(), app.StatusSuccess, step.Status.Status)
		default:
			require.Equal(e.T(), app.StatusSuccess, step.Status.Status, "%s should run after retry", step.Name)
		}
		require.Zero(e.T(), step.GroupRetryIdx, "single-step retry must not clone the group")
	}
}

func (e *FlowTestSuite) TestResidentColdRetryFinishesCloneBeforeSameGroupSibling() {
	ctx := e.service.Seed.EnsureAccount(e.T().Context(), e.T())
	ctx = e.service.Seed.EnsureOrg(ctx, e.T())
	ownerID, ownerType := newTestOwner()
	flw, queueID := e.setupGroupedFlowTest(ctx, ownerID, ownerType, []app.WorkflowStep{
		{
			Name:          "delayed-plan-retry",
			Idx:           100,
			GroupIdx:      1,
			ExecutionType: app.WorkflowStepExecutionTypeSystem,
			Retryable:     true,
			QueueSignal:   &signaldb.SignalData{Signal: &DelayedCloneManualRetrySignal{}},
		},
		{
			Name:          "apply-after-delayed-plan",
			Idx:           200,
			GroupIdx:      1,
			ExecutionType: app.WorkflowStepExecutionTypeSystem,
			QueueSignal:   &signaldb.SignalData{Signal: &SuccessSignal{}},
	...
	e.enqueueResidentFlow(ctx, queueID, flw, ownerID, ownerType, 2*time.Second)
	failedStep := e.waitForResidentAwaitRetry(ctx, flw, "delayed-plan-retry")
	e.waitForQueueSignalStatus(ctx, flw.ID, "install_workflows", executeflow.SignalType, app.StatusSuccess)
	...
	require.Eventually(e.T(), func() bool {
		return e.getStep(ctx, failedStep.ID).Status.Status == app.StatusDiscarded
	}, pollTimeout, pollInterval)
	require.Never(e.T(), func() bool {
		for _, step := range e.getStepsByWorkflow(ctx, flw.ID) {
			if step.Name != "delayed-plan-retry" && step.Status.Status != app.StatusPending {
				return true
			}
		}
		return false
	}, 3*time.Second, pollInterval, "same-group apply must not start while the retry clone is being created")

	require.NoError(e.T(), <-retryErr)
	e.waitForWorkflowStatus(ctx, flw.ID, app.StatusSuccess)
	steps := e.getStepsByWorkflow(ctx, flw.ID)
	require.Len(e.T(), steps, 4)
	for _, step := range steps {
		if step.ID == failedStep.ID {
			require.Equal(e.T(), app.StatusDiscarded, step.Status.Status)
			continue
		}
		require.Equal(e.T(), app.StatusSuccess, step.Status.Status, step.Name)
	}
}
...
func (e *FlowTestSuite) TestResidentColdRetryRewarms() {
	...
	flw, queueID := e.setupGroupedFlowTest(ctx, ownerID, ownerType, residentSkipAutoRetrySteps(deployID))

	e.enqueueResidentFlow(ctx, queueID, flw, ownerID, ownerType, 2*time.Second)
	failedStep := e.waitForResidentAwaitRetry(ctx, flw, "await-manual-retry")
	require.Equal(e.T(), true, failedStep.Status.Metadata["skip_auto_retry"])
	require.Len(e.T(), e.getStepsByWorkflow(ctx, flw.ID), 2, "skip_auto_retry should prevent an automatic retry clone")
	e.waitForQueueSignalStatus(ctx, flw.ID, "install_workflows", executeflow.SignalType, app.StatusSuccess)
...
func (e *FlowTestSuite) TestResidentColdRetryRecoversErroredHostSignal() {
	ctx := e.service.Seed.EnsureAccount(e.T().Context(), e.T())
	ctx = e.service.Seed.EnsureOrg(ctx, e.T())
	ownerID, ownerType := newTestOwner()
	flw, queueID := e.setupGroupedFlowTest(ctx, ownerID, ownerType, residentRetrySteps(true))
	...
	res := e.service.DB.WithContext(ctx).
		...
		Where(app.QueueSignal{ID: flowSignal.ID}).
		Update("status", app.CompositeStatus{
			Status:                 app.StatusError,
			StatusHumanDescription: "prior queue run overwrote completed resident signal",
		})
