...
func (s *Signal) skipStepHandler(ctx workflow.Context, req SkipStepRequest) (*SkipStepResponse, error) {
	...
	// Legacy parked flows need an explicit wakeup after a successful skip:
	// the failed run's completion callback already fired, so without this the
	// flow stays parked forever. Set unconditionally (like approve/unpause):
	// gating on awaitingResume loses skips that land between the failed run
	// unwinding and the flow parking. A skip that lands mid-run is dropped by
	// the main loop's stale-resume reset.
	if !residentAwaitRetry && skippable {
		s.resumeRunType = app.WorkflowRunTypeSkip
		s.resumeStepID = req.StepID
		s.resumeStartIdx = s.findGroupPositionForStep(ctx, req.StepID)
		s.resumeRequested = true
