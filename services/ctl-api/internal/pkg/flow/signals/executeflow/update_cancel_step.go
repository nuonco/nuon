...
// Idempotent.
func (s *Signal) finalizeCancellation(ctx workflow.Context) error {
	if err := workflowactivities.AwaitPkgWorkflowsFlowUpdateFlowFinishedAtByID(ctx, s.WorkflowID); err != nil {
		return fmt.Errorf("unable to mark workflow finished: %w", err)
	}
	if err := statusactivities.AwaitPkgStatusUpdateFlowStatus(ctx, statusactivities.UpdateStatusRequest{
		...
		return fmt.Errorf("unable to mark workflow cancelled: %w", err)
	}
	return nil
}
