...
func (a *Activities) recordTerminalReleaseUpdate(ctx context.Context, workflow app.Workflow, status app.CompositeStatus) {
	...
	if err := a.db.WithContext(ctx).Where(app.InstallAppConfigVersion{WorkflowID: &workflow.ID}).First(&update).Error; err != nil || update.AppReleaseID == nil || update.OperatingModelID == nil {
		return
	}
	var count int64
	...
	deployment := app.InstallReleaseDeployment{
		...
		InstallAppConfigVersionID: &update.ID, OperatingModelID: *update.OperatingModelID,
		Method: app.InstallDeploymentMethodVendorManaged, Actor: "vendor", Executor: "temporal",
