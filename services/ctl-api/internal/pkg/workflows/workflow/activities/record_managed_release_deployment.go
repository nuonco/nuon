...
func (a *Activities) RecordManagedReleaseDeployment(ctx context.Context, req RecordManagedReleaseDeploymentRequest) error {
	...
	return a.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		policy, err := managedInstallPolicy(tx, install, flow, finishedAt)
		if err != nil {
			return err
		}
		...
		deployment := app.InstallReleaseDeployment{
			...
			PolicyVersionID: policy.ID, Method: app.InstallDeploymentMethodNuonManaged,
			Actor: "nuon", Executor: "temporal", OperationID: flow.ID,
		...
		if err := tx.Where(app.InstallAppConfigVersion{ID: updateID, InstallID: install.ID}).First(&update).Error; err == nil {
			...
			if update.PolicyVersionID != nil {
				deployment.PolicyVersionID = *update.PolicyVersionID
			}
...
func managedInstallPolicy(tx *gorm.DB, install app.Install, flow app.Workflow, effectiveAt time.Time) (*app.InstallManagementPolicyVersion, error) {
	var policy app.InstallManagementPolicyVersion
	err := tx.Where(app.InstallManagementPolicyVersion{OrgID: install.OrgID, InstallID: install.ID}).Order("version DESC").First(&policy).Error
	if err == nil {
		return &policy, nil
	}
	if !errors.Is(err, gorm.ErrRecordNotFound) {
		return nil, fmt.Errorf("load install management policy: %w", err)
	}
	policy = app.InstallManagementPolicyVersion{
		CreatedByID: flow.CreatedByID, OrgID: install.OrgID, InstallID: install.ID, Version: 1,
		Connectivity: app.InstallConnectivityConnected, ReleaseSelection: app.InstallReleaseSelectionVendor,
		CommandAuthority: app.InstallAuthorityNuon, ApprovalAuthority: app.InstallAuthorityNuon,
		Telemetry: app.InstallTelemetryLive, EffectiveAt: effectiveAt,
	}
	if err := tx.Create(&policy).Error; err != nil {
		return nil, fmt.Errorf("create managed install policy: %w", err)
	}
	return &policy, nil
}
