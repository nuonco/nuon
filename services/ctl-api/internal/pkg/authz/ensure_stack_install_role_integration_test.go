...
// The activity behind this runs on every provision and reprovision, so a second
// call has to converge rather than pile up roles the delete path then has to reap.
func (s *EnsureStackInstallRoleTestSuite) TestEnsureIsConvergent() {
	...
	// No account in the context: this runs from a temporal activity, and
	// Role.CreatedByID is notnull.
