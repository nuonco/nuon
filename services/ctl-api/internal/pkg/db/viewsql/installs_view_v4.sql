...
install_deploys_partitioned AS (
    ...
    FROM
        install_deploys id
        JOIN install_components ic ON ic.id = id.install_component_id
    WHERE
        ic.deleted_at = 0
        AND id.deleted_at = 0
),
...
aggregated_latest_install_deploys AS (
    ...
    FROM
        install_deploys_partitioned AS ld
    WHERE
        ld.rn = 1
    GROUP BY
        (install_id)
...
FROM
    ...
    OUTER JOIN aggregated_latest_install_deploys ld ON i.id = ld.install_id
