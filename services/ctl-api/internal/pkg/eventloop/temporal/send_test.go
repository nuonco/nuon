...
func newTestSignal(id, ns string) *TestSignal {
	return &TestSignal{
		name:       id,
		ns:         ns,
...
func newTestELSignal(id, ns string) *TestELSignal {
	return &TestELSignal{
		TestSignal:       newTestSignal(id, ns),
...
func TestSend(t *testing.T) {
	...
	wrk.RegisterWorkflow(sar.Root)
	wrk.RegisterWorkflow(sar.SendAsyncWorkflow)
	wrk.RegisterWorkflow(sar.SendAndWaitWorkflow)
	wrk.RegisterWorkflow(sar.ReceiveWorkflow)
	err = wrk.Start()
	if err != nil {
...
func (o SRTestOptions) getSignal() eventloop.Signal {
	...
	return newTestSignal(o.ID, o.ReceiverNS)
}
