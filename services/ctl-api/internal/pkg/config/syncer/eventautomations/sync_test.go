...
func TestBuildRule(t *testing.T) {
	...
	cfg := &config.EventAutomationConfig{
		Name: "deploy", EventSource: "github", EventTypes: []string{"push"},
		Filters: []config.EventAutomationFilterConfig{{Op: "eq", Path: "/ref", Value: "main"}},
...
func TestConfigHashIsNormalized(t *testing.T) {
	first := &config.EventAutomationConfig{Name: "deploy", EventSource: "github", EventTypes: []string{"push", "pull_request"}, Filters: []config.EventAutomationFilterConfig{{Op: "eq", Path: "/b", Value: true}, {Op: "eq", Path: "/a", Value: 1}}, Target: &config.EventAutomationTargetConfig{Type: "app_branch_run", AppBranch: "main"}}
	second := &config.EventAutomationConfig{Name: "deploy", EventSource: "github", EventTypes: []string{"pull_request", "push"}, Filters: []config.EventAutomationFilterConfig{{Op: "eq", Path: "/a", Value: 1}, {Op: "eq", Path: "/b", Value: true}}, Target: &config.EventAutomationTargetConfig{Type: "app_branch_run", AppBranch: "main"}}
