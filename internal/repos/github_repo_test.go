...
func TestGetInstallToken(t *testing.T) {
	tests := map[string]struct {
		...
		"auth error": {
			...
			errExpected: errors.New("401 Unauthorized"),
		},
		"happy path": {
			githubInstallID: 1234567,
			githubAppID:     7654321,
			...
			githubClient: mock.NewMockedHTTPClient(
				mock.WithRequestMatchHandler(
					mock.PostAppInstallationsAccessTokensByInstallationId,
					http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
						_, err := w.Write(mock.MustMarshal(&github.InstallationToken{
							Token: toPtr("token123"),
						}))
						assert.NoError(t, err)
					}),
				),
			),
			expectedResponse: "token123",
			errExpected:      nil,
		},
	}
	...
	for name, test := range tests {
		t.Run(name, func(t *testing.T) {
			appTransport, err := gh.NewAppsTransport(test.githubClient.Transport, test.githubAppID, test.githubAppKey)
			require.NoError(t, err)

			githubRepo := NewGithubRepo(appTransport, zaptest.NewLogger(t), test.githubClient)

			r, err := githubRepo.GetInstallToken(context.Background(), test.githubInstallID)
