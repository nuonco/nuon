...
func Test_NewPrivateDockerPullBuild(t *testing.T) {
	v := validator.New()
	...
	tests := map[string]struct {
		optsFn      func() []Option
		errExpected error
		assertFn    func(*testing.T, *privateDockerPullBuild)
	}{
		"happy path": {
			optsFn: func() []Option {
				return []Option{
			...
			assertFn: func(t *testing.T, b *privateDockerPullBuild) {
				assert.Equal(t, privateImg, b.PrivateImageSource)
				assert.Equal(t, component, b.Component)
				assert.Equal(t, ecrRef, b.EcrRef)
				assert.Equal(t, waypointRef, b.WaypointRef)
			},
		},
		"missing private image source": {
			optsFn: func() []Option {
				return []Option{
					WithComponent(component),
					WithEcrRef(ecrRef),
			...
			errExpected: fmt.Errorf("private image source not provided"),
		},
	}
	...
	for name, test := range tests {
		t.Run(name, func(t *testing.T) {
			opts := test.optsFn()
			bld, err := NewPrivateDockerPullBuild(v, opts...)
...
func Test_NewPrivateDockerPullBuild_Render(t *testing.T) {
	v := validator.New()
	waypointRef := generics.GetFakeObj[*planv1.WaypointRef]()
	component := generics.GetFakeObj[*componentv1.Component]()
	ecrRef := generics.GetFakeObj[*planv1.ECRRepositoryRef]()
	...
	tests := map[string]struct {
		builderFn func(*testing.T) *privateDockerPullBuild
		assertFn  func(*testing.T, waypointv1.Hcl_Format, []byte)
	}{
		"happy path": {
			builderFn: func(t *testing.T) *privateDockerPullBuild {
				bld, err := NewPrivateDockerPullBuild(v,
					WithComponent(component),
					WithEcrRef(ecrRef),
					WithPrivateImageSource(privateImg),
					WithWaypointRef(waypointRef))
