...
func (w wkflow) provisionOdrIAM(ctx workflow.Context, req *iamv1.ProvisionIAMRequest) (string, error) {
	...
	odrPolicy, err := odrIAMPolicy(w.cfg, req.OrgId)
	if err != nil {
	...
	cdpReq := CreateIAMPolicyRequest{
		...
		PolicyName:     odrIAMName(req.OrgId),
