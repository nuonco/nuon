	...
	tests := map[string]struct {
		...
		"happy path": {
			...
			assertFn: func(t *testing.T, client k8sRoleBindingCreator) {
				...
				assert.Equal(t, req.OrgID, *cr.Subjects[0].Namespace)
				assert.Equal(t, runnerServiceAccountName(req.OrgID), *cr.Subjects[0].Name)

				// ensure role is configured correctly
				assert.Equal(t, "ClusterRole", *cr.RoleRef.Kind)
				assert.Equal(t, "edit", *cr.RoleRef.Kind)
			},
			errExpected: nil,
		},
		"error returned": {
