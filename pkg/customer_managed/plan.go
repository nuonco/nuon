...
type Envelope struct {
	...
	// Operation templates are immutable execution templates, not saved plans:
...
func (e *Envelope) Validate() error {
	...
	return e.validateOperation()
}

func (e *Envelope) validateOperation() error {
	...
	for _, action := range e.Actions {
		...
		if _, ok := refs[action.ID]; ok {
			return fmt.Errorf("duplicate operation template ID %q", action.ID)
		}
	...
	for _, drift := range e.Drift {
		...
		if _, ok := refs[drift.ID]; ok {
			return fmt.Errorf("duplicate operation template ID %q", drift.ID)
		}
	...
	for _, runbook := range e.Runbooks {
		...
		if _, ok := refs[runbook.ID]; ok {
			return fmt.Errorf("duplicate operation template ID %q", runbook.ID)
		}
