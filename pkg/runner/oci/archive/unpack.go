...
func validateArchiveManifest(ctx context.Context, fetcher content.Fetcher, descriptor ocispec.Descriptor) error {
	...
	if len(manifest.Layers) == 0 || len(manifest.Layers) > maxArchiveLayers {
		return fmt.Errorf("unsupported OCI archive layer count %d", len(manifest.Layers))
	}

	for _, layer := range manifest.Layers {
		if layer.MediaType != tarLayerMediaType || layer.Annotations[ocispec.AnnotationTitle] == "" || layer.Annotations[orasfile.AnnotationUnpack] != "true" {
