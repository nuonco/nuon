// Package operation defines the S3 contracts shared by the resident customer-managed
...
const (
	...
	CatalogKey            = "operations/catalog.json"
	BundleKey             = "operations/bundle.json"
	CandidateKey          = "operations/candidate.json"
	StagedCandidateKey    = "operations/staged-candidate.json"
	StackCandidateKey     = "operations/stack-candidate.json"
	CandidateStagesPrefix = "operations/candidates/"
	BundlesPrefix         = "operations/bundles/"
...
func CandidateApprovalKey(digest string) string {
	return CandidateStagesPrefix + strings.ReplaceAll(digest, ":", "-") + "/approval.json"
}
...
func CandidateSandboxApprovalKey(digest string) string {
	return CandidateStagesPrefix + strings.ReplaceAll(digest, ":", "-") + "/sandbox-approval.json"
}

func LegacyKey(key string) (string, bool) {
	if !strings.HasPrefix(key, "operations/") {
		return "", false
	}
	return legacyPrefix + strings.TrimPrefix(key, "operations/"), true
}
...
// RunStatus is the authoritative record of one operation run, stored at
...
type BundleInfo struct {
	...
	Release       *BundleReleaseIdentity `json:"release,omitempty"`
	Package       *BundlePackageIdentity `json:"package,omitempty"`
...
type BundleReleaseIdentity struct {
	ID     string `json:"id"`
	Digest string `json:"digest"`
}

type BundlePackageIdentity struct {
	ID     string `json:"id"`
	Digest string `json:"digest"`
