...
func TestValidateIdentity(t *testing.T) {
	active := operation.BundleInfo{DeploymentID: "install-1", BundleDigest: "sha256:active"}
...
func TestValidateCandidateSchemaVersion(t *testing.T) {
	require.ErrorContains(t, validateCandidateSchemaVersion(1), "v2 or newer")
	require.NoError(t, validateCandidateSchemaVersion(2))
	require.NoError(t, validateCandidateSchemaVersion(3))
}

func TestNeedsCanonicalHydration(t *testing.T) {
	require.True(t, needsCanonicalHydration(operation.BundleInfo{Contents: []operation.BundleContent{{Kind: operation.BundleContentKindAction, Name: "restart"}}}))
	require.True(t, needsCanonicalHydration(operation.BundleInfo{Contents: []operation.BundleContent{{Kind: operation.BundleContentKindComponent, Name: "api"}}}))
	require.False(t, needsCanonicalHydration(operation.BundleInfo{Contents: []operation.BundleContent{{Kind: operation.BundleContentKindAction, Name: "restart", ActionDefinition: &operation.BundleActionDefinition{}}}}))
}
...
func TestStageCandidateWritesDigestScopedArchiveThenControlRecords(t *testing.T) {
	...
	record := operation.CandidateStageKey("sha256:next", fixedTime())
