...
type OperationJobHandle struct {
	job *RuntimeJob
}

func (h *OperationJobHandle) Await(ctx context.Context) error {
	select {
	case <-ctx.Done():
		return ctx.Err()
	case <-h.job.done:
		if !h.job.success {
			return fmt.Errorf("operation job failed: %s", h.job.err)
		}
		return nil
	}
}

func (h *OperationJobHandle) Result() *models.ServiceCreateRunnerJobExecutionResultRequest {
	return h.job.result
}
func (h *OperationJobHandle) Outputs() *models.ServiceCreateRunnerJobExecutionOutputsRequest {
	return h.job.outputs
}

func (h *OperationJobHandle) PlanJSON() ([]byte, error) {
	if h.job.result == nil || h.job.result.ContentsDisplayCompressed == "" {
		return nil, fmt.Errorf("operation job has no terraform plan JSON")
	}
...
func (c *Client) EnqueueOperationJob(runtimeJobID, jobType, jobGroup, jobOperation string, compositePlan json.RawMessage) (*OperationJobHandle, error) {
	return c.EnqueueOperationJobWithEnvelope(runtimeJobID, jobType, jobGroup, jobOperation, compositePlan, c.envelope)
}

func (c *Client) EnqueueOperationJobWithEnvelope(runtimeJobID, jobType, jobGroup, jobOperation string, compositePlan json.RawMessage, envelope *Envelope) (*OperationJobHandle, error) {
	...
	return &OperationJobHandle{job: job}, nil
}
...
func (c *Client) writeRuntimeJSON(job *RuntimeJob, name string, value any) error {
	...
	return c.store.WriteFile(operation.RunsPrefix+job.runID+"/steps/"+job.templateID+"/"+name, append(b, '\n'))
}
