# Offline plan compilation and late binding

How a portable bundle gets concrete, correct Terraform/OCI plans in a customer
account that the control plane has never seen and can never reach.

Two halves, one contract:

- **Compile time** (vendor side): `services/ctl-api/internal/app/customer_managed/plan_compile.go`
...
- **Run time** (customer side): `pkg/runner/customer_managed/latebind.go` re-renders each
...
For offline customer-managed installs, every one of those responsibilities moves:

| Connected BYOC | Customer-managed, offline |
|---|---|
...
- **Stack outputs** → `__NUON_CUSTOMER_MANAGED_STACK_vpc_id__`,
  `__NUON_CUSTOMER_MANAGED_STACK_region__`, ... Builtin keys the planner always injects
...
- **Sandbox outputs** → a config reference like
  ...
  `__NUON_CUSTOMER_MANAGED_SANDBOX_nuon_dns_public_domain_name__` (dots flattened only
  in the token name).
...
(`snapshot.nuon_dns.public_domain.name = "__NUON_CUSTOMER_MANAGED_SANDBOX_..."`), so
...
- Placeholder **tokens** (`__NUON_CUSTOMER_MANAGED_*__`) are globally unique markers and
