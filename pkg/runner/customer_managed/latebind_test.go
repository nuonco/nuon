...
func TestRenderStepPlanRebindsMapValuedStackOutputs(t *testing.T) {
	createPlan := json.RawMessage(`{
		"sandbox_run_plan": {
			"install_id": "in1",
			"vars": {
				"break_glass_iam_role_arn": "__NUON_CUSTOMER_MANAGED_STACK_break_glass_role_arns_0__"
			},
			"state": {
				"install_stack": {
					"populated": true,
					"outputs": {
						"break_glass_role_arns": {
							"in1-sandbox-break-glass": "__NUON_CUSTOMER_MANAGED_STACK_break_glass_role_arns_0__"
						},
						"custom_role_arns": {}
					}
				}
...
func TestRenderStepPlanRebindsEmbeddedPlaceholderTokens(t *testing.T) {
	...
	deployPlan := json.RawMessage(`{
		"deploy_plan": {
			...
			"terraform": {
				"vars": {
					"domain_name": "*.__NUON_CUSTOMER_MANAGED_SANDBOX_nuon_dns_public_domain_name__",
					"zone_id": "__NUON_CUSTOMER_MANAGED_SANDBOX_nuon_dns_public_domain_zone_id__"
				},
				"state": {
					"sandbox": {
						"outputs": {
							"nuon_dns": {
								"public_domain": {
									"name": "__NUON_CUSTOMER_MANAGED_SANDBOX_nuon_dns_public_domain_name__",
									"zone_id": "__NUON_CUSTOMER_MANAGED_SANDBOX_nuon_dns_public_domain_zone_id__"
								}
							}
						}
...
func TestRebindClusterInfoSkipsComponentBoundClusters(t *testing.T) {
	...
	plan := map[string]any{
		"sandbox_bound": map[string]any{
			"cluster_info": map[string]any{"id": "__NUON_CUSTOMER_MANAGED_CLUSTER_id__", "endpoint": "__NUON_CUSTOMER_MANAGED_CLUSTER_endpoint__", "ca_data": "__NUON_CUSTOMER_MANAGED_CLUSTER_ca_data__"},
		},
