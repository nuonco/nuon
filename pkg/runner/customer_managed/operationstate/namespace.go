...
const (
	...
	legacyOperationsPrefix = "day2/"
)

type legacyOperationsReadState struct{ State }

func WithLegacyOperationsRead(state State) State {
	return &legacyOperationsReadState{State: state}
}

func (s *legacyOperationsReadState) Get(ctx context.Context, key string) ([]byte, bool, error) {
	raw, found, err := s.State.Get(ctx, key)
	if err != nil || found || !strings.HasPrefix(key, operationsPrefix) {
		return raw, found, err
	}
	return s.State.Get(ctx, legacyOperationsPrefix+strings.TrimPrefix(key, operationsPrefix))
}

func (s *legacyOperationsReadState) List(ctx context.Context, prefix string) ([]string, error) {
	keys, err := s.State.List(ctx, prefix)
	if err != nil || !strings.HasPrefix(prefix, operationsPrefix) {
		return keys, err
	}
	legacy, err := s.State.List(ctx, legacyOperationsPrefix+strings.TrimPrefix(prefix, operationsPrefix))
	if err != nil {
		return nil, err
	}
	seen := make(map[string]bool, len(keys)+len(legacy))
	for _, key := range keys {
		seen[key] = true
	}
	for _, key := range legacy {
		mapped := operationsPrefix + strings.TrimPrefix(key, legacyOperationsPrefix)
		if !seen[mapped] {
