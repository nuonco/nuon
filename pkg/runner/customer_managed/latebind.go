...
func substituteExact(s string, subs map[string]string) string {
	...
	if strings.Contains(s, customerManagedTokenPrefix) {
		for token, replacement := range subs {
			if isCustomerManagedToken(token) && strings.Contains(s, token) {
...
const customerManagedTokenPrefix = "__NUON_CUSTOMER_MANAGED_"

var customerManagedTokenPattern = regexp.MustCompile(`^__NUON_CUSTOMER_MANAGED_[A-Za-z0-9_]+__$`)

func isCustomerManagedToken(s string) bool {
	return customerManagedTokenPattern.MatchString(s)
}
