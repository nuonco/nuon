...
// payload (e.g. the offline S3 rendezvous object, which nests objects like
...
func NewClientWithOptions(envelope *Envelope, store statestore.Store, logger *zap.Logger, options ClientOptions) (*Client, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("read customer-managed status: %w", err)
	}
	...
	if status == nil {
		...
		if err := store.WriteStatus(status); err != nil {
			return nil, fmt.Errorf("initialize customer-managed status: %w", err)
		}
		...
		if err := store.WriteStatus(status); err != nil {
			return nil, fmt.Errorf("archive previous customer-managed run: %w", err)
		}
		...
		if status.Status == statestore.RunStatusInProgress {
			...
			if err := store.WriteStatus(status); err != nil {
				return nil, fmt.Errorf("terminalize interrupted customer-managed run: %w", err)
			}
			...
			if err := store.WriteStatus(status); err != nil {
				return nil, fmt.Errorf("persist customer-managed status: %w", err)
			}
		}
	}
...
func (c *Client) CreateJobExecution(_ context.Context, id string, _ *models.ServiceCreateRunnerJobExecutionRequest) (*models.AppRunnerJobExecution, error) {
	...
	if runtime := c.runtimeJobs[id]; runtime != nil {
		now := time.Now().UTC()
		execution := &models.AppRunnerJobExecution{ID: "customer-managed-" + uuid.NewString()[:8], RunnerJobID: id, Status: models.AppRunnerJobExecutionStatusPending, CreatedAt: now.Format(time.RFC3339Nano)}
	...
	if status == nil {
		return nil, fmt.Errorf("unknown customer-managed step %q", id)
	}
	now := time.Now().UTC()
	execution := &models.AppRunnerJobExecution{ID: "customer-managed-" + uuid.NewString()[:8], RunnerJobID: id, Status: models.AppRunnerJobExecutionStatusPending, CreatedAt: now.Format(time.RFC3339Nano)}
...
func (c *Client) GetAppConfig(_ context.Context, _, _ string) (*models.AppAppConfig, error) {
	if len(c.envelope.AppConfig) == 0 {
		return nil, fmt.Errorf("customer-managed envelope has no app config")
	}
...
func (c *Client) CreateHeartBeat(context.Context, *models.ServiceCreateRunnerHeartBeatRequest) (*models.AppRunnerHeartBeat, error) {
	return &models.AppRunnerHeartBeat{ID: "customer-managed-heartbeat"}, nil
}
func (c *Client) CreateHealthCheck(context.Context, *models.ServiceCreateRunnerHealthCheckRequest) (*models.AppRunnerHealthCheck, error) {
	return &models.AppRunnerHealthCheck{ID: "customer-managed-healthcheck"}, nil
}
...
func (c *Client) CreateProcess(_ context.Context, _ *models.ServiceCreateRunnerProcessRequest) (*models.AppRunnerProcess, error) {
	...
	defer c.mu.Unlock()
	if c.process == nil {
		c.process = &models.AppRunnerProcess{ID: "customer-managed-process", RunnerID: "customer-managed-" + c.envelope.InstallID, LogStreamID: ""}
	}
...
func (c *Client) GetRunner(context.Context) (*models.AppRunner, error) {
	return &models.AppRunner{ID: "customer-managed-" + c.envelope.InstallID, OrgID: c.envelope.OrgID, Status: "active"}, nil
}
...
func unsupported(method string) error {
	return fmt.Errorf("%s is not supported in customer-managed M0", method)
}
...
func (c *Client) GetInstallActionWorkflowRun(ctx context.Context, _ string, runID string) (*models.AppInstallActionWorkflowRun, error) {
	...
	if err != nil || plan.ActionWorkflowRunPlan == nil {
		return nil, fmt.Errorf("decode operation action workflow run %q: %w", runID, err)
	}
...
func (c *Client) job(step Step) *models.AppRunnerJob {
	...
	return &models.AppRunnerJob{ID: step.ID, Type: models.AppRunnerJobType(step.JobType), Operation: models.AppRunnerJobOperationType(step.JobOperation), Group: models.AppRunnerJobGroup(step.JobGroup), Status: models.AppRunnerJobStatus(status.Status), LogStreamID: "customer-managed-" + step.ID, OrgID: c.envelope.OrgID, OwnerID: c.envelope.InstallID, ExecutionTimeout: int64(24 * time.Hour)}
}
func (c *Client) checkDoneLocked() {
	if c.result.FailedStep != "" {
		...
		if err := c.store.WriteStatus(c.status); err != nil {
			c.logger.Error("write pending-retry customer-managed status", zap.Error(err))
		}
		return
	}
...
func (c *Client) finalizeLocked(runStatus, failedStep string) {
	c.doneOnce.Do(func() {
		...
		if err := c.store.WriteStatus(c.status); err != nil {
			c.logger.Error("write final customer-managed status", zap.Error(err))
		}
		report, err := BuildReport(c.envelope, c.status, c.store)
		if err != nil {
			c.logger.Error("build customer-managed report", zap.Error(err))
		} else if err := c.store.WriteReport(report); err != nil {
			c.logger.Error("write customer-managed report", zap.Error(err))
		}
		close(c.done)
	})
}
