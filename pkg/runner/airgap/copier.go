...
// airgap bundle. Plan envelopes are exported from an online reference
	...
	for _, step := range envelope.Steps {
		...
		if plan := cp.SandboxRunPlan; plan != nil {
			...
			case plan.GitSource != nil:
				missing = append(missing, fmt.Sprintf("step %s: sandbox source is git-backed (%s); airgap plans must use a packaged oci source", step.ID, plan.GitSource.URL))
			}
		}
	}
...
// air-gapped run must never fall back to the network.
