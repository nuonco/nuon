...
// deprecatedSchemas are schema types that still exist in schema.SchemaMapping
// for runtime lookups (LSP, config parsing, API schema endpoint) but are no
// longer supported, so no reference page is generated for them. Any existing
// page is removed on regeneration.
var deprecatedSchemas = []string{
	"docker-build",
}
...
func main() {
	...
	for name := range schema.SchemaMapping {
		if slices.Contains(deprecatedSchemas, name) {
			stale := filepath.Join(*outputDir, name+".mdx")
			if err := os.Remove(stale); err == nil {
				log.Printf("[%s] Removed page for deprecated schema", name)
			} else if !os.IsNotExist(err) {
				log.Fatalf("Failed to remove stale page for deprecated schema %s: %v", name, err)
			}
			continue
		}
