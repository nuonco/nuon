...
var (
	...
	basePath       = flag.String("base-path", "/config-ref", "URL base for index card hrefs (e.g., /config-ref/latest or /config-ref/v0.19.871)")
	bannerLinkBase = flag.String("banner-link-base", "", "If set, emit a <Warning> banner on each page linking to ${banner-link-base}/<schema>")
	versionLabel   = flag.String("version-label", "", "Version label displayed in the banner (e.g., v0.19.871). Empty means no specific version is named in the banner copy.")
	stubFor        = flag.String("stub-for", "", "Comma-separated schema names to emit as stub-only pages (skips schema rendering and index page)")
)
...
func main() {
	...
	// Stub-only mode: emit a stub MDX for each requested schema name and exit.
	// Used by the promotion-snapshot script to backfill older version dirs
	// with "field added in a later version" stubs.
	if *stubFor != "" {
		names := splitAndTrim(*stubFor, ",")
		log.Printf("Stub mode: generating %d stub page(s)", len(names))
		for _, name := range names {
			if err := generateStubDoc(name, *outputDir, *format); err != nil {
				log.Fatalf("Failed to generate stub for %s: %v", name, err)
			}
		}
		duration := time.Since(startTime)
		log.Printf("✓ Successfully generated %d stub page(s) in %s (took %s)", len(names), *outputDir, duration)
		return
	}
...
// splitAndTrim splits s by sep and trims whitespace from each element, dropping empties.
...
func generateSchemaDoc(schemaName, outputDir, format string) error {
	...
	// Emit a version-warning banner if requested. The banner appears above the
	// properties table and tells the reader they're viewing an archived snapshot,
	// linking back to the current "latest" page for the same schema.
	if *bannerLinkBase != "" {
		doc.AddRaw(buildVersionBanner(schemaName))
	}

	// (Frontmatter `title:` already provides the page H1; we deliberately
	// do not emit a body `# {{Title}}` to avoid a duplicate heading.)
...
func generateIndexPage(schemaNames []string, outputDir, format string) error {
	...
	// (Frontmatter `title:` already provides the page H1; the body `#
	// Configuration Reference` heading was removed to avoid duplicating it.)
	...
	if format == "mintlify" {
		...
		for _, name := range schemaNames {
			doc.AddRaw(fmt.Sprintf("  <Card title=\"%s\" icon=\"file\" href=\"%s/%s\"></Card>\n",
				formatTitle(name), *basePath, name))
		}
...
// buildVersionBanner returns a Mintlify <Warning> block telling the reader
// they're viewing a frozen/archived snapshot. The schemaName is embedded
// into the "Latest" link so each page deep-links to its own current version.
func buildVersionBanner(schemaName string) string {
	versionPhrase := "this archived"
	if *versionLabel != "" {
		versionPhrase = *versionLabel
	}
	return fmt.Sprintf(
		"<Warning>\nYou're viewing the **%s** reference. Field definitions may differ from the version of Nuon you're running. For current field definitions, see this page on [**Latest**](%s/%s).\n</Warning>\n\n",
		versionPhrase, *bannerLinkBase, schemaName,
	)
}

// generateStubDoc writes a minimal MDX page for a schema that doesn't exist
// in this version of Nuon. It's used to backfill older version-named
// snapshots when a new schema is added in a later release, so the version
// dropdown can switch pages without 404'ing.
func generateStubDoc(schemaName, outputDir, format string) error {
	log.Printf("[%s] Generating stub page...", schemaName)
	...
	versionPhrase := "this version of Nuon"
	if *versionLabel != "" {
		versionPhrase = *versionLabel
	}
	note := fmt.Sprintf(
		"<Note>\nThe **`%s`** configuration is not available in this version of Nuon (`%s`).\n\nIt was added in a later version. To use this configuration, upgrade your Nuon installation, then view the field reference on the [**Latest**](%s/%s) version of this page.\n</Note>\n\n",
		schemaName, versionPhrase, *bannerLinkBase, schemaName,
	)
	doc.AddRaw(note)
	...
	log.Printf("[%s] ✓ Stub written to %s", schemaName, filename)
	return nil
}
