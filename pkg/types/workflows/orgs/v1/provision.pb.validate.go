...
func (m *ProvisionRequest) validate(all bool) error {
	...
	if _, ok := _ProvisionRequest_Region_InLookup[m.GetRegion()]; !ok {
		err := ProvisionRequestValidationError{
			field:  "Region",
...
var _ProvisionRequest_Region_InLookup = map[string]struct{}{
	"us-west-2": {},
}
...
func (m *ProvisionResponse) validate(all bool) error {
	...
	if all {
		switch v := interface{}(m.GetKms()).(type) {
		case interface{ ValidateAll() error }:
			if err := v.ValidateAll(); err != nil {
				errors = append(errors, ProvisionResponseValidationError{
					field:  "Kms",
	...
	} else if v, ok := interface{}(m.GetKms()).(interface{ Validate() error }); ok {
		if err := v.Validate(); err != nil {
			return ProvisionResponseValidationError{
				field:  "Kms",
				reason: "embedded message failed validation",
