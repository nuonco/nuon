...
func (a AppAWSIAMPolicy) JSONSchemaExtend(schema *jsonschema.Schema) {
	NewSchemaBuilder(schema).
		...
		Field("contents").Short("[AWS] inline policy document").
		...
		Field("gcp_permissions").Short("[GCP] individual permissions").
		...
		Field("gcp_predefined_role").Short("[GCP] predefined role").
