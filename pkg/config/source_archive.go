...
const (
	maxSourceArchiveFileBytes = 5 << 20
	maxSourceArchiveBytes     = 25 << 20
)
...
func (a *SourceArchive) AddFile(relative string, contents []byte) error {
	...
	if len(contents) > maxSourceArchiveFileBytes {
		return fmt.Errorf("source file %s exceeds %d bytes", relative, maxSourceArchiveFileBytes)
	}
