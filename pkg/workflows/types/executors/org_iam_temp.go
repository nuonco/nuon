...
func AwaitProvisionIAM(ctx workflow.Context, workflowID string, req *ProvisionIAMRequest) (*ProvisionIAMResponse, error){
	...
	var resp ProvisionIAMResponse
	if err := workflow.ExecuteChildWorkflow(ctx, ProvisionIAM, req).Get(ctx, &resp); err != nil {
...
// AwaitProvisionIAM executes [ProvisionIAM] as a Temporal workflow, waits for the future to complete, then returns the 
// error, if any.
func AwaitDeprovisionIAM(ctx workflow.Context, workflowID string, req *DeprovisionIAMRequest) (*DeprovisionIAMResponse, error){
	cwo := workflow.ChildWorkflowOptions{
		TaskQueue:                "executors",
		WorkflowID:               workflowID,
		WorkflowExecutionTimeout: 10 * time.Minute,
	...
	// key for a name lookup against its activity registry, so that it knows what activity function
	// to actually call when the workflow is ready to be executed.
	var resp DeprovisionIAMResponse
	if err := workflow.ExecuteChildWorkflow(ctx, DeprovisionIAM, req).Get(ctx, &resp); err != nil {
    return nil, fmt.Errorf("unable to execute child workflow: %w", err)
  }
