...
func AwaitProvisionDNSDelegation(ctx workflow.Context, workflowID string, req *ProvisionDNSDelegationRequest) (*ProvisionDNSDelegationResponse, error){
	...
	var resp ProvisionDNSDelegationResponse
	if err := workflow.ExecuteChildWorkflow(ctx, ProvisionDNSDelegation, req).Get(ctx, &resp); err != nil {
...
// error, if any.
func AwaitDeprovisionDNSDelegation(ctx workflow.Context, workflowID string, req *DeprovisionDNSDelegationRequest) (*DeprovisionDNSDelegationResponse, error){
	cwo := workflow.ChildWorkflowOptions{
		TaskQueue:                "executors",
		WorkflowID:               workflowID,
		WorkflowExecutionTimeout: 10 * time.Minute,
	...
	// key for a name lookup against its activity registry, so that it knows what activity function
	// to actually call when the workflow is ready to be executed.
	var resp DeprovisionDNSDelegationResponse
	if err := workflow.ExecuteChildWorkflow(ctx, DeprovisionDNSDelegation, req).Get(ctx, &resp); err != nil {
    return nil, fmt.Errorf("unable to execute child workflow: %w", err)
  }
