...
//
// When shallow clones are enabled, a ref that names a branch or tag is cloned
// shallow (depth 1, single branch, no tags), which avoids downloading the
// repo's whole history. A full commit SHA cannot take that path: go-git does
// not implement the want-SHA capability, so ReferenceName has to resolve to a
// real ref and the commit is only guaranteed to be present after an unshallow
// clone.
func (w *Workspace) clone(ctx context.Context) error {
	if w.shallowCloneEnabled && !IsFullCommitSHA(w.src.Ref) {
