...
func cloneInto(t *testing.T, srcDir, ref string, opts ...Option) *Workspace {
	t.Helper()
...
func TestCloneRefRouting(t *testing.T) {
	...
	t.Run("branch ref checks out the branch tip, shallow", func(t *testing.T) {
		ws := cloneInto(t, srcDir, branch, EnableShallowClones(true))
	...
	t.Run("full sha checks out that exact commit", func(t *testing.T) {
		ws := cloneInto(t, srcDir, first.String(), EnableShallowClones(true))
	...
	t.Run("full sha keeps history rather than going shallow", func(t *testing.T) {
		ws := cloneInto(t, srcDir, second.String(), EnableShallowClones(true))
...
func TestCloneShallowConfig(t *testing.T) {
	srcDir, branch, _, _ := seedRepo(t)
	...
	for _, tc := range []struct {
		...
		{name: "unset defaults to full clone"},
		{name: "env enables", env: "true", wantShallow: true},
		{name: "env disables", env: "false"},
		{name: "unparseable env falls back to full clone", env: "maybe"},
		{name: "option overrides env off", env: "false", opts: []Option{EnableShallowClones(true)}, wantShallow: true},
		{name: "option overrides env on", env: "true", opts: []Option{EnableShallowClones(false)}},
		{name: "option without env", opts: []Option{EnableShallowClones(true)}, wantShallow: true},
	} {
		t.Run(tc.name, func(t *testing.T) {
			t.Setenv(shallowCloneEnvVar, tc.env)

			ws := cloneInto(t, srcDir, branch, tc.opts...)
			assert.Equal(t, tc.wantShallow, isShallow(t, ws.Root()))
		})
	}
}
