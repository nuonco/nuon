...
func Test_NewDockerBuild(t *testing.T) {
	v := validator.New()
	...
	tests := map[string]struct {
		optsFn      func() []Option
		errExpected error
		assertFn    func(*testing.T, *dockerBuild)
	}{
		"happy path": {
			optsFn: func() []Option {
				return []Option{
		...
		"missing docker config": {
			...
			errExpected: fmt.Errorf("docker config not provided"),
		},
	}
...
func Test_NewDockerBuild_Render(t *testing.T) {
	v := validator.New()
	waypointRef := generics.GetFakeObj[*planv1.WaypointRef]()
	component := generics.GetFakeObj[*componentv1.Component]()
	ecrRef := generics.GetFakeObj[*planv1.ECRRepositoryRef]()
	...
	tests := map[string]struct {
		builderFn func(*testing.T) *dockerBuild
		assertFn  func(*testing.T, waypointv1.Hcl_Format, []byte)
	}{
		"happy path": {
			builderFn: func(t *testing.T) *dockerBuild {
				bld, err := NewDockerBuild(v,
					WithComponent(component),
					WithEcrRef(ecrRef),
					WithDockerCfg(dockerCfg),
					WithWaypointRef(waypointRef))
			...
			assertFn: func(t *testing.T, fmt waypointv1.Hcl_Format, byts []byte) {
				...
				assert.Equal(t, "docker", appBlock.Build[0].Use[0].Name)
